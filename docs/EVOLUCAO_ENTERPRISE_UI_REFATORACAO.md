...
- Backend/WhatsApp: fatias de 2026-08-22 separaram interpretacao de consultas, respostas/politicas deterministicas, suporte/etapas de checkout e rascunhos de pedido em modulos dedicados. `processor.py` preserva a fachada, os imports historicos e os pontos de substituicao das chamadas remotas, caiu de `3932` para cerca de `1650` linhas e ganhou contrato arquitetural com limite por modulo. Proxima fatia recomendada: avaliar a separacao entre orquestracao de IA, envio e handoff, sem alterar tenant, metricas ou contratos atuais.
