...
- Backend/WhatsApp: fatias de 2026-08-22 e 2026-08-23 separaram interpretacao de consultas, respostas/politicas deterministicas, suporte/etapas de checkout, rascunhos de pedido, orquestracao de IA/function calling e envio/handoff/metricas em modulos dedicados. `processor.py` preserva a fachada, os imports historicos e os pontos de substituicao das chamadas remotas e do sender, caiu de `3932` para `1019` linhas e ganhou contrato arquitetural com limite por modulo. Proxima fatia recomendada: avaliar a separacao do roteamento central de intents apenas quando houver uma nova mudanca funcional nesse fluxo, evitando refatoracao sem ganho operacional.
...
- `backend/app/analise_racoes_routes.py`: 857 -> 29 linhas fisicas, mantendo o `router` publico com prefixo `/racoes/analises` e reexports historicos.
- `backend/app/analise_racoes_routes_parts/resumo_routes.py`: concentra o resumo do dashboard.
- `backend/app/analise_racoes_routes_parts/segmentos_routes.py`: concentra margem por segmento, comparacao de marcas e ranking de vendas.
- `backend/app/analise_racoes_routes_parts/filtros_routes.py`: concentra as opcoes dinamicas de filtros.
- `backend/app/analise_racoes_routes_parts/produtos_routes.py`: concentra a listagem de produtos para comparacao.
- Contrato dedicado: `backend/tests/unit/test_backend_large_files_700_batch_22_refactor.py`, garantindo endpoints publicos, reexports e modulos abaixo de 700 linhas.
...
- `backend/app/routes/ecommerce_checkout.py`: 917 -> 592 linhas, mantendo o prefixo publico `/checkout`, os endpoints e os fallbacks de pagamento no router principal.
- `backend/app/routes/ecommerce_checkout_support.py`: concentra schemas, identidade do checkout, frete local, carrinho, expiracao de reservas, idempotencia, origem e validacao de pagamentos online.
- Contrato dedicado: `backend/tests/unit/test_backend_large_files_700_batch_10_refactor.py`, garantindo fachada, paths publicos, helpers reexportados e modulos abaixo de 700 linhas.

Fatia backend 700 batch 9 de 2026-06-27: `backend/app/whatsapp/tools.py` saiu da faixa acima de 700 linhas ao virar fachada das tools de WhatsApp:

- `backend/app/whatsapp/tools.py`: 929 -> 8 linhas, mantendo os imports publicos `TOOLS_DEFINITIONS`, `ToolExecutor`, `_normalize_text` e `_only_digits`.
- `backend/app/whatsapp/tool_definitions.py`: concentra as definicoes das tools usadas no OpenAI Function Calling.
- `backend/app/whatsapp/tool_executor.py`: concentra o executor e os handlers de busca de produtos, pedidos, historico, loja, agendamento, carrinho, frete e finalizacao.
- `backend/app/whatsapp/tool_utils.py`: concentra normalizacao textual e extracao de digitos compartilhadas.
- Contrato dedicado: `backend/tests/unit/test_backend_large_files_700_batch_9_refactor.py`, garantindo fachada, imports antigos, tools esperadas e modulos extraidos abaixo de 700 linhas.
...
- `backend/app/cliente_info_pdv.py`: 962 -> 524 linhas, mantendo os endpoints de alertas de carrinho e informacoes PDV.
- `backend/app/cliente_info_pdv_chat.py`: concentra o endpoint `/clientes/{cliente_id}/chat-pdv`, montagem de contexto e fallback sem IA.
- `backend/app/cliente_info_pdv_schemas.py`: concentra os schemas `ItemCarrinhoIA`, `AlertasCarrinhoRequest` e `ChatPDVRequest`.
- Contrato dedicado: `backend/tests/unit/test_backend_large_files_700_batch_3_refactor.py`, garantindo paths publicos e arquivos abaixo de 700 linhas.
...
- `frontend/src/pages/comissoes/RelatoriosComissoes.jsx`: 826 -> 328 linhas, com secoes de relatorio em `frontend/src/pages/comissoes/relatorios/RelatoriosComissoesSections.jsx`.
- `frontend/src/components/ModuloBloqueado.jsx`: 821 -> 422 linhas, com previews animados em `frontend/src/components/moduloBloqueado/ModuloBloqueadoPreviews.jsx`.
- `frontend/src/components/caixa/ModalFecharCaixaContent.jsx`: 818 -> 373 linhas, com auxiliar de contagem, vendas por forma e dicas de diferenca em `frontend/src/components/caixa/ModalFecharCaixaPanels.jsx`.
- `frontend/src/components/SugestoesInteligentesRacoes.jsx`: 812 -> 369 linhas, com paineis de resumo, duplicatas, padronizacao e gaps em `frontend/src/components/racoes/SugestoesInteligentesRacoesPanels.jsx`.
- Contrato dedicado: `frontend/scripts/test-large-files-700-batch-3-refactor.mjs`, garantindo que os alvos e modulos extraidos sigam abaixo de 700 linhas.
