...
- Backend/WhatsApp: fatia de 2026-08-22 separou interpretacao de consultas de catalogo em `app/whatsapp/catalog_query_helpers.py` e respostas/politicas deterministicas em `app/whatsapp/conversation_helpers.py`. `processor.py` preserva a fachada e os imports historicos, caiu de `3932` para cerca de `3010` linhas e ganhou contrato arquitetural com limite por modulo. Proxima fatia recomendada: isolar checkout e rascunhos de pedido, mantendo monkeypatches, tenant, envio e contratos atuais.
