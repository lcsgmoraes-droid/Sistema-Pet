...
Na identificação de componentes, a combinação legada exata `tipo_produto=SIMPLES` e `tipo_kit=VIRTUAL` é uma folha conforme `EstoqueReservaService._usa_composicao_virtual`: não deve transformar uma composição identificada em incerteza para todo o catálogo. O componente permanece bloqueado localmente e suas regras próprias de custo/estoque continuam retornando `kit_policy_unsupported`; não se altera cadastro nem se libera saldo de kit. A exceção não aceita `FISICO`, marcador desconhecido ou `VARIACAO + VIRTUAL` aninhada, e preserva todas as verificações de empresa, identidade, exclusão e quantidade.
