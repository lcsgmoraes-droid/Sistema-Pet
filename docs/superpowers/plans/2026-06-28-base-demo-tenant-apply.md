...
Add assertions that `apply_seed_plan(..., tenant_email="corepeterp@gmail.com")` includes `tenant_email` in the result and that CLI dry-run accepts `--tenant-email corepeterp@gmail.com`.
...
Add a fake session/query test for `resolve_tenant_context_by_email(fake_db, "corepeterp@gmail.com")`, expecting tenant id, user id, and normalized email.
...
Document the dry-run command with `--tenant-email corepeterp@gmail.com`, explain that real apply is only for DEV/demo and not a production deploy.
