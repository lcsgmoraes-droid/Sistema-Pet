...
### 0.1. Escopo comercial, trial e Beta
...
Nao vender ainda como parte contratada do Plano Basico:
...
- Banho & Tosa completo.
- Campanhas, WhatsApp, IA avancada, e-commerce, app mobile, entregas, marketplaces e fiscal/NF.
...
### 0.7. Smoke real em producao - 2026-05-16

Rodadas executadas contra `https://mlprohub.com.br`, sem deploy, sem SSH e sem criar venda, cliente, produto ou estoque.

| Rodada | Resultado | Observacao |
|---|---|---|
| Smoke publico | 6/6 OK | Health e paginas publicas (`/login`, `/termos`, `/privacidade`, `/verificar-email`). |
| Smoke autenticado em tenant full/legado | 17/17 OK | Tenant tinha `plano_legado_liberado=true` e todos os modulos ativos; nao serve para validar bloqueio do Basico. |
| Smoke autenticado em tenant Basico | 17/17 OK | Tenant `basico`, trial, `plano_legado_liberado=false`, sem modulos premium ativos. |
| Admin Basico - gates principais | 9/9 OK | Basico liberou produtos/clientes/PDV/configuracoes essenciais e bloqueou financeiro ERP, SEFAZ/compras e contas. |
...
- Usuario temporario do operador Basico teve logout forcado e foi desativado.
- Role temporaria do operador Basico ficou com 0 permissoes.
- Nenhum dado operacional de venda, cliente, produto ou estoque foi criado.
...
- Comparacao com outro tenant:
  ...
  - A rodada API/visual `20260515222244` confirmou operador nao-admin em tenants A/B, produto/cliente/venda/financeiro do cliente isolados por tenant, `Financeiro > Vendas` em modo historico por cliente, abertura da venda no PDV por `/pdv?venda=...` sem deslogar, Chat IA financeiro e Contas a Pagar bloqueados no Basico. Apos ajuste em `ModuloBloqueado`, URL direta premium nao monta filhos nem gera 403 no console.
...
| Pessoas | Financeiro do cliente | `/clientes/:id/financeiro` | Endpoints de resumo financeiro e vendas | Sim por API e navegador | Contrato estatico confirma rota liberada no Basico por `clientes.visualizar`, sem gate `financeiro_erp`, e backend filtrando cliente/vendas/contas por tenant. Rodada `20260515222244` confirmou historico financeiro do cliente com operador nao-admin e 404 no cliente do outro tenant. | Contrato PR #39 adicionado para evitar regressao de tenant/gate. | OK local + smoke visual |
...
| Financeiro | Vendas | `/financeiro/vendas` | Endpoints de historico/listagem de vendas | Sim por API e navegador | Operador nao-admin no Plano Basico acessou apenas o historico por cliente, pesquisou cliente do proprio tenant, viu a venda finalizada e abriu a venda no PDV por `/pdv?venda=...` sem cair na landing/logout. | `ModuloBloqueado` passou a nao montar filhos premium enquanto o status de modulos carrega, evitando 403 indevido em URL direta premium. | OK local + smoke visual |
...
| Financeiro Vendas | Historico/reflexos de venda nao vazaram na rodada A/B local; acesso cruzado a venda/financeiro do cliente retornou 404 | Sim, finalizacao gerou DRE/financeiro sem 500 depois do alinhamento de schema | Historico por cliente abriu venda propria e bloqueou cliente/venda do outro tenant por ID | Nao testado | API premium bloqueada retornou 403; rota visual premium mostrou upsell sem chamadas indevidas apos ajuste | OK local + smoke visual |
...
| Modulos premium bloqueados | Bling, WhatsApp, Chat IA financeiro e Contas a Pagar nao ficam liberados no plano basico | Nao aplicavel | Nao aplicavel | Nao aplicavel | Chat IA financeiro e Contas a Pagar retornaram 403 por API; URL direta `/ia/chat` mostrou upsell premium sem montar o componente bloqueado | OK local + smoke visual |

## 5. Correcoes aplicadas

| Arquivo alterado | Problema encontrado | Como foi corrigido | Risco antes | Como validou depois |
|---|---|---|---|---|
| `frontend/src/pages/ClientesNovo.jsx`, `frontend/src/components/clientes/ClientesNovoActionsBar.jsx`, `frontend/src/components/clientes/ClientesNovoTabelaSection.jsx` | Tela `/clientes` ainda exibia header, loading, bloco de busca/acoes, empty state e acoes de linha com classes locais; o botao `Novo` estava roxo em vez da cor semantica de criacao. | Migrado para `PageHeader`, `Panel`, `LoadingState`, `EmptyState`, `ActionButton` e `IconActionButton`; `Novo` usa `intent=create` e `Importar Excel` usa acao secundaria neutra. | A tela de Pessoas ficava fora do padrao visual do Plano Basico e podia confundir a regra de cor por intencao. | `npm --prefix frontend run build`. |
...
| `frontend/src/components/ModuloBloqueado.jsx`, `backend/tests/unit/test_plano_basico_tenant_contract.py` | URL direta premium podia montar o componente filho enquanto o status de modulos ainda carregava, gerando chamadas 403 antes do upsell. | `ModuloBloqueado` agora nao renderiza filhos enquanto `modulosAtivos` esta carregando; contrato impede voltar a `return children` nesse estado. | Console com 403 em tenant basico e possivel chamada indevida a endpoint premium. | Smoke visual `/ia/chat` com operador Basico: upsell exibido, nenhuma chamada `/chat/conversas`, console limpo; contrato `22 passed`; build frontend passou. |
