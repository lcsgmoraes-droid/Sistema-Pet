...
def _load_migration_smoke_module():
    spec = importlib.util.spec_from_file_location(
        "ci_migration_smoke", MIGRATION_SMOKE_SCRIPT
    )
...
def test_migration_smoke_runs_rls_no_debt_guard_after_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        smoke, "_database_url", lambda: smoke.make_url("postgresql://example")
    )
    ...
    monkeypatch.setattr(
        smoke, "_prepare_database", lambda *args: calls.append(("prepare", args))
    )
    monkeypatch.setattr(
        smoke, "_drop_database", lambda *args: calls.append(("drop", args))
    )
    monkeypatch.setattr(
        smoke, "_run_alembic", lambda *args: calls.append(("alembic", args))
    )
    monkeypatch.setattr(
        smoke, "_assert_database_at_head", lambda *args: calls.append(("head", args))
    )
    monkeypatch.setattr(
        smoke, "_assert_rls_no_debt", lambda *args: calls.append(("rls", args))
    )
