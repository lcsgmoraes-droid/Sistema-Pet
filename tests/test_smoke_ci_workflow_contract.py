...
def test_frontend_package_exposes_core_lint_and_format_scripts():
    ...
    assert (
        ...
        == 'eslint src/api "src/components/**/*.{js,jsx,mjs,cjs}" src/hooks src/utils src/helpers scripts --max-warnings=0'
    )
    assert (
        package["scripts"]["format:core:check"]
        == 'prettier --check src/api "src/components/**/*.{js,jsx,mjs,cjs}" src/hooks src/utils src/helpers scripts'
    )
