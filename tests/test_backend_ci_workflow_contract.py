...
def test_backend_ci_has_blocking_backend_tests_lint_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")

    assert "Backend tests lint (blocking)" in source
    assert "          ruff check tests\n" in source


def test_backend_ci_has_blocking_backend_app_tests_lint_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")

    assert "Backend app tests lint (blocking)" in source
...
def test_backend_ci_has_blocking_backend_critical_packages_format_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")

    assert "Backend critical packages format (blocking)" in source
    assert (
        "ruff format --check app/core app/tenancy app/auth app/db app/schemas app/security"
        in source
    )
...
def test_backend_ci_has_blocking_backend_banho_tosa_root_format_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")

    assert "Backend banho tosa root format (blocking)" in source
...
def test_backend_ci_has_blocking_backend_routers_format_step():
    ...
    assert "Backend routers format (blocking)" in source
...
def test_backend_ci_has_blocking_backend_ai_packages_format_step():
    ...
    assert "Backend AI packages format (blocking)" in source
...
def test_backend_ci_has_blocking_backend_whatsapp_format_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")
...
def test_backend_ci_has_blocking_backend_api_banho_tosa_format_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")

    assert "Backend api banho tosa format (blocking)" in source
...
def test_backend_ci_has_blocking_backend_multi_tenant_tests_format_step():
    ...
    assert "Backend multi tenant tests format (blocking)" in source
    assert "ruff format --check tests/multi_tenant" in source


def test_backend_ci_has_blocking_backend_unit_tests_format_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")
...
def test_backend_ci_has_blocking_backend_vendas_whatsapp_analytics_format_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")

    assert "Backend vendas WhatsApp analytics format (blocking)" in source
    assert (
        "ruff format --check app/vendas_routes.py app/whatsapp/analytics.py" in source
    )
