...
def test_backend_ci_has_blocking_backend_ia_dre_format_step():
    source = BACKEND_CI_WORKFLOW.read_text(encoding="utf-8")

    assert "Backend IA DRE format (blocking)" in source
    assert "ruff format --check app/ia/aba7_dre.py" in source
