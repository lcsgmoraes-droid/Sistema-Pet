...
const files = [
  ...
  "src/screens/shop/cart/CartContent.tsx",
  "src/screens/shop/cart/CartItemCard.tsx",
  "src/screens/shop/cart/CartAddressModal.tsx",
  "src/screens/shop/cart/CartStyles.ts",
  "src/screens/shop/cart/CartUtils.ts",
];
...
const utilsPath = join(root, "src/screens/shop/cart/CartUtils.ts");
if (existsSync(utilsPath)) {
  const utilsSource = readFileSync(utilsPath, "utf8");
  for (const expectedExport of [
    "export function montarEnderecoInicial",
    "export function formatarEnderecoEntrega",
    "export function buildPagamentoLabel",
    "export function formatarCep",
  ]) {
    if (!utilsSource.includes(expectedExport)) {
      failures.push(`CartUtils.ts: export esperado ausente: ${expectedExport}`);
    }
  }
}

if (failures.length) {
  console.error("Carrinho mobile batch 51 ainda fora do contrato");
  console.error(failures.join("\n"));
  console.error(counts);
  process.exit(1);
}

console.log("Carrinho mobile batch 51 abaixo de 700 linhas", counts);
