...
export async function listarStatusBanhoTosa(): Promise<BanhoTosaStatusResponse> {
  const { data } = await api.get<BanhoTosaStatusResponse>(
    "/app/banho-tosa/status",
  );
  return {
...
export async function listarCalendarioBanhoTosa(params?: {
  ...
  const { data } = await api.get<BanhoTosaCalendarioResponse>(
    "/app/banho-tosa/calendario",
    {
      params: {
...
export async function criarAgendamentoBanhoTosa(
  payload: CriarAgendamentoBanhoTosaPayload,
): Promise<void> {
  await api.post("/app/banho-tosa/agendamentos", payload);
}
