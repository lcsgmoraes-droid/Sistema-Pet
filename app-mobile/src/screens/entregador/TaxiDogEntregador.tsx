...
export function TaxiDogEntregador() {
  ...
  const carregar = useCallback(async (mostrarErro = true) => {
    try {
      ...
      if (mostrarErro) {
        Alert.alert(
          "Erro",
          error?.response?.data?.detail || "Nao foi possivel carregar os pets do Taxi Dog.",
        );
      }
    } finally {
      setLoading(false);
  ...
  async function avancar(item: TaxiDogEntregadorItem) {
    ...
    try {
      ...
      Alert.alert(
        "Erro",
        error?.response?.data?.detail || "Nao foi possivel atualizar o Taxi Dog.",
      );
    } finally {
      setSalvandoId(null);
    }
  ...
  function abrirRota(item: TaxiDogEntregadorItem) {
    ...
    if (!origem && !destino) {
      Alert.alert("Endereco", "Origem e destino ainda nao foram informados no ERP.");
      return;
    }
    const url = destino
    ...
    Linking.openURL(url).catch(() => Alert.alert("Mapa", "Nao foi possivel abrir o mapa."));
  }
    ...
    >
      <View style={taxiStyles.hero}>
        ...
        <Text style={taxiStyles.heroText}>
          Cada mudanca atualiza o ERP. Ao entregar na loja, o pet entra automaticamente na fila.
        </Text>
      </View>

      <View style={taxiStyles.dayNav}>
        <TouchableOpacity style={taxiStyles.dayButton} onPress={() => navegarDia(-1)}>
      ...
      {itens.map((item) => (
        <View key={item.id} style={taxiStyles.card}>
          <View style={taxiStyles.cardTop}>
            <View style={{ flex: 1 }}>
              <Text style={taxiStyles.pet}>{item.pet_nome || `Pet #${item.pet_id}`}</Text>
              <Text style={taxiStyles.tutor}>{item.cliente_nome || "Tutor nao informado"}</Text>
            </View>
          ...
          <Text style={taxiStyles.window}>
            Janela {hora(item.janela_inicio)} - {hora(item.janela_fim)} · {labelTipo(item.tipo)}
          </Text>
          ...
          <View style={taxiStyles.actions}>
            <TouchableOpacity style={taxiStyles.mapButton} onPress={() => abrirRota(item)}>
            ...
            {item.proximo_status ? (
              <TouchableOpacity
                ...
                style={[taxiStyles.nextButton, salvandoId === item.id && taxiStyles.disabled]}
      ...
      {!itens.length && (
        <View style={taxiStyles.empty}>
          ...
          <Text style={taxiStyles.muted}>O responsavel precisa escolher este motorista no Taxi Dog do ERP.</Text>
        </View>
      )}
    </ScrollView>
  );
...
function Endereco({ label, value }: { label: string; value?: string | null }) {
  return (
    <View style={taxiStyles.addressRow}>
      ...
      <Text style={taxiStyles.address} numberOfLines={2}>{value || "Nao informado"}</Text>
    </View>
  );
}
...
const taxiStyles = StyleSheet.create({
  ...
  dayNav: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "#FFFFFF", borderRadius: 12, padding: 8, borderWidth: 1, borderColor: "#E2E8F0" },
  dayButton: { width: 38, height: 38, borderRadius: 10, backgroundColor: "#EFF6FF", alignItems: "center", justifyContent: "center" },
  dayTitle: { flex: 1, textAlign: "center", textTransform: "capitalize", color: "#0F172A", fontWeight: "800" },
  card: { borderRadius: 14, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#E2E8F0", padding: 15 },
  cardTop: { flexDirection: "row", alignItems: "flex-start", gap: 10 },
  pet: { color: "#0F172A", fontSize: 18, fontWeight: "900" },
  tutor: { color: "#64748B", marginTop: 2 },
  badge: { borderRadius: 999, backgroundColor: "#DBEAFE", paddingHorizontal: 9, paddingVertical: 5 },
  badgeText: { color: "#1D4ED8", fontSize: 11, fontWeight: "900" },
  window: { color: "#334155", fontWeight: "700", marginTop: 12 },
  addressRow: { marginTop: 9 },
  addressLabel: { color: "#64748B", fontSize: 11, fontWeight: "900", textTransform: "uppercase" },
  address: { color: "#0F172A", marginTop: 2, lineHeight: 18 },
  actions: { flexDirection: "row", alignItems: "center", justifyContent: "flex-end", gap: 8, marginTop: 14 },
  mapButton: { minHeight: 42, flexDirection: "row", alignItems: "center", gap: 5, borderRadius: 10, borderWidth: 1, borderColor: "#BFDBFE", paddingHorizontal: 12 },
  mapButtonText: { color: "#1D4ED8", fontWeight: "800" },
  nextButton: { minHeight: 42, borderRadius: 10, backgroundColor: "#0F766E", paddingHorizontal: 14, alignItems: "center", justifyContent: "center" },
  nextButtonText: { color: "#FFFFFF", fontWeight: "900" },
  disabled: { opacity: 0.55 },
  done: { color: "#047857", fontWeight: "900" },
  empty: { alignItems: "center", borderRadius: 14, borderWidth: 1, borderStyle: "dashed", borderColor: "#CBD5E1", padding: 28 },
