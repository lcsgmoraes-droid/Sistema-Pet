...
const HistoricoInline = ({ clienteId, clienteInfo }) => {
  ...
  const toggleExpansao = async (transacao, index) => {
    ...
    if (expandedRows[key]) {
      setExpandedRows((r) => ({ ...r, [key]: false }));
      return;
    }
    setExpandedRows((r) => ({ ...r, [key]: true }));
    if (
      transacao.tipo === "venda" &&
      transacao.detalhes?.venda_id &&
      !detalhesVendas[transacao.detalhes.venda_id]
    ) {
      try {
  ...
  const getTipoIcon = (tipo) =>
    ({ venda: "🛒", devolucao: "↩️", conta_receber: "📄", recebimento: "💰", credito: "💳" })[
      tipo
    ] || "📊";
          ...
          {[
            {
              ...
              value: `R$ ${resumo.total_vendas_90d?.toFixed(2).replace(".", ",") || "0,00"}`,
              color: "text-blue-600",
            },
            {
              label: "Total em Aberto",
              value: `R$ ${resumo.total_em_aberto?.toFixed(2).replace(".", ",") || "0,00"}`,
              color: "text-orange-600",
            },
            {
              label: "Última Compra",
              value: resumo.ultima_compra
                ? `R$ ${resumo.ultima_compra.valor?.toFixed(2).replace(".", ",")}`
                : "—",
                      ...
                      <React.Fragment key={key}>
                        <tr className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-4 text-sm text-right whitespace-nowrap">
                            <span
                              className={`font-bold ${transacao.valor < 0 ? "text-red-600" : "text-green-600"}`}
                            >
                              {transacao.valor < 0 ? "- " : "+ "}R${" "}
                          ...
                          <td className="px-4 py-4 text-center whitespace-nowrap">
                            <span
                              className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadge(transacao.status)}`}
                            >
                              {transacao.status}
                            </span>
                          </td>
                          <td className="px-4 py-4 text-center whitespace-nowrap">
                            {transacao.tipo === "venda" && vendaId && (
                              <button
                                onClick={() => navigate(`/pdv?venda=${vendaId}`)}
            ...
            {paginacao && paginacao.total_paginas > 1 && (
              <div className="bg-gray-50 px-5 py-3 border-t border-gray-200 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">
                    Pág. {paginacao.pagina_atual}/{paginacao.total_paginas}
                  </span>
                  <button
                    onClick={() => mudarPagina(paginacao.pagina_atual + 1)}
...
const HistoricoVendasClienteTab = () => {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (wrapperRef.current && !wrapperRef.current.contains(e.target)) setMostrarDropdown(false);
    };
    ...
    <div className="p-6">
      ...
      <div className="mb-5">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Pesquise um cliente pelo nome, código ou telefone para visualizar seu histórico financeiro
          completo.
        </p>
      </div>
