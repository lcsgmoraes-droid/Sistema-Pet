...
export function montarDadosExameIA(exame, resumo = {}) {
  ...
  const alertasIA = Array.isArray(exame?.interpretacao_ia_alertas)
    ? exame.interpretacao_ia_alertas
    : [];
  ...
  const condutasSugeridas = Array.isArray(payloadIA.conduta_sugerida)
    ? payloadIA.conduta_sugerida
    : [];
