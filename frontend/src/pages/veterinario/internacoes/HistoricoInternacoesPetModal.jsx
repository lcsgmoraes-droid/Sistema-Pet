            ...
            {historicoPet.map((hist) => (
              <div key={hist.internacao_id} className="border border-gray-200 rounded-xl p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_CORES[hist.status] ?? "bg-gray-100 text-gray-700"}`}
                  >
                    {hist.status}
                  </span>
                </div>
                ...
                <p className="text-xs text-gray-600 mt-1">
                  Evoluções: {hist.evolucoes?.length ?? 0} • Procedimentos:{" "}
                  {hist.procedimentos?.length ?? 0}
                </p>
                {Array.isArray(hist.procedimentos) && hist.procedimentos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {hist.procedimentos.map((proc, idx) => (
                      <div
                        key={`${hist.internacao_id}_proc_${proc.id ?? idx}`}
                      ...
                      >
                        ...
                        <p className="text-gray-500">
                          {proc.horario_execucao
                            ? formatDateTime(proc.horario_execucao)
                            : formatDateTime(proc.data_hora)}
                        </p>
                        {(proc.quantidade_prevista != null ||
                          proc.quantidade_executada != null ||
                          proc.quantidade_desperdicio != null) && (
                          <p className="mt-1 text-gray-600">
                            Previsto:{" "}
                            {formatQuantity(proc.quantidade_prevista, proc.unidade_quantidade)} •
                            Feito:{" "}
                            {formatQuantity(proc.quantidade_executada, proc.unidade_quantidade)} •
                            Desperdício:{" "}
                            {formatQuantity(proc.quantidade_desperdicio, proc.unidade_quantidade)}
                          </p>
                        )}
                        {Array.isArray(proc.insumos) && proc.insumos.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {proc.insumos.map((insumo, insumoIdx) => (
                              <span
                                key={`${hist.internacao_id}_proc_${idx}_insumo_${insumoIdx}`}
