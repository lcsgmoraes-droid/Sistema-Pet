...
test("normalizarColunasSelecionadas remove duplicadas e ignora invalidas", () => {
  assert.deepEqual(
    normalizarColunasSelecionadas(["nome", "nome", "preco_total", "x"]),
    ["nome", "preco_total"],
  );
});
...
test("buildExtratoDownloadName usa contexto preferencial", () => {
  assert.equal(buildExtratoDownloadName({ consultaId: 55 }, "pdf"), "extrato_veterinario_consulta_55.pdf");
  assert.equal(buildExtratoDownloadName({ internacaoId: 9 }, "xlsx"), "extrato_veterinario_internacao_9.xlsx");
});
