...
const CategoriasFinanceiras = () => {
  ...
  const getSubcategoriasDREDaCategoria = (categoria) => {
    ...
    const porCatFinanceira = subcategoriasDRE.filter(
      (s) => s.categoria_financeira_id === categoria.id,
    );
    ...
    const subPrincipal = subcategoriasDRE.find(
      (s) => s.id === categoria.dre_subcategoria_id && !s.categoria_financeira_id,
    );
    ...
    return subcategoriasDRE.filter(
      (s) => s.categoria_id === subPrincipal.categoria_id && !s.categoria_financeira_id,
    );
  };
  ...
  const resolverCategoriaDREId = (categoriaFinanceiraId) => {
    ...
    if (categoriaFinanceira.dre_subcategoria_id) {
      const subPrincipal = subcategoriasDRE.find(
        (s) => s.id === categoriaFinanceira.dre_subcategoria_id,
      );
  ...
  const _handleDeleteSub = async (id) => {
  ...
  const _abrirModalSubcategoria = (categoriaId) => {
    setFormSubData({
                      ...
                      <div className="flex items-center gap-3 flex-1">
                        <div>
                          <div className="font-semibold text-gray-800">
                            {normalizeDisplayText(cat.nome)}
                          </div>
                          {cat.descricao && (
                            <div className="text-sm text-gray-500">
                              {normalizeDisplayText(cat.descricao)}
                            </div>
                          )}
                        </div>
                      </div>
                        ...
                        <>
                          <div className="px-6 py-2 bg-orange-50 border-b border-orange-200">
                            <span className="text-xs font-semibold text-orange-700 uppercase tracking-wide">
                              Subcategorias Financeiras ({filhasFinanceiras.length}) — classifique
                              cada uma
                            </span>
                          </div>
                                ...
                                <button
                                  onClick={() =>
                                    handleQuickTipoCusto(
                                      filha.id,
                                      filha.tipo_custo === "fixo" ? null : "fixo",
                                    )
                                  }
                                  ...
                                  <div className="flex gap-1">
                                    ...
                                    <button
                                      onClick={() =>
                                        handleQuickCustoPeDRE(
                                          sub.id,
                                          sub.custo_pe === "variavel" ? "" : "variavel",
                                        )
                                      }
                                      className={`px-2 py-1 rounded text-xs font-medium border transition-colors ${
                    ...
                    {[
                      ...
                      <button
                        ...
                        onClick={() =>
                          setFormData({
                            ...formData,
                            tipo_custo: formData.tipo_custo === opt.value ? null : opt.value,
                          })
                        }
              ...
              <div className="border-t pt-4">
                ...
                {formData.novasSubcategorias.length === 0 && (
                  <p className="text-sm text-gray-500 italic">
                    Clique em "Adicionar" ou crie a categoria primeiro e depois adicione
                    subcategorias
                  </p>
                )}
              </div>
      ...
      {showSubModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            ...
            {formSubData.categoria_id &&
              categorias.find((c) => c.id === formSubData.categoria_id) && (
