...
export default function CanalDescontos() {
  ...
  const salvar = async (e) => {
    ...
    if (isNaN(pct) || pct <= 0 || pct > 100) {
      setErro("Desconto deve ser entre 0,01% e 100%.");
      return;
    }
    ...
    if (payload.ativo) {
      const conflito = descontos.find((d) => {
        ...
        const novoInicio = payload.data_inicio
          ? new Date(payload.data_inicio).getTime()
          : Number.NEGATIVE_INFINITY;
        const novoFim = payload.data_fim
          ? new Date(payload.data_fim).getTime()
          : Number.POSITIVE_INFINITY;
        const existenteInicio = d.data_inicio
          ? new Date(d.data_inicio).getTime()
          : Number.NEGATIVE_INFINITY;
  ...
  const alternarAtivo = async (d) => {
    if (!d.ativo) {
      const conflito = descontos.find((x) => {
        ...
        const novoInicio = d.data_inicio
          ? new Date(d.data_inicio).getTime()
          : Number.NEGATIVE_INFINITY;
        const novoFim = d.data_fim ? new Date(d.data_fim).getTime() : Number.POSITIVE_INFINITY;
        const existenteInicio = x.data_inicio
          ? new Date(x.data_inicio).getTime()
          : Number.NEGATIVE_INFINITY;
