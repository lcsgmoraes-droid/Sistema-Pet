...
import { ModalClassificacao, ModalRegras } from "./conciliacaoBancaria/ConciliacaoBancariaModals";
...
export default function ConciliacaoBancaria() {
  ...
  const classificarMovimentacao = async (movimentacao, dados) => {
    try {
      await api.post(`/api/conciliacao/movimentacoes/${movimentacao.id}/classificar`, dados);
            ...
            <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-red-800">
                  <span className="font-semibold">Nenhuma instituição bancária cadastrada.</span> Vá
                  em Financeiro → Contas Bancárias e marque o checkbox "🏦 Instituição Bancária" nas
                  contas que são bancos reais (não caixa físico).
                </div>
              </div>
            </div>
          )}
            ...
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-yellow-800">
                  <span className="font-semibold">Nenhuma conta selecionada.</span> Clique em uma
                  conta acima para visualizar movimentações e fazer upload de extrato OFX.
                </div>
              </div>
            </div>
          )}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {estatisticas.total_movimentacoes}
                  </p>
                </div>
                    ...
                    movimentacoes.map((mov) => (
                      <tr key={mov.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-right font-medium">
                          <span
                            className={mov.tipo === "CREDIT" ? "text-green-600" : "text-red-600"}
                          >
