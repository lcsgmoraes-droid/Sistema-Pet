...
function MetricCard({ tipo, dados, onClick }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm font-semibold text-gray-700 mb-2">{config.titulo}</p>

      <p className="text-xs text-gray-600 mb-3">{config.descricao}</p>

      {dadosSafe?.impacto && (
        <div className="pt-3 border-t border-gray-200">
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600">Impacto estimado:</span>
            <span className={`font-bold ${config.textColor}`}>{dadosSafe.impacto}</span>
          </div>
        </div>
      )}

      <div
        className={`mt-3 w-full py-2 text-sm font-medium ${config.textColor} hover:bg-white rounded-lg transition-colors flex items-center justify-center gap-2`}
      >
        Ver detalhes
...
export default function DashboardGerencial() {
  ...
  const abrirAssistenteGestao = () => {
    navigate("/ia/chat", {
      state: {
        perguntaInicial:
          "Quero um raio-x do negócio: vendas do mês, margem, DRE e riscos do caixa.",
      },
    });
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ...
          <MetricCard
            ...
            dados={
              metricas?.clientes_endividados || { quantidade: 0, impacto: "R$ 0", clientes: [] }
            }
          ...
          <MetricCard
            ...
            dados={
              metricas?.oportunidades_novos || { quantidade: 0, impacto: "R$ 0", clientes: [] }
            }
          ...
          <MetricCard
            ...
            dados={
              metricas?.pets_sem_eventos || { quantidade: 0, impacto: "0 donos", clientes: [] }
            }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="border-l-4 border-red-500 pl-4">
              ...
              <p className="text-sm text-gray-700 mb-2">
                <strong>{metricas?.vips_inativos?.quantidade ?? 0}</strong> clientes VIP em risco de
                churn
              </p>
              <p className="text-xs text-gray-600">
                Contate imediatamente para evitar perda de{" "}
                {metricas?.vips_inativos?.impacto ?? "R$ 0"} em receita
              </p>
            </div>
            ...
            <div className="border-l-4 border-yellow-500 pl-4">
              ...
              <p className="text-sm text-gray-700 mb-2">
                <strong>{metricas?.clientes_endividados?.quantidade ?? 0}</strong> clientes
                endividados
              </p>
              <p className="text-xs text-gray-600">
                Total de {metricas?.clientes_endividados?.impacto ?? "R$ 0"} em aberto • Avaliar
                condições
              </p>
            </div>
            ...
            <div className="border-l-4 border-green-500 pl-4">
              ...
              <p className="text-sm text-gray-700 mb-2">
                <strong>{metricas?.oportunidades_novos?.quantidade ?? 0}</strong> novos clientes
                promissores
              </p>
              <p className="text-xs text-gray-600">Potencial de fidelização • Ticket médio alto</p>
            </div>
          </div>
