 ...
 *   Desbloqueia Aba 2
 */
export default function Aba1ConciliacaoVendas({ onConcluida, status: _status }) {
        ...
        if (error.response?.status === 401) {
          ...
          setErro(
            error.response?.data?.detail ||
              "Erro ao carregar operadoras. Por favor, recarregue a página.",
          );
        }
      } finally {
  ...
  const handleProcessar = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setErro(
          error.response?.data?.detail || "Erro ao processar arquivo. Por favor, tente novamente.",
        );
      }
    } finally {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Aba 1: Conciliação de Vendas</h2>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-blue-700">
                <strong>Esta aba NÃO mexe em financeiro.</strong> Apenas corrige cadastro das
                vendas. Se parcelas/valor/taxa mudarem, o sistema regenerará as Contas a Receber
                automaticamente.
              </p>
            </div>
          </div>
        ...
        <div className="bg-white border-2 border-dashed border-gray-300 rounded-lg p-6">
          <div className="text-center">
            ...
            <div className="mt-4">
              ...
              {arquivo && (
                <p className="mt-2 text-sm text-green-600 font-medium">✓ {arquivo.name}</p>
              )}
            </div>
          </div>
        </div>
              ...
              <div className="ml-3 flex-1">
                ...
                {resultado.persistido && (
                  <div className="mt-2 mb-3 p-3 bg-white rounded border border-green-200">
                    ...
                    <div className="text-xs text-gray-600 space-y-1">
                      <p>
                        • <strong>Importação ID:</strong> {resultado.importacao_id}
                      </p>
                      <p>
                        • <strong>Arquivo ID:</strong> {resultado.arquivo_id}
                      </p>
                ...
                <div className="mt-2 text-sm text-green-700">
                  <ul className="list-disc list-inside space-y-1">
                    <li>
                      <strong>{resultado.conferidas}</strong> vendas conferidas (OK)
                    </li>
                    <li>
                      <strong>{resultado.corrigidas}</strong> vendas com divergências
                    </li>
                    <li>
                      <strong>{resultado.sem_nsu}</strong> vendas sem NSU (precisa vincular)
                    </li>
                    <li>
                      <strong>{resultado.orfaos}</strong> NSUs Stone sem venda no PDV
                    </li>
                  </ul>
                </div>
              </div>
