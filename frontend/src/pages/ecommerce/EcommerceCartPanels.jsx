  ...
  return (
    <div style={S.cartItem}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "grid", gap: 4 }}>
        <div style={{ fontWeight: 700, fontSize: 14, lineHeight: 1.3, color: "#1a1a2e" }}>
          {item.nome}
        </div>
        ...
        >
          <button
            onClick={() => onUpdateItem(item.item_id, item.quantidade - 1)}
            style={S.qtyBtn}
            aria-label={`Reduzir quantidade de ${item.nome}`}
          >
            -
          </button>
          <span style={{ fontWeight: 700, fontSize: 14, minWidth: 26, textAlign: "center" }}>
            {item.quantidade}
          </span>
          <button
            onClick={() => onUpdateItem(item.item_id, item.quantidade + 1)}
          ...
          <button
            onClick={() => onUpdateItem(item.item_id, 0)}
            style={{ ...S.removeBtn, display: "inline-flex", alignItems: "center", gap: 4 }}
          >
            <Trash2 size={13} />
            Remover
          </button>
      ...
      {cartLoading ? (
        ...
        >
          ...
          <div style={{ fontSize: 14, color: "#9ca3af" }}>
            Explore nossa loja e adicione produtos!
          </div>
          <button
            onClick={onContinueShopping}
            style={{ ...S.checkoutBig, width: "auto", padding: "12px 28px" }}
          >
            Ver produtos
          </button>
        </div>
