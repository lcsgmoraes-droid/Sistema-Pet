...
function SearchIcon() {
  return (
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)' }}>
...
function HeartIcon({ active }) {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? '#f97316' : 'none'} stroke={active ? '#f97316' : 'currentColor'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div style={S.topbar}>
      <div style={S.topbarInner}>
        ...
        <span>{cartCount > 0 ? `${formatCurrency(cartTotal)} →` : 'Frete grátis acima de R$ 199'}</span>
      </div>
    </div>
  );
}
    ...
    <div style={S.header}>
      <div style={S.headerInner}>
        ...
        <div style={S.headerActions}>
          <button onClick={() => onNavigate('conta')} style={S.headerWishBtn} title={`Lista de desejos${hasWishlist ? ` (${wishlist.length})` : ''}`}>
          ...
          {customerDisplayName ? (
            ...
            <button onClick={() => onNavigate('conta')} style={{ ...S.loginBtn, gap: 6, padding: isMobile ? '7px 10px' : '7px 16px' }}>
              <UserIcon />
              {!isMobile && 'Entrar'}
            </button>
          )}
          <button onClick={() => onNavigate('carrinho')} style={{ ...S.cartBtn, padding: isMobile ? '0 12px' : '0 18px' }}>
            <CartIcon />
            {isMobile ? (cartCount > 0 ? `(${cartCount})` : '') : `Carrinho${cartCount > 0 ? ` (${cartCount})` : ''}`}
          </button>
        </div>
      </div>
    </div>
            ...
            {banner.type === 'image' ? (
              ...
              <div style={{ background: banner.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 48px', gap: 24, height: '100%' }}>
                ...
                <div>
                  ...
                  <button onClick={() => onNavigate('loja')} style={{ marginTop: 16, background: '#fff', color: '#f97316', border: 'none', borderRadius: 24, padding: '10px 24px', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
                    Ver produtos →
                  </button>
                </div>
              </div>
...
function StoreNavTabs({ cart, isMobile, styles: S, view, onNavigate }) {
  const navItems = [
    ...
    { id: 'carrinho', label: cart?.itens?.length ? `Carrinho (${cart.itens.length})` : 'Carrinho', icon: <CartIcon size={15} /> },
    ...
    <div style={{ ...S.navWrap, overflowX: isMobile ? 'auto' : 'visible' }}>
      <div style={S.navInner}>
        {navItems.map(({ id, label, icon }) => (
          <button key={id} onClick={() => onNavigate(id)} style={{ ...S.navTab(view === id), display: 'flex', alignItems: 'center', gap: 5 }}>
            {icon}
            {label}
          </button>
        ))}
...
export default function EcommerceStorefrontChrome({
  ...
  return (
    <>
      ...
      <StoreNavTabs cart={cart} isMobile={isMobile} styles={S} view={view} onNavigate={onNavigate} />
    </>
  );
}
