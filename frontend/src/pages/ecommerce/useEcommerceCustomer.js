...
export default function useEcommerceCustomer({
  ...
  async function handlePasswordRecoveryRequest(e) {
    ...
    try {
      ...
      onError(extractApiErrorMessage(err, 'N\u00e3o foi poss\u00edvel iniciar a recupera\u00e7\u00e3o agora.'));
    } finally {
  ...
  async function handlePasswordRecoveryReset(e) {
    ...
    if (!normalizedEmail || !token) {
      onError(recoveryTokenFromLink ? 'Link de recuperacao invalido. Solicite um novo link.' : 'Preencha o e-mail e o codigo recebido.');
      return;
    }
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      if (response?.data?.requires_email_verification) {
        ...
        setRecoveryStep('request');
        clearRecoveryParamsFromUrl();
        onSuccess('Cadastro realizado. Enviamos um link de confirmacao para o seu e-mail antes do primeiro acesso.');
        return;
      }
