...
function CustomerProfileForm({
  ...
  const updateProfile = (field) => (e) => setProfileForm((prev) => ({ ...prev, [field]: e.target.value }));
      ...
      <div style={S.accountCard}>
        ...
        <form onSubmit={onSaveProfile} style={{ display: 'grid', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <input value={profileForm.nome} onChange={updateProfile('nome')} placeholder="Nome completo" style={S.formInput} />
            <input value={customer?.email || ''} disabled placeholder="Email" style={{ ...S.formInput, background: '#f8fafc', color: '#9ca3af' }} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <input value={profileForm.telefone} onChange={updateProfile('telefone')} placeholder="Telefone *" style={S.formInput} required />
            <input value={profileForm.cpf} onChange={updateProfile('cpf')} placeholder="CPF" style={S.formInput} />
          </div>

          <div style={{ fontWeight: 600, fontSize: 13, color: '#374151', marginTop: 6, marginBottom: 2 }}>Endereço principal</div>
          <input value={profileForm.cep} onChange={updateProfile('cep')} onBlur={onProfileCepBlur} placeholder="CEP" style={S.formInput} />
          <input value={profileForm.endereco} onChange={updateProfile('endereco')} placeholder="Rua / Avenida" style={S.formInput} />
          <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 8 }}>
            <input value={profileForm.numero} onChange={updateProfile('numero')} placeholder="Número" style={S.formInput} />
            <input value={profileForm.complemento} onChange={updateProfile('complemento')} placeholder="Complemento" style={S.formInput} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px', gap: 8 }}>
            <input value={profileForm.bairro} onChange={updateProfile('bairro')} placeholder="Bairro" style={S.formInput} />
            <input value={profileForm.cidade} onChange={updateProfile('cidade')} placeholder="Cidade" style={S.formInput} />
            <input value={profileForm.estado} onChange={updateProfile('estado')} placeholder="UF" style={S.formInput} />
          </div>
          ...
          {profileForm.usar_endereco_entrega_diferente && (
            <div style={{ display: 'grid', gap: 8, background: '#faf7f4', border: '1px solid #e7e5e4', borderRadius: 12, padding: 14 }}>
              ...
              <input value={profileForm.entrega_nome} onChange={updateProfile('entrega_nome')} placeholder="Nome para entrega" style={S.formInput} />
              <input value={profileForm.entrega_cep} onChange={updateProfile('entrega_cep')} onBlur={onDeliveryCepBlur} placeholder="CEP" style={S.formInput} />
              <input value={profileForm.entrega_endereco} onChange={updateProfile('entrega_endereco')} placeholder="Rua / Avenida" style={S.formInput} />
              <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 8 }}>
                <input value={profileForm.entrega_numero} onChange={updateProfile('entrega_numero')} placeholder="Número" style={S.formInput} />
                <input value={profileForm.entrega_complemento} onChange={updateProfile('entrega_complemento')} placeholder="Complemento" style={S.formInput} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px', gap: 8 }}>
                <input value={profileForm.entrega_bairro} onChange={updateProfile('entrega_bairro')} placeholder="Bairro" style={S.formInput} />
                <input value={profileForm.entrega_cidade} onChange={updateProfile('entrega_cidade')} placeholder="Cidade" style={S.formInput} />
                <input value={profileForm.entrega_estado} onChange={updateProfile('entrega_estado')} placeholder="UF" style={S.formInput} />
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button type="button" onClick={onLogout} style={{ background: '#f1f5f9', border: '1.5px solid #e5e7eb', color: '#ef4444', borderRadius: 10, padding: '10px 20px', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
              Sair da conta
            </button>
          </div>
        </form>
...
function RegisterCard({
  ...
  const updateRegister = (field) => (e) => setRegisterForm((prev) => ({ ...prev, [field]: e.target.value }));
  const updateRegisterCheck = (field) => (e) => setRegisterForm((prev) => ({ ...prev, [field]: e.target.checked }));
  ...
  return (
    <div style={S.accountCard}>
      ...
      <form onSubmit={onRegister} autoComplete="off" style={{ display: 'grid', gap: 10 }}>
        <input name="ecommerce_register_nome" autoComplete="off" value={registerForm.nome} onChange={updateRegister('nome')} placeholder="Nome completo" style={S.formInput} />
        <input name="ecommerce_register_cpf" autoComplete="off" value={registerForm.cpf} onChange={updateRegister('cpf')} placeholder="CPF *  (000.000.000-00)" inputMode="numeric" style={S.formInput} required />
        <input name="ecommerce_register_telefone" autoComplete="off" value={registerForm.telefone} onChange={updateRegister('telefone')} placeholder="Telefone/WhatsApp *" inputMode="tel" style={S.formInput} required />
        <input name="ecommerce_register_email" autoComplete="off" value={registerForm.email} onChange={updateRegister('email')} placeholder="Email" type="email" style={S.formInput} />
        <PasswordInput
        ...
        <label style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 12, color: '#57534e', lineHeight: 1.45 }}>
          <input type="checkbox" checked={registerForm.accepted_terms} onChange={updateRegisterCheck('accepted_terms')} style={{ marginTop: 2 }} />
          <span>Li e aceito os <a href="/termos" target="_blank" rel="noreferrer" style={{ color: '#7c3aed', fontWeight: 700 }}>Termos de Uso</a>.</span>
        </label>
        <label style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 12, color: '#57534e', lineHeight: 1.45 }}>
          <input type="checkbox" checked={registerForm.accepted_privacy} onChange={updateRegisterCheck('accepted_privacy')} style={{ marginTop: 2 }} />
...
function PasswordRecoveryForm({
  ...
  const updateRecovery = (field) => (e) => setRecoveryForm((prev) => ({ ...prev, [field]: e.target.value }));
...
function LoginCard({
  ...
  const updateLogin = (field) => (e) => setLoginForm((prev) => ({ ...prev, [field]: e.target.value }));
    ...
    <div style={S.accountCard}>
      ...
      {passwordRecoveryMode ? (
        ...
        <form onSubmit={onLogin} autoComplete="off" style={{ display: 'grid', gap: 10 }}>
          <input name="ecommerce_login_email" autoComplete="off" value={loginForm.email} onChange={updateLogin('email')} placeholder="Email" type="email" style={S.formInput} />
          <PasswordInput
