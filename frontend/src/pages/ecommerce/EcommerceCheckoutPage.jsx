...
function CheckoutDeliveryForm({
  ...
  const updateAddress = (field) => (e) => setAddressFields((prev) => ({ ...prev, [field]: e.target.value }));
  ...
  return (
    <div style={S.formCard}>
      ...
      <form onSubmit={onCalculateSummary} style={{ display: 'grid', gap: 10 }}>
        <div style={{ display: 'flex', gap: 10 }}>
          {DELIVERY_OPTIONS.map(({ value, label }) => (
            <label key={value} style={deliveryMode === value ? S.radioLabelActive : S.radioLabel}>
              <input type="radio" name="deliveryMode" value={value} checked={deliveryMode === value} onChange={() => onDeliveryModeChange(value)} style={{ display: 'none' }} />
              {label}
            </label>
          ))}
        </div>
          ...
          <div style={{ background: '#faf7f4', border: '1px solid #e7e5e4', borderRadius: 10, padding: 14, display: 'grid', gap: 8 }}>
            ...
            {PICKUP_OPTIONS.map(({ value, label }) => (
              <label key={value} style={tipoRetirada === value ? S.radioLabelActive : S.radioLabel}>
                <input type="radio" name="tipoRetirada" value={value} checked={tipoRetirada === value} onChange={() => onTipoRetiradaChange(value)} style={{ display: 'none' }} />
                {label}
              </label>
            ))}
            ...
            {tipoRetirada === 'proprio' && (
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '10px 12px', background: isDrive ? '#fff7ed' : '#f8fafc', border: `1.5px solid ${isDrive ? '#f97316' : '#e5e7eb'}`, borderRadius: 10 }}>
                <input type="checkbox" checked={isDrive} onChange={(e) => onIsDriveChange(e.target.checked)} style={{ width: 18, height: 18, cursor: 'pointer' }} />
                <div>
        ...
        <input value={tenantContext?.cidade || cidadeDestino} onChange={(e) => onCidadeDestinoChange(e.target.value)} placeholder="Cidade da loja" disabled={hasTenantCity} style={{ ...S.formInput, background: hasTenantCity ? '#f8fafc' : '#fff' }} />

        {deliveryMode === 'entrega' && (
          <>
            <input value={addressFields.cep} onChange={updateAddress('cep')} onBlur={onCheckoutCepBlur} placeholder="CEP" style={S.formInput} />
            <input value={addressFields.endereco} onChange={updateAddress('endereco')} placeholder="Rua / Avenida" style={S.formInput} />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <input value={addressFields.numero} onChange={updateAddress('numero')} placeholder="Número" style={S.formInput} />
              <input value={addressFields.complemento} onChange={updateAddress('complemento')} placeholder="Complemento" style={S.formInput} />
            </div>
            <input value={addressFields.bairro} onChange={updateAddress('bairro')} placeholder="Bairro" style={S.formInput} />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: 8 }}>
              <input value={addressFields.cidade || tenantContext?.cidade || ''} onChange={updateAddress('cidade')} placeholder="Cidade" disabled={hasTenantCity} style={{ ...S.formInput, background: hasTenantCity ? '#f8fafc' : '#fff' }} />
              <input value={addressFields.estado || tenantContext?.uf || ''} onChange={updateAddress('estado')} placeholder="UF" style={S.formInput} />
            </div>
          </>
        )}
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {PAYMENT_OPTIONS.map((option) => (
            <label key={option.key} style={pagamentoTipo === option.key ? S.radioLabelActive : S.radioLabel}>
              <input type="radio" name="pagamentoTipo" value={option.key} checked={pagamentoTipo === option.key} onChange={() => onPagamentoTipoChange(option.key)} style={{ display: 'none' }} />
        ...
        {(pagamentoTipo === 'debito' || pagamentoTipo === 'credito') && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {CARD_BRANDS.map((brand) => (
              <label key={brand} style={pagamentoBandeira === brand ? S.radioLabelActive : { ...S.radioLabel, padding: '6px 12px', fontSize: 12 }}>
                <input type="radio" checked={pagamentoBandeira === brand} onChange={() => onPagamentoBandeiraChange(brand)} style={{ display: 'none' }} /> {brand}
              </label>
            ))}
          </div>
        )}
        {pagamentoTipo === 'credito' && (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            {CREDIT_INSTALLMENTS.map((installment) => (
              <label key={installment} style={pagamentoParcelas === installment ? S.radioLabelActive : { ...S.radioLabel, padding: '6px 14px' }}>
                <input type="radio" checked={pagamentoParcelas === installment} onChange={() => onPagamentoParcelasChange(installment)} style={{ display: 'none' }} /> {installment}x
              </label>
            ))}
          </div>
        )}
...
function CheckoutResult({ result }) {
  ...
  return (
    <div style={{ background: '#ecfdf5', border: '1.5px solid #6ee7b7', borderRadius: 12, padding: 14, marginTop: 8, display: 'grid', gap: 6 }}>
      ...
      <div style={{ fontSize: 13, color: '#374151' }}>Número: <strong>{result.pedido_id}</strong></div>
      ...
      {result.palavra_chave_retirada && (
        <div style={{ background: '#fff7ed', border: '2px solid #f97316', borderRadius: 10, padding: 12, textAlign: 'center', marginTop: 4 }}>
          ...
          <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: 3, color: '#ea580c' }}>{result.palavra_chave_retirada}</div>
...
function CheckoutSummary({
  ...
  return (
    <div style={S.resumoBox}>
      ...
      <button onClick={onFinalizeCheckout} disabled={finalizeDisabled} style={S.finalizarBtn(finalizeDisabled)}>
...
export default function EcommerceCheckoutPage({
  ...
  const finalizeDisabled = checkoutLoading || !(tenantContext?.cidade || cidadeDestino) || !cart?.itens?.length || !isProfileComplete;
