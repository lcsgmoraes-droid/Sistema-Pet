...
const form = { modelo: "55", serie: "003", ambiente_codigo: "2", proximo_numero: "4501" };
...
test("revisão identifica ambiente, modelo, série e número consultado", () => {
  ...
  assert.equal(currentSequence(rows, "3", "1", "55").ultimoNumero, 4500);
...
test("NFC-e usa sequência própria do modelo 65", () => {
  assert.deepEqual(
    prepareNumbering(rows, {
      ...form,
      modelo: "65",
      proximo_numero: "101",
    }).payload,
    {
      ...
      modelo: 65,
      proximo_numero: 101,
      ultimo_numero_consultado: 99,
    },
  );
  assert.ok(prepareNumbering(rows, { ...form, modelo: "57" }).error);
});
