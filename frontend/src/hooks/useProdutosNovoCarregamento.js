...
export default function useProdutosNovoCarregamento({
      ...
      if (requestAindaAtiva()) {
        setFormData((prev) => ({
          ...
          tributacao: {
            ...
            codigo_beneficio_fiscal: data.codigo_beneficio_fiscal ?? "",
            fcp_aliquota: data.fcp_aliquota ?? "",
  ...
  const salvarFiscal = async (produto) => {
    const payload = {
      ...
      codigo_beneficio_fiscal: formData.tributacao.codigo_beneficio_fiscal,
      fcp_aliquota: formData.tributacao.fcp_aliquota,
