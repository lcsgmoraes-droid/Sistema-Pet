export default function CampanhasRankingClientesTable({ ranking, rankLabels, formatBRL }) {
  return (
    <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
      ...
      {ranking.clientes.length === 0 ? (
        <div className="p-8 text-center text-gray-400">Nenhum cliente neste nivel.</div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium text-gray-600">Gasto total</th>
                <th className="px-4 py-3 text-center font-medium text-gray-600">Compras</th>
                <th className="px-4 py-3 text-center font-medium text-gray-600">Meses ativos</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {ranking.clientes.map((cliente, index) => {
                const rankLabel = rankLabels[cliente.rank_level] || rankLabels.bronze;
                return (
                  <tr key={cliente.customer_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center text-gray-500">{cliente.active_months}</td>
                  </tr>
                );
              })}
            </tbody>
