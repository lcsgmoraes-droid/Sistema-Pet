  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium ${
        isMesmoCpf ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"
      }`}
    >
...
function ClienteCell({ cliente }) {
  return (
    <>
      ...
      {cliente.telefone && <p className="text-xs text-gray-400">Tel: {cliente.telefone}</p>}
...
function AcoesCell({ sugestao, confirmandoMerge, onConfirmarMerge }) {
  ...
  return (
    <div className="flex flex-col gap-1 items-center">
      <button
        onClick={() =>
          onConfirmarMerge(sugestao.cliente_a.id, sugestao.cliente_b.id, sugestao.motivo)
        }
      ...
      <button
        onClick={() =>
          onConfirmarMerge(sugestao.cliente_b.id, sugestao.cliente_a.id, sugestao.motivo)
        }
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b">
          <tr>
            ...
            <th className="px-4 py-3 text-left font-medium text-gray-600">Cliente A</th>
            <th className="px-4 py-3 text-left font-medium text-gray-600">Cliente B</th>
            <th className="px-4 py-3 text-center font-medium text-gray-600">Acao</th>
          </tr>
        </thead>
