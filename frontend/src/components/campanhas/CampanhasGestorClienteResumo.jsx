...
export default function CampanhasGestorClienteResumo({ gestorCliente, gestorSaldo, rankLabels }) {
  ...
  return (
    <div className="bg-white rounded-xl border shadow-sm p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-gray-900 truncate">{gestorCliente.nome}</p>
        <p className="text-xs text-gray-400">
          ID #{gestorCliente.id} -{" "}
          {gestorCliente.telefone || gestorCliente.celular || "Sem telefone"}
        </p>
      </div>
      <span className={`px-3 py-1 rounded-full text-sm font-medium shrink-0 ${rank.color}`}>
