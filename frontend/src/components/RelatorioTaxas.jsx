  ...
  const carregarDados = async () => {
    try {
      ...
      vendas.forEach((venda) => {
        ...
        const taxa =
          (venda.valor_total * (venda.taxa_percentual || 0)) / 100 + (venda.taxa_fixa || 0);
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-green-50 rounded-lg p-4">
            ...
            <div className="text-2xl font-bold text-green-900">
              {formatarMoeda(totalGeral - taxasTotais)}
            </div>
          </div>
        </div>
      </div>
        ...
        <table className="w-full">
              ...
              dados.map((item, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-orange-600">
                    {formatarMoeda(item.total_descontos)}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">
                        {((item.valor_total / totalGeral) * 100).toFixed(1)}%
                      </span>
                    </div>
                  </td>
                </tr>
          ...
          <tfoot className="bg-gray-50 font-bold">
            <tr>
              ...
              <td className="px-6 py-4">
                {formatarMoeda(dados.reduce((acc, d) => acc + d.valor_bruto, 0))}
              </td>
              <td className="px-6 py-4 text-orange-900">
                {formatarMoeda(dados.reduce((acc, d) => acc + d.total_descontos, 0))}
              </td>
              <td className="px-6 py-4 text-blue-900">
                {formatarMoeda(dados.reduce((acc, d) => acc + d.taxa_entrega, 0))}
              </td>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-800">
              Este relatório agrupa vendas por forma de pagamento E número de parcelas (ex: Crédito
              1x, Crédito 2x, Crédito 3x...). Mostra o fluxo completo: Valor Bruto → Descontos →
              Taxa Entrega → Valor Líquido → Taxas Financeiras → Valor Final Recebido. Para análises
              precisas, certifique-se de que as taxas estejam sempre atualizadas nas formas de
              pagamento antes de registrar vendas.
            </p>
          </div>
        </div>
      </div>
