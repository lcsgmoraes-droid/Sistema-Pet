...
export default function IndicesSaudeCards({ indices }) {
  ...
  if (!indices) {
    return <div className="text-center py-8 text-gray-500">Nenhum índice disponível</div>;
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 relative group">
          ...
          <div className="relative">
            ...
            {tooltipAtivo === "saldo" && (
              <div className="absolute -right-40 top-8 bg-gray-900 text-white text-xs rounded p-2 w-48 z-10 whitespace-normal">
                ...
                <p>
                  Seu saldo de caixa real neste momento. O número verde ao lado mostra a variação
                  dos últimos 7 dias.
                </p>
              </div>
            )}
        ...
        {percentual_variacao_7d && (
          <p className="text-sm text-gray-500 mt-1">
            {percentual_variacao_7d > 0 ? "+" : ""}
            {percentual_variacao_7d.toFixed(1)}% em 7 dias
          </p>
        )}
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 relative group">
        <div className="flex items-center justify-between mb-4">
          ...
          {tooltipAtivo === "dias" && (
            <div className="absolute -right-40 top-8 bg-gray-900 text-white text-xs rounded p-2 w-48 z-10 whitespace-normal">
              ...
              <p>
                Quantos dias você consegue pagar as despesas com seu saldo atual. Se tem R$ 10.000 e
                gasta R$ 1.000/dia = 10 dias.
              </p>
            </div>
          )}
        </div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          {dias_de_caixa < 7
            ? "Crítico - menos de 1 semana"
            : dias_de_caixa < 15
              ? "Alerta - menos de 2 semanas"
              : "Saudável"}
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 relative group">
        <div className="flex items-center justify-between mb-4">
          <div className={`p-3 rounded-lg ${getStatusColor(status)}`}>{getStatusIcon(status)}</div>
          <button
          ...
          {tooltipAtivo === "status" && (
            <div className="absolute -right-40 top-8 bg-gray-900 text-white text-xs rounded p-2 w-48 z-10 whitespace-normal">
              ...
              <p>
                <strong>CRITICO:</strong> &lt;7 dias (risco iminente). <strong>ALERTA:</strong> 7-15
                dias. <strong>OK:</strong> &gt;15 dias.
              </p>
            </div>
          )}
        </div>
        <h3 className="text-gray-600 text-sm font-medium mb-1">Status</h3>
        <p className="text-2xl font-bold text-gray-900 capitalize">{status || "N/A"}</p>
        <p className="text-sm text-gray-500 mt-1 capitalize">Tendência: {tendencia || "estável"}</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 relative group">
        <div className="flex items-center justify-between mb-4">
          ...
          {tooltipAtivo === "score" && (
            <div className="absolute -right-40 top-8 bg-gray-900 text-white text-xs rounded p-2 w-48 z-10 whitespace-normal">
              ...
              <p>
                Nota de 0-100 que resume a saúde financeira. Considera dias de caixa, tendência e
                volatilidade.
              </p>
            </div>
          )}
        ...
        <p className="text-2xl font-bold text-gray-900">{score_saude?.toFixed(0) || "0"}/100</p>
