...
export default function LaunchOfferBanner({ dark = false, compact = false }) {
  return (
    <aside
      className={`rounded-2xl border ${compact ? "p-3 sm:p-4" : "p-4"} sm:flex sm:items-center sm:justify-between sm:gap-6 ${
        dark
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 font-extrabold">
            {compact
              ? "30 dias com CorePet completo e implantação humana."
              : "30 dias com acesso completo ao CorePet, independentemente do plano escolhido."}
          </p>
        </div>
      </div>
      <p
        className={`mt-3 items-center gap-2 text-sm font-semibold sm:mt-0 sm:max-w-xs ${
