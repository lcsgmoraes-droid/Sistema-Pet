...
import {
  montarComparativoManualRacoes,
  racaoPodeCompararPreco,
} from "../utils/racaoComparadorManual";

const extrairProdutosCalculadora = (data) =>
  Array.isArray(data) ? data : data?.items || data?.produtos || [];
...
export default function ModalCalculadoraUniversal({
  ...
  const [comparadorBuscas, setComparadorBuscas] = useState({ a: "", b: "" });
  const [comparadorOpcoes, setComparadorOpcoes] = useState({ a: [], b: [] });
  const [comparadorSelecionadas, setComparadorSelecionadas] = useState({
    a: null,
    b: null,
  });
  const [comparadorAberto, setComparadorAberto] = useState(null);
  const [comparadorCarregando, setComparadorCarregando] = useState({
    a: false,
    b: false,
  });
  const [erroBuscaComparador, setErroBuscaComparador] = useState(null);
  ...
  const racoesCarrinho = estaNoPDV
    ...
    : [];
  const comparativoManualPreco = montarComparativoManualRacoes(
    comparadorSelecionadas.a,
    comparadorSelecionadas.b,
  );
  ...
  const formatarOpcaoComparador = (racao) => {
    const resumo = obterResumoPrecoPorKg(racao);
    const nome = resumo.nome || racao?.nome || racao?.produto_nome || "";

    if (!resumo.disponivel) {
      return nome;
    }

    return `${nome} - ${resumo.pesoFormatado} - ${resumo.precoPorKgFormatado}`;
  };

  const selecionarRacaoComparador = (lado, racao) => {
    setComparadorSelecionadas((atual) => ({
      ...atual,
      [lado]: racao,
    }));
    setComparadorBuscas((atual) => ({
      ...atual,
      [lado]: formatarOpcaoComparador(racao),
    }));
    setComparadorOpcoes((atual) => ({
      ...atual,
      [lado]: [racao],
    }));
    setComparadorAberto(null);
    setErroBuscaComparador(null);
  };

  const limparSelecaoComparador = (lado) => {
    setComparadorSelecionadas((atual) => ({
      ...atual,
      [lado]: null,
    }));
    ...
    if (termo.length >= 1) {
      const timer = setTimeout(async () => {
        ...
        try {
          ...
          const produtos = extrairProdutosCalculadora(response.data);
  ...
  useEffect(() => {
    if (!isOpen || modo !== "comparar-preco") return;

    const termo = comparadorBuscas.a.trim();
    if (
      comparadorSelecionadas.a &&
      termo === formatarOpcaoComparador(comparadorSelecionadas.a).trim()
    ) {
      setComparadorOpcoes((atual) => ({ ...atual, a: [comparadorSelecionadas.a] }));
      setComparadorAberto(null);
      setErroBuscaComparador(null);
      return;
    }
    ...
    if (termo.length >= 1) {
      const timer = setTimeout(async () => {
        setComparadorCarregando((atual) => ({ ...atual, a: true }));
        setErroBuscaComparador(null);
        try {
          const response = await api.get("/produtos/calculadora-racao/opcoes", {
            params: {
          ...
          setComparadorOpcoes((atual) => ({
            ...atual,
            a: produtos.filter(racaoPodeCompararPreco),
          }));
          setComparadorAberto("a");
        } catch (error) {
          setErroBuscaComparador("Nao foi possivel buscar racoes para comparar agora.");
          setComparadorAberto("a");
          console.error("Erro ao buscar racoes para comparacao:", error);
        } finally {
          setComparadorCarregando((atual) => ({ ...atual, a: false }));
        }
      }, 250);
      return () => clearTimeout(timer);
    }

    setComparadorOpcoes((atual) => ({ ...atual, a: [] }));
    setComparadorAberto((atual) => (atual === "a" ? null : atual));
    setErroBuscaComparador(null);
  }, [comparadorBuscas.a, comparadorSelecionadas.a, isOpen, modo]);

  useEffect(() => {
    if (!isOpen || modo !== "comparar-preco") return;

    const termo = comparadorBuscas.b.trim();
    if (
      comparadorSelecionadas.b &&
      termo === formatarOpcaoComparador(comparadorSelecionadas.b).trim()
    ) {
      setComparadorOpcoes((atual) => ({ ...atual, b: [comparadorSelecionadas.b] }));
      setComparadorAberto(null);
      setErroBuscaComparador(null);
      return;
    }
    ...
    if (termo.length >= 1) {
      const timer = setTimeout(async () => {
        setComparadorCarregando((atual) => ({ ...atual, b: true }));
        setErroBuscaComparador(null);
        try {
          const response = await api.get("/produtos/calculadora-racao/opcoes", {
            params: {
          ...
          setComparadorOpcoes((atual) => ({
            ...atual,
            b: produtos.filter(racaoPodeCompararPreco),
          }));
          setComparadorAberto("b");
        } catch (error) {
          setErroBuscaComparador("Nao foi possivel buscar racoes para comparar agora.");
          setComparadorAberto("b");
          console.error("Erro ao buscar racoes para comparacao:", error);
        } finally {
          setComparadorCarregando((atual) => ({ ...atual, b: false }));
        }
      }, 250);
      return () => clearTimeout(timer);
    }

    setComparadorOpcoes((atual) => ({ ...atual, b: [] }));
    setComparadorAberto((atual) => (atual === "b" ? null : atual));
    setErroBuscaComparador(null);
  }, [comparadorBuscas.b, comparadorSelecionadas.b, isOpen, modo]);
  ...
  const renderCampoComparador = (lado, label) => {
    const busca = comparadorBuscas[lado];
    const opcoes = comparadorOpcoes[lado];
    const selecionada = comparadorSelecionadas[lado];
    const carregandoBusca = comparadorCarregando[lado];
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <input
            ...
            onFocus={() => {
              if (busca.trim().length > 0) setComparadorAberto(lado);
            }}
            onKeyDown={(event) => {
              if (event.key === "Enter" && opcoes.length > 0) {
                event.preventDefault();
                selecionarRacaoComparador(lado, opcoes[0]);
              }
              if (event.key === "Escape") {
                setComparadorAberto(null);
              }
            ...
            className="w-full px-4 py-3 pr-14 border-2 border-gray-300 rounded-lg focus:border-teal-500 focus:ring-2 focus:ring-teal-100"
          />

          {selecionada ? (
            <button
              type="button"
              onClick={() => limparSelecaoComparador(lado)}
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-teal-700">
              Buscar
            </div>
          )}
        </div>

        {comparadorAberto === lado && opcoes.length > 0 && (
          <div className="bg-white border-2 border-gray-300 rounded-lg shadow-xl max-h-64 overflow-y-auto">
            {opcoes.map((racao) => {
              ...
              return (
                <button
                  key={`${resumo.produtoId || racao.id || resumo.nome}-${resumo.pesoKg}`}
                  type="button"
                  onMouseDown={(event) => {
                    event.preventDefault();
                    selecionarRacaoComparador(lado, racao);
                  }}
                  className="w-full px-4 py-3 text-left hover:bg-teal-50 transition-colors border-b last:border-b-0"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="font-medium text-gray-900">{resumo.nome}</div>
                      <div className="mt-1 text-xs text-gray-500">
                        Peso {resumo.pesoFormatado} | Venda {resumo.precoFormatado}
                      </div>
                    </div>
                    <span className="shrink-0 rounded-full bg-teal-50 px-2 py-1 text-xs font-semibold text-teal-700">
                      {resumo.precoPorKgFormatado}
                    </span>
                  </div>
                </button>
              );
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              ...
              <p className="text-xs md:text-sm text-orange-100">
                {modo === "comparar-preco"
                  ? "Comparar preco por kg"
                  : estaNoPDV
                    ? "Rações do carrinho"
                    : "Buscar ração manualmente"}
              </p>
            </div>
          </div>
          <button
          ...
          {modo === "comparar-preco" && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {renderCampoComparador("a", "Racao 1")}
                {renderCampoComparador("b", "Racao 2")}
              </div>
                ...
                <div className="space-y-3">
                  <div className="rounded-lg border border-teal-200 bg-teal-50 p-4">
                    ...
                    <div className="mt-1 text-base font-bold text-teal-950">
                      {comparativoManualPreco.melhor.nome}
                    </div>
                    <div className="mt-2 flex flex-wrap gap-2 text-sm text-teal-800">
                      <span>{comparativoManualPreco.melhor.pesoFormatado}</span>
                      <span>{comparativoManualPreco.melhor.precoFormatado}</span>
                      <span className="font-bold">
                        {comparativoManualPreco.melhor.precoPorKgFormatado}
                      </span>
                    </div>
                    <div className="mt-3 text-sm font-medium text-teal-900">
                      Diferenca: {comparativoManualPreco.diferencaPorKgFormatada} contra a outra
                      opcao.
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {comparativoManualPreco.itens.map((item) => (
                      <div
                      ...
                      >
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                          <div>
                            <div className="text-xs uppercase text-gray-500">Peso</div>
                            <div className="font-semibold text-gray-900">{item.pesoFormatado}</div>
                          </div>
                          <div>
                            <div className="text-xs uppercase text-gray-500">Venda</div>
                            <div className="font-semibold text-gray-900">{item.precoFormatado}</div>
                          </div>
                          <div>
                            <div className="text-xs uppercase text-gray-500">Preco/kg</div>
                            <div className="font-semibold text-teal-700">
                              {item.precoPorKgFormatado}
                            </div>
                          </div>
                          <div>
                            <div className="text-xs uppercase text-gray-500">Comparacao</div>
                            <div
                              className={`font-semibold ${
                                item.melhorOpcao ? "text-green-700" : "text-orange-700"
                              }`}
                            >
                              {item.melhorOpcao ? "Melhor opcao" : item.diferencaMelhorFormatada}
                            </div>
                          </div>
                        </div>
                      </div>
