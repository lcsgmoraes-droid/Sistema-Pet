...
function TooltipComposicao({ custo, composicao, texto = 'Ver composição' }) {
  ...
  if (!composicao || !composicao.componentes_unitario) {
    return null;
  }
        ...
        >
            ...
            {linhas.map((linha) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`font-bold ${isNegativo ? 'text-red-600' : isDestaque ? 'text-emerald-700' : 'text-slate-700'}`}>
          ...
          {composicao.tem_rateio && (
            <div className="mt-2 text-xs text-blue-700 bg-blue-50 border border-blue-200 rounded-lg p-2">
              <span className="font-semibold">💡 Rateio aplicado:</span> Valores de frete/impostos que vieram só no total foram distribuídos proporcionalmente.
            </div>
          )}
