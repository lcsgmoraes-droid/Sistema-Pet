...
export default function useEntradaXmlUpload({ api, carregarDados, toast }) {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      if (response.data.fornecedor_criado_automaticamente) {
        toast.success(`🏢 Novo fornecedor cadastrado: ${response.data.fornecedor}`, {
          duration: 4000,
        });
      }
      ...
      const errorMsg =
        error.response?.data?.detail || error.message || "Erro ao processar XML da NF-e";
  ...
  const handleMultipleFilesUpload = async (event) => {
    ...
    if (invalidFiles.length > 0) {
      toast.error(
        `❌ ${invalidFiles.length} arquivo(s) nao são XML: ${invalidFiles.map((f) => f.name).join(", ")}`,
      );
      return;
    }
    ...
    try {
      ...
      if (response.data.erros > 0) {
        toast.error(`⚠️ ${response.data.erros}/${response.data.total_arquivos} nota(s) com erro`, {
          duration: 5000,
        });
      }
