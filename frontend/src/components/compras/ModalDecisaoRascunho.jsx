...
const OpcaoRascunho = ({ titulo, descricao, destaque, onClick, intent = "slate" }) => {
  ...
  return (
    ...
    >
      ...
      {destaque ? (
        <div className="mt-3 text-xs font-semibold uppercase tracking-wide">{destaque}</div>
      ) : null}
    </button>
  );
};

const ModalDecisaoRascunho = ({ contexto, onClose, onSelecionar }) => {
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-gray-900">
                Ja existe rascunho deste fornecedor
              </h2>
              <p className="mt-1 text-sm text-gray-600">
                Escolha se quer reaproveitar o rascunho ou iniciar outro pedido para o mesmo
                fornecedor.
              </p>
            </div>
            <button
