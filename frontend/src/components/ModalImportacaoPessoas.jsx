...
export default function ModalImportacaoPessoas({ isOpen, onClose, onSuccess }) {
  ...
  const baixarTemplate = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `template_pessoas_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleImportar = async () => {
    ...
    try {
      ...
      const totalSucesso = (response.data.criados?.length || 0) + (response.data.atualizados?.length || 0);
      if (totalSucesso > 0) {
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-blue-600">{resultado.total_processados || 0}</div>
                  <div className="text-sm text-blue-800 mt-1">Total Processado</div>
                </div>
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-green-600">{resultado.criados?.length || 0}</div>
                  <div className="text-sm text-green-800 mt-1">Criados</div>
                </div>
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-yellow-600">{resultado.atualizados?.length || 0}</div>
                  <div className="text-sm text-yellow-800 mt-1">Atualizados</div>
                </div>
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-red-600">{resultado.total_erros || 0}</div>
                    ...
                    <ul className="text-sm space-y-1">
                      {resultado.criados.map((item, idx) => (
                        <li key={idx} className="text-green-800">
                          Linha {item.linha}: <span className="font-medium">{item.nome}</span> ({item.tipo})
                        </li>
                      ))}
                    </ul>
                  </div>
