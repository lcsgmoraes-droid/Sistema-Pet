...
function CardFiscal({ composicao }) {
  ...
  const linhas = [
    ...
    {
      label: "Frete",
      valor: componentes.valor_frete || 0,
      icon: "🚚",
      color: "green",
      mostrar: componentes.valor_frete > 0,
    },
    {
      label: "Seguro",
      valor: componentes.valor_seguro || 0,
      icon: "🛡️",
      color: "purple",
      mostrar: componentes.valor_seguro > 0,
    },
    {
      label: "Outras Despesas",
      valor: componentes.valor_outras_despesas || 0,
      icon: "💰",
      color: "orange",
      mostrar: componentes.valor_outras_despesas > 0,
    },
    {
      label: "Desconto",
      valor: -(componentes.valor_desconto || 0),
      icon: "📉",
      color: "red",
      mostrar: componentes.valor_desconto > 0,
    },
    {
      label: "ICMS ST",
      valor: componentes.valor_icms_st || 0,
      icon: "🏛️",
      color: "indigo",
      mostrar: componentes.valor_icms_st > 0,
    },
    {
      label: "IPI",
      valor: componentes.valor_ipi || 0,
      icon: "📋",
      color: "amber",
      mostrar: componentes.valor_ipi > 0,
    },
            ...
            {linhas.map((linha) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`text-xs font-bold mt-0.5 ${isNegativo ? "text-red-600" : textColor}`}
                  >
                    {isNegativo ? "- " : ""}R${" "}
                    {formatarValorFiscal(Math.abs(Number(linha.valor)), 2)}
                  </div>
                </div>
              );
            })}
            ...
            {(componentes.valor_icms > 0 ||
              ...
              <div className="mt-2 rounded-md bg-slate-100 border border-slate-300 p-2 text-[11px]">
                <div className="font-semibold text-slate-700 mb-1">
                  Tributos Informativos (por unidade):
                </div>
                <div className="flex flex-wrap gap-2">
                  {componentes.valor_icms > 0 && (
                    <span className="bg-white px-2 py-1 rounded text-slate-600">
                      ICMS: R${" "}
                      {formatarValorFiscal(
                        componentes.valor_icms / (composicao.quantidade_efetiva || 1),
                        4,
                      )}
                    </span>
                  )}
                  {componentes.valor_pis > 0 && (
                    <span className="bg-white px-2 py-1 rounded text-slate-600">
                      PIS: R${" "}
                      {formatarValorFiscal(
                        componentes.valor_pis / (composicao.quantidade_efetiva || 1),
                        4,
                      )}
                    </span>
                  )}
                  {componentes.valor_cofins > 0 && (
                    <span className="bg-white px-2 py-1 rounded text-slate-600">
                      COFINS: R${" "}
                      {formatarValorFiscal(
                        componentes.valor_cofins / (composicao.quantidade_efetiva || 1),
                        4,
                      )}
                    </span>
                  )}
