...
export default function ModalImportacaoProdutos({ isOpen, onClose, onSuccess }) {
  ...
  const baixarTemplate = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `template_produtos_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-blue-600 to-blue-700">
          ...
          <button onClick={fechar} className="text-white hover:text-gray-200 transition-colors">
            ...
            <>
              ...
              <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ul className="text-sm text-blue-800 space-y-1 ml-7">
                  ...
                  <li>
                    4. Produtos novos seṛo criados, existentes seṛo atualizados (baseado no SKU)
                  </li>
                </ul>
              </div>
                    ...
                    <ul className="text-sm space-y-1">
                      {resultado.detalhes.criados.map((item, idx) => (
                        <li key={idx} className="text-green-800">
                          Linha {item.linha}: <span className="font-medium">{item.sku}</span> -{" "}
                          {item.nome}
                        </li>
                      ))}
                    </ul>
