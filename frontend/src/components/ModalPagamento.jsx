...
import { X, CreditCard, Wallet, CheckCircle, AlertCircle } from "lucide-react";
                    ...
                    {venda.cliente && venda.cliente.credito > 0 && (
                      <button
                        onClick={() => {
                          ...
                          setValorRecebido(
                            Math.min(parseFloat(venda.cliente.credito), valorRestante),
                          );
                        }}
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                    ...
                    <div>
                      ...
                      {formaPagamentoSelecionada.id === "cashback" && (
                        <p className="text-xs text-gray-600 mt-1">
                          Máximo: R${" "}
                          {Math.min(saldoCashback, valorRestante).toFixed(2).replace(".", ",")}
                        </p>
                      )}
                    </div>
                    ...
                    {formaPagamentoSelecionada?.permite_parcelamento && (
                      <div ref={opcoesParcelamentoRef}>
                          ...
                          {Array.from(
                            ...
                            return (
                              <option key={n} value={n} className={estilo.optionClass}>
                                ...
                                {valorRecebido > 0
                                  ? `(Total: R$ ${valorParaParcelar.toFixed(2)})`
                                  : ""}
                              </option>
                            );
                          })}
                        ...
                        {valorRecebido > 0 && numeroParcelas > 1 && (
                          ...
                          >
                            <p
                              className={`text-sm font-medium ${estiloVisualParcelamento.tituloClass}`}
                            >
                              {estiloVisualParcelamento.prefixo}
                              💳 {numeroParcelas}x de R${" "}
                              {(valorRecebido / numeroParcelas).toFixed(2)}
                            </p>
                            <p
                              className={`text-xs mt-1 ${estiloVisualParcelamento.descricaoClass}`}
                            >
