...
export function montarItensVendaPayload(vendaAtual) {
  return (vendaAtual.itens || []).map((item) => ({
    ...
    desconto_item: normalizarNumero(item.desconto_valor ?? item.desconto_item ?? 0),
...
export function montarPayloadVenda(vendaAtual, entregadorSelecionado = null) {
  ...
  const taxaEntregaTotal = temEntrega
    ...
    : 0;
  const taxaLoja = temEntrega ? normalizarNumero(vendaAtual.entrega?.taxa_loja) : 0;
  const taxaEntregador = temEntrega ? normalizarNumero(vendaAtual.entrega?.taxa_entregador) : 0;
  const entregadorId = temEntrega
    ? vendaAtual.entregador_id || entregadorSelecionado?.id || null
    : null;

  const percentualTaxaLoja = taxaEntregaTotal > 0 ? (taxaLoja / taxaEntregaTotal) * 100 : 0;
  ...
  return {
    ...
    endereco_entrega: temEntrega ? vendaAtual.entrega?.endereco_completo || "" : null,
    observacoes_entrega: temEntrega ? vendaAtual.entrega?.observacoes_entrega || "" : null,
    distancia_km: temEntrega ? normalizarNumero(vendaAtual.entrega?.distancia_km) : null,
