...
for (const path of ["src/components/OpsLayout.jsx"]) {
  const content = read(path);
...
for (const path of [".env.example", "backend/.env.example"]) {
  ...
  assert.match(
    ...
    `${path} should document CorePet as the app name`,
  );
  assert.doesNotMatch(
    content,
    /Pet Shop Pro|PetShop ERP/,
    `${path} should not expose old product branding`,
  );
}
