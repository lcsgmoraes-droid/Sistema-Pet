...
def get_tenant_id(db):
    ...
    result = db.execute(
        text("SELECT tenant_id FROM users WHERE id = :uid LIMIT 1"), {"uid": USER_ID}
    )
    row = result.fetchone()
    if row:
        TENANT_ID = str(row[0])
...
def importar_clientes(db, limite=None):
    ...
    for row in registros:
        try:
            ...
            if codigo:
                result = db.execute(
                    text("SELECT id FROM clientes WHERE codigo = :cod"), {"cod": codigo}
                )
            ...
            result = db.execute(
                query,
                {
                    ...
                    if row.get("pes_var_email") != "NULL"
                    else None,
                    "cep": row.get("end_var_cep")
                    if row.get("end_var_cep") != "NULL"
                    else None,
                    "endereco": row.get("end_var_endereco")
                    if row.get("end_var_endereco") != "NULL"
                    else None,
                    "numero": row.get("end_var_numero")
                    if row.get("end_var_numero") != "NULL"
                    else None,
                    "complemento": row.get("end_var_complemento")
                    if row.get("end_var_complemento") != "NULL"
                    else None,
                    "bairro": row.get("end_var_bairro")
                    if row.get("end_var_bairro") != "NULL"
                    else None,
                    "cidade": row.get("end_var_municipio")
                    if row.get("end_var_municipio") != "NULL"
                    else None,
                    "estado": estado,
    ...
    db.commit()
    log(
        f"Clientes: {STATS['clientes']['novos']} novos | {STATS['clientes']['duplicado']} duplicados | {STATS['clientes']['erro']} erros",
        "OK",
    )
...
def exibir_resumo():
    ...
    print("=" * 80)
    print(
        f"{'ENTIDADE':<15} | {'TOTAL':>6} | {'NOVOS':>6} | {'DUPLIC':>6} | {'ERROS':>6}"
    )
    print("-" * 80)
    ...
    for entidade, stats in STATS.items():
        if stats["total"] > 0:
            print(
                f"{entidade.upper():<15} | {stats['total']:>6} | {stats['novos']:>6} | "
...
def main():
    parser = argparse.ArgumentParser(
        description="Importar dados do SimplesVet (versão simplificada)"
    )
    parser.add_argument("--limite", type=int, default=20, help="Limite de registros")
    parser.add_argument(
        "--all", action="store_true", help="Importar todos os registros"
    )
