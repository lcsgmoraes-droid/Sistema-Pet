...
def verificar_tipos_produtos():
    ...
    try:
        ...
        print("\nPor tipo_produto:")
        tipos = (
            session.query(Produto.tipo_produto, func.count(Produto.id))
            .filter(Produto.tenant_id == tenant_id)
            .group_by(Produto.tipo_produto)
            .all()
        )
        ...
        ocultos = (
            session.query(func.count(Produto.id))
            .filter(Produto.tenant_id == tenant_id, Produto.tipo_produto == "VARIACAO")
            .scalar()
        )
        ...
        simples = (
            ...
            .filter(Produto.tenant_id == tenant_id, Produto.tipo_produto == "SIMPLES")
            .limit(5)
            .all()
        )
