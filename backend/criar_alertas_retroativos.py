...
def criar_alertas_retroativos():
    ...
    try:
        ...
        produtos_negativos = db.query(Produto).filter(Produto.estoque_atual < 0).all()
        ...
        for produto in produtos_negativos:
            ...
            if alerta_existente:
                print(
                    f"⏭️  {produto.nome} - Já possui alerta ativo (ID: {alerta_existente.id})"
                )
                continue
            ...
            print(
                f"✅ {produto.nome} - Estoque: {produto.estoque_atual} - Alerta criado"
            )
