...
def tenant_e_usuario(db_session):
    ...
    if not tenant:
        tenant = Tenant(
            tenant_id="test_tenant_estorno", nome="Tenant Teste Estorno", ativo=True
        )
    ...
    user = (
        ...
        .filter_by(email="test_estorno@test.com", tenant_id="test_tenant_estorno")
        .first()
    )
...
class TestTransactionRollbackEstornoComissoes:
    ...
    def test_rollback_total_quando_excecao_no_meio_do_estorno(
        self, db_session, cenario_venda_com_comissoes
    ):
        """
        ...
        # Buscar comissões
        result_antes = db_session.execute(
            text("""
        ...
        for comissao in comissoes_antes:
            assert comissao[1] == "pendente", (
                f"Comissão {comissao[0]} deve estar pendente"
            )
            assert comissao[2] is None, (
                f"Comissão {comissao[0]} não deve ter data_estorno"
            )
            assert comissao[3] is None, (
                f"Comissão {comissao[0]} não deve ter motivo_estorno"
            )
            assert comissao[4] is None, (
                f"Comissão {comissao[0]} não deve ter estornado_por"
            )
        ...
        for comissao in comissoes_antes:
            print(
                f"   - Comissão ID {comissao[0]}: status='{comissao[1]}', data_estorno={comissao[2]}"
            )
        print("=" * 80)
        ...
        def execute_tenant_safe_mock(db, query, params=None, *args, **kwargs):
            ...
            if call_count["count"] == 1:
                ...
                raise Exception(
                    "ERRO SIMULADO: Falha ao atualizar status das comissões"
                )
        ...
        with patch(
            "app.comissoes_estorno.execute_tenant_safe",
            side_effect=execute_tenant_safe_mock,
        ):
            print("\n" + "=" * 80)
        ...
        result_depois = db_session.execute(
            text("""
            SELECT id, status, data_estorno, motivo_estorno, estornado_por
            FROM comissoes_itens
            WHERE venda_id = :venda_id
        ...
        for i, comissao_depois in enumerate(comissoes_depois):
            ...
            print(
                f"✅ Comissão ID {comissao_depois[0]}: status='{comissao_depois[1]}' (NÃO foi estornada)"
            )
