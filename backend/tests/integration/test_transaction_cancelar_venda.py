...
def tenant_e_usuario(db_session):
    ...
    tenant = (
        db_session.query(Tenant).filter_by(tenant_id="test_tenant_cancelar").first()
    )
    if not tenant:
        tenant = Tenant(
            tenant_id="test_tenant_cancelar", nome="Tenant Teste Cancelar", ativo=True
        )
    ...
    user = (
        ...
        .filter_by(email="test_cancelar@test.com", tenant_id="test_tenant_cancelar")
        .first()
    )
...
class TestTransactionRollbackCancelarVenda:
    ...
    def test_rollback_total_quando_excecao_no_meio_do_cancelamento(
        self, db_session, cenario_venda_ativa
    ):
        """
        ...
        def estornar_estoque_mock(*args, **kwargs):
            ...
            if call_count["count"] == 1:
                ...
                raise Exception(
                    "ERRO SIMULADO: Falha ao estornar estoque do segundo item durante cancelamento"
                )
        ...
        assert venda_depois.status == "finalizada", (
            "❌ FALHA: Status deveria continuar 'finalizada'"
        )
        ...
        assert itens_depois == itens_antes, (
            f"❌ FALHA: Itens foram alterados ({itens_antes} --> {itens_depois})"
        )
        ...
        assert mov_caixa_depois is not None, (
            "❌ FALHA: Movimentação de caixa foi removida"
        )
        ...
        assert mov_bancaria_depois is not None, (
            "❌ FALHA: Movimentação bancária foi removida"
        )
        print(
            f"✅ Movimentação bancária NÃO foi removida (ID: {mov_bancaria_depois.id})"
        )
