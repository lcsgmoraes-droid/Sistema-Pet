...
def test_venda_criada_idempotente(mock_db_session):
    ...
    print(
        f"✅ VendaCriada é idempotente: quantidade_aberta={estado_1[0] if estado_1 else None}"
    )
...
def test_venda_finalizada_idempotente(mock_db_session):
    ...
    print(
        f"✅ VendaFinalizada idempotente: finalizada={estado_1[0]}, total={estado_1[1]}"
    )
...
def test_venda_cancelada_idempotente(mock_db_session):
    ...
    print(
        f"✅ VendaCancelada idempotente: cancelada={estado_1[0]}, total_cancelado={estado_1[1]}"
    )
...
def test_side_effects_suprimidos_em_replay():
    ...
    enable_replay_mode()
    handler.send_notification()
    assert not handler.side_effect_executed, (
        "Side effect NÃO deveria executar em modo replay"
    )
...
def test_upsert_performance(mock_db_session):
    ...
    print(
        f"✅ UPSERT performance: 100 eventos em {elapsed:.2f}s ({elapsed / 100 * 1000:.2f}ms/evento)"
    )
