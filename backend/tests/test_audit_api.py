...
def sample_replay_logs(db: Session):
    ...
    log1 = AuditLog(
        ...
        details=json.dumps(
            {"filters": {"event_type": "VendaCriada", "batch_size": 1000}}
        ),
...
def sample_rebuild_logs(db: Session):
    ...
    log1 = AuditLog(
        ...
        details=json.dumps({"phase": "creating_temp_schema"}),
        ...
        details=json.dumps(
            {
                ...
                "tables_updated": [
                    "vendas_resumo_diario",
                    "performance_parceiro",
                    "receita_mensal",
                ],
...
def test_get_audit_summary_filtered_by_date(
    db: Session, sample_replay_logs, sample_rebuild_logs
):
...
def test_list_replays_non_admin_forbidden(
    client, db: Session, normal_user_token, sample_replay_logs
):
...
def test_list_replays_admin_success(
    client, db: Session, admin_user_token, sample_replay_logs
):
...
def test_list_rebuilds_admin_success(
    client, db: Session, admin_user_token, sample_rebuild_logs
):
...
def test_get_replay_by_id_admin_success(
    client, db: Session, admin_user_token, sample_replay_logs
):
...
def test_get_summary_admin_success(
    client, db: Session, admin_user_token, sample_replay_logs, sample_rebuild_logs
):
...
def test_filter_by_date_range(
    ...
    response = client.get(
        f"/audit/replays?start_date={start_date}&end_date={end_date}", headers=headers
    )
