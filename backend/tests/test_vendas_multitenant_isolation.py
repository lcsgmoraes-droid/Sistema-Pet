...
def test_venda_tem_tenant_id_obrigatorio(db_session):
    ...
    if not user_a:
        pytest.skip(
            "Nenhum usuário encontrado para o tenant A. Execute o sistema primeiro para criar dados de teste."
        )

    set_current_tenant(tenant_a)

    # Buscar ou criar produto
    produto_a = db_session.query(Produto).filter(Produto.tenant_id == tenant_a).first()

    if not produto_a:
        pytest.skip(
            "Nenhum produto encontrado para o tenant A. Execute o sistema primeiro para criar dados de teste."
        )
    ...
    venda_dict = VendaService.criar_venda(
        payload=payload, user_id=user_a.id, db=db_session
    )
    ...
    assert venda.tenant_id == tenant_a, (
        f"❌ FALHA: Venda sem tenant_id correto! Esperado={tenant_a}, Obtido={venda.tenant_id}"
    )
...
def test_venda_item_tem_tenant_id_obrigatorio(db_session):
    ...
    assert item.tenant_id is not None, (
        "❌ FALHA CRÍTICA: VendaItem SEM tenant_id! RISCO DE VAZAMENTO!"
    )
    assert item.tenant_id == tenant_a, (
        f"❌ FALHA: VendaItem com tenant_id errado! Esperado={tenant_a}, Obtido={item.tenant_id}"
    )
...
def test_isolamento_simples_tenant(db_session):
    ...
    print(
        f"✅ TESTE 3 PASSOU: Tenant A tem {vendas_a} vendas, Tenant B tem {vendas_b} vendas. Total: {todas_vendas}"
    )
...
def test_venda_item_herda_tenant_da_venda(db_session):
    ...
    for venda in vendas_com_itens:
        for item in venda.itens:
            ...
            if item.tenant_id is None:
                ...
                erros.append(
                    f"Venda {venda.id}, Item {item.id}: tenant_id diferente da venda"
                )
    ...
    print(
        f"✅ TESTE 4 PASSOU: {len(vendas_com_itens)} vendas verificadas, todos os itens têm tenant_id correto!"
    )
...
def test_venda_pagamento_herda_tenant_da_venda(db_session):
    ...
    for venda in vendas_com_pagamentos:
        for pagamento in venda.pagamentos:
            ...
            if pagamento.tenant_id is None:
                erros.append(
                    f"Venda {venda.id}, Pagamento {pagamento.id}: SEM tenant_id"
                )
            elif pagamento.tenant_id != venda.tenant_id:
                erros.append(
                    f"Venda {venda.id}, Pagamento {pagamento.id}: tenant_id diferente da venda"
                )
    ...
    print(
        f"✅ TESTE 5 PASSOU: {len(vendas_com_pagamentos)} vendas verificadas, todos os pagamentos têm tenant_id correto!"
    )
