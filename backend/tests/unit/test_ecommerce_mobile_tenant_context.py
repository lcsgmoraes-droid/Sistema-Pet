...
def test_get_current_ecommerce_user_sets_tenant_context_from_token():
    ...
    current_user = _get_current_ecommerce_user(credentials=credentials, db=_Db(user))

    assert current_user is user
    assert get_current_tenant() == tenant_id


def test_get_entregador_cliente_sets_tenant_context_from_token():
    tenant_id = uuid4()
    user = SimpleNamespace(id=123, tenant_id=tenant_id, is_active=True)
    ...
    entregador = _get_entregador_cliente(credentials=credentials, db=_Db(user, cliente))
