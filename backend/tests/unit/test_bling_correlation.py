...
def test_bling_flow_payload_adds_active_correlation_without_overwriting():
    ...
    payload = _payload_with_correlation(
        {"pedido_bling_id": "BL-1", "correlation_id": "external"}
    )
...
def test_pedido_status_reconciliation_rate_limit_result_has_job_correlation(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        service, "_contar_pedidos_recentes_reconciliaveis", lambda *args, **kwargs: 12
    )

    result = service.reconciliar_status_pedidos_recentes(
        object(), "tenant-1", dias=3, limite_pedidos=5
    )
