...
PEDIDOS_COMPRA_ENVIO_ROUTES = (
    BACKEND_ROOT / "app" / "pedidos_compra" / "envio_routes.py"
)
PEDIDOS_COMPRA_EXPORTACAO_ROUTES = (
    BACKEND_ROOT / "app" / "pedidos_compra" / "exportacao_routes.py"
)
...
def test_pedido_email_route_has_no_returned_dead_success_block():
    source = PEDIDOS_COMPRA_ENVIO_ROUTES.read_text(encoding="utf-8")
...
def test_pedido_excel_export_has_no_dead_legacy_workbook_block_after_streaming_return():
    source = PEDIDOS_COMPRA_EXPORTACAO_ROUTES.read_text(encoding="utf-8")
