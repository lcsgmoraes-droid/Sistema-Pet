...
def test_mapa_reservas_ativas_por_produto_desloca_kit_virtual_para_componentes():
    ...
    with (
        patch.object(
            EstoqueReservaService, "_itens_reservados_ativos", return_value=[item]
        ),
        patch.object(
        ...
        reservas = EstoqueReservaService.mapa_reservas_ativas_por_produto(
            db=Mock(), tenant_id="tenant-1"
        )
...
def test_reservar_kit_virtual_valida_estoque_no_componente():
    ...
    produto_componente = SimpleNamespace(
        id=20, codigo="COMP-1", nome="Componente", estoque_atual=5
    )
    ...
    with (
        ...
        patch.object(
            ...
            "_componentes_por_kit",
            return_value={
                10: [SimpleNamespace(produto_componente_id=20, quantidade=2)]
            },
        ),
    ):
