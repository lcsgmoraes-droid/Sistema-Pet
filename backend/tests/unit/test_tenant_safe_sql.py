...
class TestTenantSafeSuccess:
    ...
    def test_select_with_tenant_filter(
        self, db_session, setup_tenant_context, create_test_table, test_table_name
    ):
        """
    ...
    def test_scalar_aggregation(
        ...
        """
        total = execute_tenant_safe_scalar(
            db_session,
            f"""
            SELECT SUM(valor)
    ...
    def test_count_aggregation(
        ...
        """
        count = execute_tenant_safe_scalar(
            db_session,
            f"""
            SELECT COUNT(*)
            FROM {test_table_name}
    ...
    def test_first_or_none(
        ...
        row_none = execute_tenant_safe_first(
            db_session,
            f"""
            SELECT * FROM {test_table_name}
            WHERE {{tenant_filter}} AND status = :status
    ...
    def test_update_with_tenant_filter(
        ...
        count_pago_tenant_atual = db_session.execute(
            text(f"""
        ...
        count_pendente_outro = db_session.execute(
            text(f"""
            SELECT COUNT(*) FROM {test_table_name}
    ...
    def test_delete_with_tenant_filter(
        ...
        count_tenant_atual = db_session.execute(
            text(f"""
            SELECT COUNT(*) FROM {test_table_name}
        ...
        # Verificar que outro tenant não foi afetado
        count_outro_tenant = db_session.execute(
            text(f"""
            SELECT COUNT(*) FROM {test_table_name}
            WHERE tenant_id = :tenant_id
        """),
    ...
    def test_system_query_without_tenant(self, db_session):
        ...
        # Health check
        result = execute_tenant_safe(
            db_session, "SELECT 1 as health_check", require_tenant=False
        )
...
class TestTenantSafeErrors:
    ...
    def test_error_unsafe_concatenation_fstring(self, db_session, setup_tenant_context):
        ...
        unsafe_sql = (
            f"SELECT * FROM comissoes WHERE {{tenant_filter}} AND status = '{status}'"
        )
    ...
    def test_error_unsafe_concatenation_plus(self, db_session, setup_tenant_context):
        ...
        """
        unsafe_sql = (
            "SELECT * FROM comissoes WHERE {tenant_filter} AND status = '"
            + "pendente' + "
        )
...
class TestTenantIsolation:
    ...
    def test_isolation_select(
        ...
        rows_tenant1 = execute_tenant_safe_all(
            db_session,
            f"""
            SELECT * FROM {test_table_name}
            WHERE {{tenant_filter}}
        ...
        rows_tenant2 = execute_tenant_safe_all(
            db_session,
            f"""
            SELECT * FROM {test_table_name}
            WHERE {{tenant_filter}}
    ...
    def test_isolation_update(
        ...
        count_cancelado_t1 = db_session.execute(
            text(f"""
            SELECT COUNT(*) FROM {test_table_name}
        ...
        count_cancelado_t2 = db_session.execute(
            text(f"""
            SELECT COUNT(*) FROM {test_table_name}
            WHERE tenant_id = :tenant_id AND status = 'cancelado'
        """),
...
class TestEdgeCases:
    ...
    def test_require_tenant_false_with_placeholder(
        self, db_session, create_test_table, test_table_name
    ):
        """
...
class TestBehavior:
    ...
    def test_transaction_rollback_preservation(
        ...
        count_before = execute_tenant_safe_scalar(
            db_session,
            f"""
            SELECT COUNT(*) FROM {test_table_name}
            WHERE {{tenant_filter}}
        ...
        count_after = execute_tenant_safe_scalar(
            db_session,
            f"""
            SELECT COUNT(*) FROM {test_table_name}
            WHERE {{tenant_filter}}
    ...
    def test_multiple_queries_same_context(
        ...
        # Query 3
        avg = execute_tenant_safe_scalar(
            db_session,
            f"""
