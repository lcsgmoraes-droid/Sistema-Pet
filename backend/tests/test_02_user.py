...
def test_create_user_with_tenant(db_session, tenant_factory, user_factory):
    ...
    assert user.tenant_id == _tenant_uuid(tenant)
...
def test_user_password_is_hashed(db_session, tenant_factory, user_factory):
    ...
    # Hash deve validar pelo mesmo mecanismo usado pelo app.
    assert verify_password(password_plain, user.hashed_password)
...
def test_multiple_users_same_tenant(db_session, tenant_factory, user_factory):
    ...
    assert len(users) >= 3
    assert all(u.tenant_id == _tenant_uuid(tenant) for u in users)
