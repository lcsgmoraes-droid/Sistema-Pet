...
from app.read_models.rebuild import rebuild_read_models_zero_downtime, RebuildResult
...
class TestSchemaValidation:
    ...
    def test_validate_schema_detects_future_dates(self, db_session):
        ...
        assert any(
            "data" in error.lower() and "futura" in error.lower()
...
class TestSchemaSwap:
    ...
    def test_swap_schemas_success(self, mock_log_success, mock_validate, db_session):
        """✅ Teste: Swap atômico bem-sucedido"""
        # Arrange
        mock_validate.return_value = SchemaValidation(
            is_valid=True, table_counts={"table1": 100}, errors=[], warnings=[]
        )
    ...
    def test_swap_schemas_fails_validation(
        self, mock_log_failure, mock_validate, db_session
    ):
        """✅ Teste: Swap abortado se validação falhar"""
        # Arrange
        mock_validate.return_value = SchemaValidation(
            is_valid=False, table_counts={}, errors=["Schema inválido"], warnings=[]
        )
    ...
    @patch("app.read_models.schema_swap.validate_schema")
    def test_swap_schemas_rollback_on_error(self, mock_validate, db_session):
...
class TestRebuildComplete:
    ...
    def test_rebuild_zero_downtime_success(
        ...
        mock_replay.return_value = ReplayStats(
            total_events=1000, batches_processed=1, duration_seconds=10.0, success=True
        )

        mock_validate.return_value = SchemaValidation(
            is_valid=True, table_counts={"table1": 1000}, errors=[], warnings=[]
        )

        mock_swap.return_value = SwapResult(
            success=True, duration_seconds=0.5, tables_swapped=["table1", "table2"]
        )
    ...
    def test_rebuild_fails_on_replay_error(
        self, mock_create_temp, mock_replay, mock_log, mock_drop, db_session
    ):
