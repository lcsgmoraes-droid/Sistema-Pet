...
class TestConfidenceCalculator:
    ...
    def test_calculate_weighted(self):
        ...
        inputs = [
            ...
            ConfidenceInput(
                ConfidenceSource.HISTORICAL_ACCURACY, 90.0, 0.1, "Histórico"
            ),
        ]
    ...
    def test_create_from_simple_scores(self):
        ...
        inputs = ConfidenceCalculator.create_from_simple_scores(
            rule_score=100.0, pattern_score=80.0, history_score=90.0
        )
...
class TestIntegration:
    ...
    def test_full_flow_high_confidence(self):
        ...
        inputs = calculator.create_from_simple_scores(
            rule_score=95.0, pattern_score=88.0, history_score=92.0
        )
        confidence_score = calculator.calculate(inputs)

        # 2. Aplicar política
        policy = DecisionPolicy()
        policy_result = policy.evaluate(int(confidence_score), "categorizar_lancamento")
    ...
    def test_full_flow_low_confidence(self):
        ...
        calculator = ConfidenceCalculator()
        inputs = calculator.create_from_simple_scores(
            rule_score=45.0, pattern_score=40.0, history_score=50.0
        )
        confidence_score = calculator.calculate(inputs)
