...
def mock_caixa_service(monkeypatch):
    ...
    # Caixa aberto
    mock.validar_caixa_aberto.return_value = {"caixa_id": 1, "saldo_atual": 500.00}
    ...
    mock.eh_forma_dinheiro.side_effect = lambda forma: (
        forma.lower() in ["dinheiro", "money"]
    )
...
def fake_pagamentos():
    ...
    """
    return [{"forma_pagamento": "Dinheiro", "valor": 100.00, "numero_parcelas": 1}]
...
def assert_evento_publicado():
    ...
    def _assert(mock_dispatcher, event_type, **kwargs):
        ...
        """
        eventos = [
            e for e in mock_dispatcher.eventos_publicados if isinstance(e, event_type)
        ]
