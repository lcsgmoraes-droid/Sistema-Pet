...
MIGRATION_FILE = migration_path("su20260613a1_rls_cash_flow_ai.py")
...
def test_cash_flow_ai_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="su20260613a1",
        down_revision="st20260613a1",
        table_constant="CASH_FLOW_AI_RLS_TABLES",
        table_names=CASH_FLOW_AI_RLS_TABLES,
    )


def test_cash_flow_ai_rls_upgrade_targets_declared_tables(monkeypatch):
    assert_upgrade_emits_rls_for_declared_tables(
...
def test_cash_flow_ai_rls_upgrade_skips_missing_tables(monkeypatch):
    assert _capture(monkeypatch, "upgrade", existing=()) == []


def test_cash_flow_ai_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_cash_flow_ai_rls_skips_when_not_applicable(monkeypatch):
