...
MIGRATION_FILE = migration_path("sg20260612a1_rls_bulk_conversions.py")
...
def _capture(monkeypatch, action_name: str, *, dialect="postgresql", existing=None):
    return capture_migration_sql(
        ...
        MIGRATION_FILE,
        action_name,
        BULK_CONVERSIONS_RLS_TABLES,
        dialect=dialect,
        existing=existing,
    )


def test_bulk_conversions_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="sg20260612a1",
        down_revision="sf20260612a1",
        table_constant="BULK_CONVERSIONS_RLS_TABLES",
        table_names=BULK_CONVERSIONS_RLS_TABLES,
    )


def test_bulk_conversions_rls_upgrade_targets_declared_tables(monkeypatch):
    assert_upgrade_emits_rls_for_declared_tables(
...
def test_bulk_conversions_rls_upgrade_skips_missing_tables(monkeypatch):
    assert _capture(monkeypatch, "upgrade", existing=()) == []


def test_bulk_conversions_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_bulk_conversions_rls_skips_when_not_applicable(monkeypatch):
...
def test_bulk_conversions_are_tracked_by_tenant_safe_sql_guardrail():
    assert "granel_conversoes" in TENANT_SCOPED_TABLES
