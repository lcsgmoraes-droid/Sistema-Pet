...
MIGRATION_FILE = migration_path("sn20260613a1_rls_purchase_order_tables.py")
...
def _capture(monkeypatch, action_name: str, *, dialect="postgresql", existing=None):
    return capture_migration_sql(
        ...
        MIGRATION_FILE,
        action_name,
        PURCHASE_ORDER_RLS_TABLES,
        dialect=dialect,
        existing=existing,
    )


def test_purchase_order_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="sn20260613a1",
        down_revision="sm20260613a1",
        table_constant="PURCHASE_ORDER_RLS_TABLES",
        table_names=PURCHASE_ORDER_RLS_TABLES,
    )


def test_purchase_order_rls_upgrade_targets_declared_tables(monkeypatch):
    assert_upgrade_emits_rls_for_declared_tables(
...
def test_purchase_order_rls_upgrade_skips_missing_tables(monkeypatch):
    assert _capture(monkeypatch, "upgrade", existing=()) == []


def test_purchase_order_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_purchase_order_rls_skips_when_not_applicable(monkeypatch):
