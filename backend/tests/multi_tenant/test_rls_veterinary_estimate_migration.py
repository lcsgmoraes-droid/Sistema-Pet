...
MIGRATION_FILE = migration_path("rp20260612a1_rls_veterinary_estimate_tables.py")
...
def test_veterinary_estimate_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="rp20260612a1",
        down_revision="ro20260612a1",
        table_constant="VETERINARY_ESTIMATE_RLS_TABLES",
        table_names=VETERINARY_ESTIMATE_RLS_TABLES,
    )


def test_veterinary_estimate_rls_upgrade_targets_declared_tables(monkeypatch):
    emitted = _capture(monkeypatch, "upgrade")

    assert_upgrade_emits_rls_for_declared_tables(emitted, VETERINARY_ESTIMATE_RLS_TABLES)


def test_veterinary_estimate_rls_upgrade_skips_missing_tables(monkeypatch):
    assert _capture(monkeypatch, "upgrade", existing=()) == []


def test_veterinary_estimate_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_veterinary_estimate_rls_skips_when_not_applicable(monkeypatch):
