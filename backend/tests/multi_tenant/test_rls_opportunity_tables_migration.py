...
MIGRATION_FILE = migration_path("rj20260612a1_rls_opportunity_tables.py")
...
def _capture(monkeypatch, action_name: str, *, dialect="postgresql", existing=None):
    return capture_migration_sql(
        ...
        MIGRATION_FILE,
        action_name,
        OPPORTUNITY_RLS_TABLES,
        dialect=dialect,
        existing=existing,
    )


def test_opportunity_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="rj20260612a1",
        down_revision="ri20260612a1",
        table_constant="OPPORTUNITY_RLS_TABLES",
        table_names=OPPORTUNITY_RLS_TABLES,
    )


def test_opportunity_rls_upgrade_targets_declared_tables(monkeypatch):
    emitted = _capture(monkeypatch, "upgrade")

    assert_upgrade_emits_rls_for_declared_tables(emitted, OPPORTUNITY_RLS_TABLES)


def test_opportunity_rls_upgrade_skips_missing_tables(monkeypatch):
    assert _capture(monkeypatch, "upgrade", existing=()) == []


def test_opportunity_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_opportunity_rls_skips_when_not_applicable(monkeypatch):
