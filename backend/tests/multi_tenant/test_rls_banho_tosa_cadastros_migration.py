...
MIGRATION_FILE = migration_path("rt20260612a1_rls_banho_tosa_cadastros.py")
...
def test_banho_tosa_cadastros_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="rt20260612a1",
        down_revision="rs20260612a1",
        table_constant="BANHO_TOSA_CADASTRO_RLS_TABLES",
        table_names=BANHO_TOSA_CADASTRO_RLS_TABLES,
    )


def test_banho_tosa_cadastros_rls_upgrade_targets_declared_tables(monkeypatch):
    assert_upgrade_emits_rls_for_declared_tables(
...
def test_banho_tosa_cadastros_rls_upgrade_skips_missing_tables(monkeypatch):
    assert _capture(monkeypatch, "upgrade", existing=()) == []


def test_banho_tosa_cadastros_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_banho_tosa_cadastros_rls_skips_when_not_applicable(monkeypatch):
