...
MIGRATION_FILE = migration_path("te20260614a1_rls_sale_settlements.py")
...
def _capture(monkeypatch, action_name: str, *, dialect="postgresql", existing=None):
    return capture_migration_sql(
        ...
        MIGRATION_FILE,
        action_name,
        SALE_SETTLEMENT_RLS_TABLES,
        dialect=dialect,
        existing=existing,
    )


def test_sale_settlement_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="te20260614a1",
        down_revision="td20260614a1",
        table_constant="SALE_SETTLEMENT_RLS_TABLES",
        table_names=SALE_SETTLEMENT_RLS_TABLES,
    )


def test_sale_settlement_rls_upgrade_targets_declared_tables(monkeypatch):
    assert_upgrade_emits_rls_for_declared_tables(
...
def test_sale_settlement_rls_upgrade_skips_missing_tables(monkeypatch):
    assert _capture(monkeypatch, "upgrade", existing=()) == []


def test_sale_settlement_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_sale_settlement_rls_skips_when_not_applicable(monkeypatch):
...
def test_sale_settlement_table_is_tracked_by_raw_sql_guardrails():
    assert "venda_baixas" in TENANT_SCOPED_TABLES
    assert "venda_baixas" in TENANT_TABLES
