...
MIGRATION_FILE = migration_path("rl20260612a1_rls_veterinary_schedule_tables.py")
...
def test_veterinary_schedule_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="rl20260612a1",
        down_revision="rk20260612a1",
        table_constant="VETERINARY_SCHEDULE_RLS_TABLES",
...
def test_veterinary_schedule_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_veterinary_schedule_rls_skips_when_not_applicable(monkeypatch):
