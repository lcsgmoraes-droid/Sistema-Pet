...
MIGRATION_FILE = migration_path("rm20260612a1_rls_veterinary_clinical_record_tables.py")
...
def test_veterinary_clinical_record_rls_migration_metadata_and_scope():
    assert_rls_migration_metadata(
        MIGRATION_FILE,
        revision="rm20260612a1",
        down_revision="rl20260612a1",
        table_constant="VETERINARY_CLINICAL_RECORD_RLS_TABLES",
        table_names=VETERINARY_CLINICAL_RECORD_RLS_TABLES,
    )


def test_veterinary_clinical_record_rls_upgrade_targets_declared_tables(monkeypatch):
...
def test_veterinary_clinical_record_rls_upgrade_skips_missing_tables(monkeypatch):
    assert _capture(monkeypatch, "upgrade", existing=()) == []


def test_veterinary_clinical_record_rls_downgrade_unwinds_in_reverse_order(monkeypatch):
    assert_downgrade_unwinds_in_reverse_order(
...
def test_veterinary_clinical_record_rls_skips_when_not_applicable(monkeypatch):
