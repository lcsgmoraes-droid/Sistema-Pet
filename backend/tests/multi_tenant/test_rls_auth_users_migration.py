...
def test_auth_users_rls_upgrade_allows_pre_tenant_auth_and_tenant_membership(
    monkeypatch,
):
    ...
    users_select = statements_containing(
        emitted, "CREATE POLICY users_auth_select ON users"
    )
    ...
    assert (
        ...
        in user_tenants_select[0]
    )
    assert (
        "user_id = NULLIF(current_setting('app.auth_user_id', true), '')::integer"
        in user_tenants_select[0]
    )
...
def test_auth_users_rls_writes_stay_tenant_scoped_except_user_self_updates(monkeypatch):
    ...
    users_insert = statements_containing(
        emitted, "CREATE POLICY users_auth_insert ON users"
    )
    ...
    users_update = statements_containing(
        emitted, "CREATE POLICY users_auth_update ON users"
    )
    ...
    user_tenants_writes = [
        ...
        and any(
            fragment in sql for fragment in ("FOR INSERT", "FOR UPDATE", "FOR DELETE")
        )
    ]
    assert len(user_tenants_writes) == 3
    assert all(
        "current_setting('app.auth_email', true)" not in sql
        for sql in user_tenants_writes
    )
    assert all(
        "tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid" in sql
        for sql in user_tenants_writes
    )
