...
try:
    database_url = database_url_from_env("PROD_DATABASE_URL", "DATABASE_URL")
    conn = connect_database("PROD_DATABASE_URL", "DATABASE_URL")
    print(f"✅ Conectado em {masked_database_url(database_url)}!")
...
try:
    ...
    print("🔑 Senha: definida fora do script")
    print(f"👤 Nome: {nome}")
