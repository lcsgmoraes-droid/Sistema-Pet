...
class ImportadorProducao:
    ...
    def __init__(self, database_url: str, dry_run: bool = False):
        ...
        # Log file
        self.log_file = (
            LOG_DIR / f"importacao_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )

        self.log(f"{'=' * 80}")
        self.log(
            f"IMPORTADOR SIMPLESVET - MODO {'DRY-RUN (TESTE)' if dry_run else 'PRODUÇÃO'}"
        )
        self.log(f"{'=' * 80}")
    ...
    def get_tenant_id(self, db) -> str:
        ...
        result = db.execute(
            text("SELECT tenant_id FROM users WHERE id = :uid LIMIT 1"),
            {"uid": USER_ID},
        )
        row = result.fetchone()
        if row:
    ...
    def truncar_campo(self, valor: Any, max_length: int, nome_campo: str = "") -> str:
        ...
        if len(valor_str) > max_length:
            ...
            self.log(
                f"Campo '{nome_campo}' truncado de {len(valor_str)} para {max_length} chars",
                "WARN",
            )
            return truncado
    ...
    def validar_cliente(self, row: Dict) -> ResultadoValidacao:
        ...
        dados = {
            ...
            "cpf": cpf,
            "telefone": self.truncar_campo(
                row.get("telefone"), limites["telefone"], "telefone"
            ),
            "celular": self.truncar_campo(
                row.get("celular"), limites["celular"], "celular"
            ),
            "email": self.truncar_campo(
                row.get("pes_var_email"), limites["email"], "email"
            ),
            "cep": self.truncar_campo(row.get("end_var_cep"), limites["cep"], "cep"),
            "endereco": self.truncar_campo(
                row.get("end_var_endereco"), limites["endereco"], "endereco"
            ),
            "numero": self.truncar_campo(
                row.get("end_var_numero"), limites["numero"], "numero"
            ),
            "complemento": self.truncar_campo(
                row.get("end_var_complemento"), limites["complemento"], "complemento"
            ),
            "bairro": self.truncar_campo(
                row.get("end_var_bairro"), limites["bairro"], "bairro"
            ),
            "cidade": self.truncar_campo(
                row.get("end_var_municipio"), limites["cidade"], "cidade"
            ),
            "estado": estado,
            "tipo_cadastro": "cliente",
            "tipo_pessoa": "PF",  # Corrigido: PF (Pessoa Física) ou PJ (Pessoa Jurídica) - VARCHAR(2)
            "observacoes": row.get("pes_txt_observacao")
            if row.get("pes_txt_observacao") != "NULL"
            else None,
            "ativo": True,
    ...
    def carregar_contatos(self):
        ...
        for row in registros:
            ...
            if "cel" in tipo:
                self.contatos[pes_id]["celular"] = (
                    self.contatos[pes_id]["celular"] or contato
                )
            elif "tel" in tipo or "fone" in tipo:
                self.contatos[pes_id]["telefone"] = (
                    self.contatos[pes_id]["telefone"] or contato
                )
            ...
            for idx, row in enumerate(registros, 1):
                try:
                    ...
                    if not resultado.valido:
                        ...
                        self.log(
                            f"[{idx}/{self.stats['clientes'].total}] INVÁLIDO: {row.get('pes_var_nome')} - {resultado.erros}",
                            "ERROR",
                        )
                        continue
                    ...
                    existe = db.execute(
                        text("SELECT id FROM clientes WHERE codigo = :cod"),
                        {"cod": codigo},
                    ).fetchone()
                    ...
                    if existe:
                        ...
                        self.log(
                            f"[{idx}/{self.stats['clientes'].total}] DUPLICADO: {resultado.dados_limpos['nome']} (#{codigo})",
                            "WARN",
                        )
                        continue
                    ...
                    if self.dry_run:
                        self.log(
                            f"[{idx}/{self.stats['clientes'].total}] DRY-RUN: Importaria {resultado.dados_limpos['nome']}",
                            "INFO",
                        )
                        ...
                        self.log(
                            f"[{idx}/{self.stats['clientes'].total}] IMPORTADO: {resultado.dados_limpos['nome']} (#{codigo})",
                            "OK",
                        )
                        ...
                        if inserts_neste_batch >= commit_batch:
                            ...
                            inserts_neste_batch = 0
                            self.log(
                                f"Commit parcial realizado ({idx} registros processados)",
                                "INFO",
                            )
                    ...
                    self.log(
                        f"[{idx}] ERRO: {row.get('pes_var_nome')} - {erro_msg[:150]}",
                        "ERROR",
                    )
    ...
    def gerar_relatorio(self):
        ...
        for entidade, stats in self.stats.items():
            if stats.total > 0:
                ...
                self.log(
                    f"  Válidos:          {stats.validos} ({stats.validos / stats.total * 100:.1f}%)"
                )
        ...
        relatorio_file = (
            LOG_DIR / f"relatorio_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        relatorio_data = {
            ...
            "estatisticas": {
                k: asdict(v) for k, v in self.stats.items() if v.total > 0
            },
        }
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Modo teste (não insere dados)"
    )
    parser.add_argument("--limite", type=int, help="Limitar número de registros")
    parser.add_argument(
        "--fase",
        default="clientes",
        choices=["clientes", "produtos", "pets", "vendas", "all"],
    )
    parser.add_argument(
        "--database", default=DATABASE_URL, help="URL do banco de dados"
    )

    args = parser.parse_args()

    # Criar importador
    importador = ImportadorProducao(database_url=args.database, dry_run=args.dry_run)
