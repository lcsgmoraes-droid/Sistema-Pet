...
from app.financeiro_models import ContaPagar, LancamentoManual, CategoriaFinanceira
...
def montar_demonstrativo_calculo_comissao(
    *,
    ...
    desconto = _round_money(desconto)
    beneficio = _round_money(beneficio)
    taxa_cartao = _round_money(taxa_cartao)
    imposto = _round_money(imposto)
    taxa_entregador = _round_money(taxa_entregador)
    custo_operacional = _round_money(custo_operacional)
    receita_taxa_entrega = _round_money(receita_taxa_entrega)
    custo_produto = _round_money(custo_produto)
    valor_bruto = _round_money(valor_bruto)
    valor_base_calculo = _round_money(valor_base_calculo)
    percentual_comissao = _round_money(percentual_comissao)
    valor_comissao = _round_money(valor_comissao)
    valor_comissao_gerada = _round_money(valor_comissao_gerada)
    percentual_aplicado = _round_money(percentual_aplicado)

    inclui_custo_produto = tipo_normalizado == "lucro"
    lucro_conferido = _round_money(
        valor_bruto
    ...
    linhas = [
        ...
        {
            "chave": "base_calculo",
            "label": "Lucro comissionavel"
            if inclui_custo_produto
            else "Base comissionavel",
            "operador": "=",
            "tipo": "resultado",
...
def _campanha_item_do_snapshot(snapshot: Any, produto_id: Optional[int]) -> float:
    ...
    for item in snapshot.get("itens") or []:
        try:
            if int(item.get("produto_id") or 0) == int(produto_id):
                return _round_money(item.get("campanha", 0))
...
async def detalhe_comissao(
    ...
    try:
        ...
        valor_bruto_item = (
            _round_money(preco_unitario_item * quantidade_item)
            if preco_unitario_item is not None and quantidade_item
            else _round_money(valor_venda)
        )
        desconto_total_item = max(_round_money(valor_bruto_item - valor_venda), 0.0)
        beneficio_item = _campanha_item_do_snapshot(
        ...
        detalhe = {
            ...
            "valores_financeiros": {
                ...
                "valor_base_comissionada": decimal_to_float(
                    r["valor_base_comissionada"]
                ),
            },
            # EXPLICAÇÃO DO CÁLCULO DA BASE
            "origem_base_calculo": {
                "valor_inicial": valor_bruto_item,
                "deducoes_aplicadas": _round_money(deducoes_aplicadas),
            ...
            "deducoes": {
                ...
                "desconto": desconto_manual_item,
                "beneficio": beneficio_item,
                "taxa_cartao": decimal_to_float(r.get("taxa_cartao_item"))
                if r.get("taxa_cartao_item")
                else 0.0,
                "imposto": decimal_to_float(r.get("impostos_item"))
                if r.get("impostos_item")
                else 0.0,
                "taxa_entregador": decimal_to_float(r.get("taxa_entregador_item"))
                if r.get("taxa_entregador_item")
                else 0.0,
                "custo_operacional": decimal_to_float(r.get("custo_operacional_item"))
                if r.get("custo_operacional_item")
                else 0.0,
                "receita_taxa_entrega": decimal_to_float(
                    r.get("receita_taxa_entrega_item")
                )
                if r.get("receita_taxa_entrega_item")
                else 0.0,
                "percentual_impostos": decimal_to_float(r.get("percentual_impostos"))
                if r.get("percentual_impostos")
                else 0.0,
                "forma_pagamento": r.get(
                    "forma_pagamento"
                ),  # ✅ Usar forma_pagamento da comissao
                "numero_parcelas": r.get("numero_parcelas"),
                "taxa_percentual": decimal_to_float(r.get("taxa_percentual"))
...
async def fechar_comissoes(
    ...
    try:
        ...
        try:
            ...
            result = execute_tenant_safe(
                db, stmt, {"ids": tuple(request.comissoes_ids)}
            )
            rows = result.fetchall()
            ...
            logger.info(
                f"Pendentes: {len(ids_pendentes)}, Ignorados: {len(ids_ignorados)}"
            )
            ...
            if ids_pendentes:
                ...
                db.commit()
                logger.info(
                    f"✅ {len(comissoes_fechadas)} comissões fechadas com sucesso"
                )
                ...
                if funcionario_id:
                    funcionario = (
                        db.query(Cliente).filter(Cliente.id == funcionario_id).first()
                    )
                    if funcionario:
