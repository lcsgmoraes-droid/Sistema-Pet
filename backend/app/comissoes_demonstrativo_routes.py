...
def montar_demonstrativo_calculo_comissao(
    *,
    ...
    linhas = [
        ...
        {
            "chave": "base_calculo",
            "label": "Lucro comissionavel" if inclui_custo_produto else "Base comissionavel",
            "operador": "=",
            "tipo": "resultado",
...
async def detalhe_comissao(
    ...
    try:
        ...
        quantidade_item = decimal_to_float(r.get('item_quantidade')) or decimal_to_float(r['quantidade']) or 0.0
        preco_unitario_item = decimal_to_float(r.get('item_preco_unitario')) if r.get('item_preco_unitario') is not None else None
        valor_bruto_item = (
        ...
        beneficio_item = _campanha_item_do_snapshot(r.get('rentabilidade_snapshot'), r.get('produto_id'))
        if beneficio_item <= 0 and r.get('cupom_discount_applied') and r.get('desconto_total_venda'):
            ...
            if desconto_total_venda > 0:
                beneficio_item = _round_money(desconto_total_item * min(cupom_total / desconto_total_venda, 1.0))
        beneficio_item = min(beneficio_item, desconto_total_item)
        desconto_manual_item = max(_round_money(desconto_total_item - beneficio_item), 0.0)
