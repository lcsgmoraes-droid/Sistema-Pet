...
async def listar_lembretes_pendentes(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao listar lembretes: {str(e)}"
        )
...
async def marcar_notificacao_enviada(
    ...
    try:
        lembrete = (
            db.query(Lembrete)
            .filter(Lembrete.id == lembrete_id, Lembrete.tenant_id == tenant_id)
            .first()
        )
...
async def renovar_lembrete(
    ...
    try:
        ...
        historico = (
            json.loads(lembrete_anterior.historico_doses)
            if lembrete_anterior.historico_doses
            else []
        )
        historico.append(
            {
        ...
        if (
            lembrete_anterior.dose_total
            and lembrete_anterior.dose_atual >= lembrete_anterior.dose_total
        ):
...
async def cancelar_lembrete(
    ...
    try:
        ...
        return {"id": lembrete.id, "status": "cancelado"}
...
async def simular_data_lembrete(
    lembrete_id: int,
    dias_no_futuro: int = Query(
        ..., description="Dias para adicionar (negativo = passado)"
    ),
    user_and_tenant=Depends(get_current_user_and_tenant),
    db: Session = Depends(get_session),
):
