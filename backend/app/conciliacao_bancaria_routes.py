...
def ensure_regras_conciliacao_table_exists(db: Session) -> None:
    ...
    try:
        ...
        db.execute(
            text(
                "CREATE INDEX IF NOT EXISTS ix_regras_conciliacao_tenant_id ON regras_conciliacao (tenant_id);"
            )
        )
        db.execute(
            text(
                "CREATE INDEX IF NOT EXISTS ix_regras_conciliacao_ativo ON regras_conciliacao (ativo);"
            )
        )
        db.execute(
            text(
                "CREATE INDEX IF NOT EXISTS ix_regras_conciliacao_confianca ON regras_conciliacao (confianca);"
            )
        )
        db.commit()
...
async def upload_ofx(
    ...
    if not validacao["valido"]:
        raise HTTPException(
            status_code=400, detail=f"OFX inválido: {validacao['erros']}"
        )
    ...
    for transacao in extrato_ofx.transacoes:
        ...
        if melhor_regra:
            ...
            if melhor_confianca >= 80:
                ...
                melhor_regra.confianca = int(
                    (melhor_regra.vezes_confirmada / melhor_regra.vezes_aplicada) * 100
                )
            else:
    ...
    return UploadOFXResponse(
        ...
        periodo_inicio=extrato_ofx.data_inicio.isoformat()
        if extrato_ofx.data_inicio
        else None,
...
async def listar_movimentacoes(
    ...
    for mov in movimentacoes:
        ...
        if mov.fornecedor_id:
            ...
            fornecedor = (
                db.query(Cliente).filter(Cliente.id == mov.fornecedor_id).first()
            )
            if fornecedor:
        ...
        resultado.append(
            MovimentacaoResponse(
                id=mov.id,
                data_movimento=mov.data_movimento.isoformat()
...
async def classificar_movimentacao(
    ...
    if mov.extrato_id:
        extrato = (
            ...
            .filter(ExtratoBancario.id == mov.extrato_id)
            .first()
        )
        if extrato:
            extrato.conciliadas += 1
    ...
    if mov.regra_aplicada_id:
        regra = (
            db.query(RegraConciliacao)
            .filter(RegraConciliacao.id == mov.regra_aplicada_id)
            .first()
        )
        if regra:
...
async def listar_regras(
    ...
    query = db.query(RegraConciliacao).filter(RegraConciliacao.tenant_id == tenant_id)
    ...
    for regra in regras:
        ...
        if regra.fornecedor_id:
            ...
            fornecedor = (
                db.query(Cliente).filter(Cliente.id == regra.fornecedor_id).first()
            )
            if fornecedor:
                fornecedor_nome = fornecedor.nome
...
async def deletar_regra(
    ...
    regra = (
        ...
        .filter(
            and_(
                RegraConciliacao.id == regra_id, RegraConciliacao.tenant_id == tenant_id
            )
        )
        .first()
    )
...
async def listar_templates(
    ...
    templates = (
        ...
        .filter(TemplateAdquirente.tenant_id == tenant_id)
        .all()
    )
...
async def obter_estatisticas(
    ...
    pendentes = query.filter(
        MovimentacaoBancaria.status_conciliacao == "pendente"
    ).count()
    sugeridas = query.filter(
        MovimentacaoBancaria.status_conciliacao == "sugerido"
    ).count()
    conciliadas = query.filter(
        MovimentacaoBancaria.status_conciliacao == "conciliado"
    ).count()
    ...
    return {
        ...
        "percentual_conciliado": round(
            (conciliadas / total * 100) if total > 0 else 0, 1
        ),
