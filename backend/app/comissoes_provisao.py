...
def _normalizar_data_competencia(value) -> date:
    ...
    if isinstance(value, str):
        try:
            ...
            logger.warning(
                "Data de venda invalida para provisao de comissoes: %s", value
            )
            return date.today()
    return value
...
def _buscar_comissionado(db: Session, funcionario_id: int, tenant_id: str):
    result_cliente = execute_tenant_safe(
        db,
        """
    ...
    result_user = execute_tenant_safe(
        db,
        """
        SELECT nome, data_fechamento_comissao
        FROM users
...
def _resolver_usuario_responsavel(
    db: Session, tenant_id: str, user_id: Optional[int] = None
) -> int:
    ...
    result_fallback = execute_tenant_safe(
        db,
        """
        SELECT id
        FROM users
...
def _resolver_subcategoria_comissoes(db: Session, tenant_id: str) -> Optional[int]:
    ...
    result_categoria = execute_tenant_safe(
        db,
        """
        SELECT id
        FROM dre_categorias
    ...
    result_insert = execute_tenant_safe(
        db,
        f"""
...
def provisionar_comissoes_venda(
    ...
    with transactional_session(db):
        ...
        if venda.status not in ["baixa_parcial", "finalizada"]:
            logger.info(
                f"Venda {venda_id} com status '{venda.status}' - Provisão não aplicável"
            )
            return {
                "success": False,
                "comissoes_provisionadas": 0,
        ...
        for comissao in comissoes_pendentes:
            ...
            funcionario_nome = (
                funcionario[0] if funcionario else f"Funcionário #{funcionario_id}"
            )
                ...
                if data_competencia.day <= dia_fechamento:
                    ...
                    if data_competencia.month == 12:
                        data_vencimento = data_competencia.replace(
                            year=data_competencia.year + 1, month=1, day=dia_fechamento
                        )
                    else:
                        data_vencimento = data_competencia.replace(
                            month=data_competencia.month + 1, day=dia_fechamento
                        )
            else:
            ...
            logger.info(
                f"  ✅ DRE atualizada: Despesa de R$ {float(valor_comissao):.2f}"
            )
