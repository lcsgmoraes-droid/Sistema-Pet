...
class VerificarConciliacaoRequest(BaseModel):
    data_referencia: date = Field(
        ..., description="Data a ser conciliada (ex: 2026-02-10)"
    )
...
class IniciarConciliacaoRequest(BaseModel):
    ...
    operadora: str
    arquivos: Optional[List[dict]] = Field(
        default=[], description="Lista de arquivos enviados"
    )
...
def concluir_aba1(
    ...
    todas_concluidas = (
        historico.aba1_concluida
        and historico.aba2_concluida
        and historico.aba3_concluida
    )
...
def concluir_aba3(
    ...
    if request.parcelas_amarradas is not None and request.parcelas_orfas is not None:
        ...
        if total_parcelas > 0:
            historico.taxa_amarracao = Decimal(
                request.parcelas_amarradas / total_parcelas * 100
            )
...
def cancelar_conciliacao(
    ...
    if historico.status == "concluida":
        raise HTTPException(
            status_code=400, detail="Não é possível cancelar conciliação já concluída"
        )
