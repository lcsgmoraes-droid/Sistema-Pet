...
class PendenciaEstoque(BaseTenantModel):
    ...
    valor_referencia = Column(
        Float, nullable=True
    )  # Preço do produto no momento do registro
    ...
    venda_id = Column(
        Integer, ForeignKey("vendas.id"), nullable=True
    )  # Venda que finalizou a pendência
    ...
    cliente = relationship(
        "Cliente", foreign_keys=[cliente_id], backref="pendencias_estoque"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "quantidade_desejada": float(self.quantidade_desejada)
            if self.quantidade_desejada
            else 0,
            "valor_referencia": float(self.valor_referencia)
            ...
            "data_notificacao": self.data_notificacao.isoformat()
            ...
            "usuario_registrou": self.usuario_registrou.nome
            if self.usuario_registrou
            else None,
        }
