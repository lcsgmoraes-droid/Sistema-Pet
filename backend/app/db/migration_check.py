...
def _is_strict_migration_environment() -> bool:
    ...
    if any(
        environment in STRICT_MIGRATION_ENVIRONMENTS for environment in environments
    ):
        return True
    if any(
        environment in TOLERANT_MIGRATION_ENVIRONMENTS for environment in environments
    ):
        return False
    if _env_bool("DEBUG"):
        return False
...
def ensure_db_ready(engine: Engine, alembic_ini_path: Optional[str] = None) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"✅ Database ready (version: {current_version}, head check skipped)"
            )
            return
        ...
        logger.info(
            f"✅ Database ready: migrations up to date (version: {current_version})"
        )
...
def get_migration_status(
    engine: Engine, alembic_ini_path: Optional[str] = None
) -> dict:
    """
    ...
    try:
        ...
        if status["current_version"] == status["expected_head"]:
            ...
            status["message"] = (
                f"Pending: {status['current_version']} -> {status['expected_head']}"
            )
