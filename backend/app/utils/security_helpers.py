...
def get_by_id_user(
    ...
    entity = (
        db.query(model)
        .filter(
            getattr(model, id_field) == entity_id, getattr(model, user_field) == user_id
        )
        .first()
    )
...
def bulk_validate_user_access(
    ...
    entities = (
        db.query(getattr(model, id_field))
        .filter(
...
def safe_get_produto(db: Session, produto_id: int, user_id: int):
    ...
    return get_by_id_user(
        db, Produto, produto_id, user_id, error_message="Produto não encontrado"
    )
...
def safe_get_cliente(db: Session, cliente_id: int, user_id: int):
    ...
    return get_by_id_user(
        db, Cliente, cliente_id, user_id, error_message="Cliente não encontrado"
    )
...
def safe_get_venda(db: Session, venda_id: int, user_id: int):
    ...
    return get_by_id_user(
        db, Venda, venda_id, user_id, error_message="Venda não encontrada"
    )


def safe_get_caixa(
    db: Session, caixa_id: int, user_id: int, check_usuario_id: bool = True
):
    """
    ...
    if check_usuario_id:
        ...
        return get_by_id_user(
            db, Caixa, caixa_id, user_id, error_message="Caixa não encontrado"
        )
