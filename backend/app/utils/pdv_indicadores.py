...
def calcular_indicadores_venda(
    ...
    taxa_entrega_comissao_entregador = (
        taxa_entrega_cobrada - taxa_entrega_receita_empresa
    )

    # ========== CÁLCULO DE CUSTOS ==========
    # Custo Total = produtos + operacional entrega + comissão entregador
    custo_total_real = (
        custo_total + custo_operacional_entrega + taxa_entrega_comissao_entregador
    )
    ...
    if forma_pagamento_id:
        forma_pag = (
            ...
            .filter(FormaPagamento.id == forma_pagamento_id)
            .first()
        )
    ...
    margem_bruta_percentual = (
        (lucro_bruto / total_venda * 100) if total_venda > 0 else 0
    )
    margem_liquida_percentual = (
        (lucro_liquido / total_venda * 100) if total_venda > 0 else 0
    )
    ...
    return {
        ...
        "custos": {
            ...
            "custos_fiscais_totais": round(
                taxa_valor + imposto_valor + comissao_final, 2
            ),
        },
        "margens": {
...
def calcular_indicadores_item(
    ...
    margem_estimada_percentual = (
        (lucro_estimado / total_venda * 100) if total_venda > 0 else 0
    )
...
def calcular_sugestao_pix(
    ...
    if taxa_cartao_pct > 0:
        ...
        if percentual_sugerido < 0.5:
            return {"tem_sugestao": False}
        percentual_sugerido = min(
            percentual_sugerido, taxa_cartao_pct - 0.5
        )  # nunca igual ou maior que taxa
    ...
    margem_pix_sem_desconto = (
        (lucro_pix_sem_desconto / valor_base * 100) if valor_base > 0 else 0
    )
    ...
    margem_final = (
        round(lucro_final / total_com_desconto * 100, 1)
