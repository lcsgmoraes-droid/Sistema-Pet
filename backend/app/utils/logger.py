...
def configure_logging():
    ...
    logging.getLogger("uvicorn.access").setLevel(
        logging.WARNING
    )  # Evita duplicação (usamos nosso middleware)
...
class StructuredLogger:
    ...
    def info(
        self, event_or_message: str = None, message: Optional[str] = None, **kwargs
    ):
        ...
        if event_or_message is None and "event" in kwargs:
            ...
            self._log(
                logging.INFO, "info", event_or_message, kwargs if kwargs else None
            )
        else:
            # Uso estruturado: logger.info("evento", "mensagem")
            self._log(
                logging.INFO, event_or_message, message, kwargs if kwargs else None
            )
    ...
    def warning(
        ...
        if event_or_message is None and "event" in kwargs:
            ...
            self._log(
                logging.WARNING, "warning", event_or_message, kwargs if kwargs else None
            )
        else:
            # Uso estruturado: logger.warning("evento", "mensagem")
            self._log(
                logging.WARNING, event_or_message, message, kwargs if kwargs else None
            )
    ...
    def error(
        ...
        if event_or_message is None and "event" in kwargs:
            ...
            self._log(
                logging.ERROR, "error", event_or_message, kwargs if kwargs else None
            )
        else:
            # Uso estruturado: logger.error("evento", "mensagem")
            self._log(
                logging.ERROR, event_or_message, message, kwargs if kwargs else None
            )
    ...
    def debug(
        ...
        if event_or_message is None and "event" in kwargs:
            ...
            self._log(
                logging.DEBUG, "debug", event_or_message, kwargs if kwargs else None
            )
        else:
            # Uso estruturado: logger.debug("evento", "mensagem")
            self._log(
                logging.DEBUG, event_or_message, message, kwargs if kwargs else None
            )
