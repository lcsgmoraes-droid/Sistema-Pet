"""CLI para reparar inconsistencias financeiras historicas com dry-run padrao."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Planeja ou aplica reparos financeiros historicos seguros por tenant."
        )
    )
    parser.add_argument("--tenant-id", required=True, help="UUID do tenant alvo.")
    parser.add_argument("--data-inicio", required=True, help="Data inicial YYYY-MM-DD.")
    parser.add_argument(
        "--data-fim",
        required=True,
        help="Data final exclusiva YYYY-MM-DD.",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Persiste os reparos. Sem esta flag executa apenas dry-run.",
    )
    parser.add_argument(
        "--confirm-token",
        default=None,
        help=f"Token literal exigido no apply: {CONFIRM_TOKEN_REPARO_FINANCEIRO}.",
    )
    parser.add_argument(
...
def _validate_args(args: argparse.Namespace) -> tuple[str, date, date]:
    ...
    if args.confirm_token != CONFIRM_TOKEN_REPARO_FINANCEIRO:
        raise ValueError(f"--confirm-token deve ser {CONFIRM_TOKEN_REPARO_FINANCEIRO}.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    db = SessionLocal()
    try:
        payload = reparar_financeiro_consistencia(
            db,
            tenant_id=tenant_id,
