...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Troca o e-mail de login de um usuario sem alterar seus dados.")
...
def _build_update_sql(columns: set[str]) -> str:
    ...
    assignments.extend(f"{column} = NULL" for column in nullable_token_columns if column in columns)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if existing:
            db.rollback()
            return _fail(f"Novo e-mail ja esta em uso pelo usuario #{existing['id']}: {new_email}.", dry_run=dry_run)

        db.execute(text(_build_update_sql(columns)), {"new_email": new_email, "user_id": current["id"]})
