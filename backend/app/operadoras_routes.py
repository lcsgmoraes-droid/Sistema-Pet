...
class OperadoraCartaoCreate(BaseModel):
    ...
    nome: str = Field(
        ..., min_length=1, max_length=100, description="Nome da operadora"
    )
    codigo: Optional[str] = Field(
        None, max_length=50, description="Código da operadora (STONE, CIELO, etc)"
    )
    max_parcelas: int = Field(
        12, ge=1, le=24, description="Máximo de parcelas permitidas (1-24)"
    )
    ...
    taxa_debito: Optional[float] = Field(
        None, ge=0, le=100, description="Taxa de débito (%)"
    )
    taxa_credito_vista: Optional[float] = Field(
        None, ge=0, le=100, description="Taxa de crédito à vista (%)"
    )
    taxa_credito_parcelado: Optional[float] = Field(
        None, ge=0, le=100, description="Taxa de crédito parcelado (%)"
    )

    # API
    api_enabled: bool = Field(False, description="Se integração API está habilitada")
    api_endpoint: Optional[str] = Field(
        None, max_length=255, description="Endpoint da API"
    )
    api_token_encrypted: Optional[str] = Field(
        None, description="Token criptografado da API"
    )

    # UI
    cor: Optional[str] = Field(None, max_length=7, description="Cor hex (#RRGGBB)")
    icone: Optional[str] = Field(
        None, max_length=50, description="Nome do ícone Lucide"
    )
...
def validar_vendas_vinculadas(db: Session, operadora_id: int) -> None:
    ...
    total_vendas = (
        ...
        .filter(VendaPagamento.operadora_id == operadora_id)
        .count()
    )
...
def desmarcar_outras_operadoras_padrao(
    db: Session, tenant_id: str, operadora_id: int
) -> None:
    """
...
def listar_operadoras(
    ...
    query = db.query(OperadoraCartao).filter(OperadoraCartao.tenant_id == tenant_id)
...
@router.post(
    "", response_model=OperadoraCartaoResponse, status_code=status.HTTP_201_CREATED
)
...
def atualizar_operadora(
    ...
    if (
        operadora_data.padrao is False or operadora_data.ativo is False
    ) and operadora.padrao:
