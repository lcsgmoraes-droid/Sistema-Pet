...
if ENCRYPTION_KEY:
    try:
        ...
        logger.info(
            '💡 Gere uma nova chave com: python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"'
        )
else:
    logger.warning(
        "⚠️  ENCRYPTION_KEY não configurada no .env. Criptografia desabilitada."
    )
    logger.info(
