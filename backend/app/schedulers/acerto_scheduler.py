...
class AcertoScheduler:
    ...
    def _processar_acertos_diarios_por_tenant(self, db: Session, dia_hoje: int):
        ...
        for row in tenant_rows:
            ...
            try:
                ...
                logger.warning(
                    "[SCHEDULER] Ignorando tenant_id invalido em acertos: %s",
                    tenant_id_raw,
                )
                continue
            ...
            with tenant_context(tenant_id):
                ...
                parceiros_elegiveis = (
                    ...
                    .filter(
                        ...
                        Cliente.parceiro_tipo_acerto.in_(
                ...
                for parceiro in parceiros_elegiveis:
                    try:
                        logger.info(
                            f"   Processando: {parceiro.nome} (ID: {parceiro.id})"
                        )
                        ...
                        if resultado.get("idempotente"):
                            logger.info(
                                "      [SKIP] Acerto ja existente (idempotencia)"
                            )
                            acertos_pulados += 1
                            continue

                        logger.info(
                            f"      [OK] Acerto gerado: ID {resultado['acerto_id']}"
                        )
                        logger.info(
                            f"         Comissoes: {resultado['comissoes_fechadas']}"
                        )
                        logger.info(
                            f"         Valor liquido: R$ {resultado['valor_liquido']:.2f}"
                        )
                        ...
                        if parceiro.parceiro_notificar and resultado.get("dados_email"):
                            try:
                                assunto, corpo_html, corpo_texto = (
                                    EmailService.renderizar_template(
                                        db=db,
                                ...
                                if parceiro.parceiro_email_principal or parceiro.email:
                                    destinatarios.append(
                                        parceiro.parceiro_email_principal
                                        or parceiro.email
                                    )
                                ...
                                if destinatarios:
                                    ...
                                    logger.info(
                                        f"      [EMAIL] Enfileirado para: {', '.join(destinatarios)}"
                                    )

                            except Exception as e:
                                logger.error(
                                    f"      [ERROR] Erro ao enfileirar email: {str(e)}"
                                )
    ...
    def processar_acertos_diarios(self):
        ...
        try:
            ...
            logger.error(
                f"[CRITICAL] Erro critico no processamento de acertos: {str(e)}"
            )
            import traceback
    ...
    def ajustar_media_entregas_mensal(self):
        ...
        try:
            ...
            for tenant in tenants:
                try:
                    logger.info(
                        f"   Processando tenant: {tenant.name} (ID: {tenant.id})"
                    )

                    resultados = ajustar_media_entregas_mensal(
                        db=db, tenant_id=tenant.id, mes=hoje.month, ano=hoje.year
                    )
            ...
            logger.info(
                f"[OK] Ajuste concluído. Total de entregadores ajustados: {total_ajustados}"
            )
