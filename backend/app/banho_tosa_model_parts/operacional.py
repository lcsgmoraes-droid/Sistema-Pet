...
class BanhoTosaAtendimento(BaseTenantModel):
    ...
    conta_receber_id = Column(
        Integer, ForeignKey("contas_receber.id"), nullable=True, index=True
    )
    pacote_credito_id = Column(
        Integer, ForeignKey("banho_tosa_pacote_creditos.id"), nullable=True, index=True
    )
    pacote_movimento_id = Column(
        Integer,
        ForeignKey("banho_tosa_pacote_movimentos.id"),
        nullable=True,
        index=True,
    )
    ...
    pacote_credito = relationship(
        "BanhoTosaPacoteCredito", foreign_keys=[pacote_credito_id]
    )
    pacote_movimento = relationship(
        "BanhoTosaPacoteMovimento", foreign_keys=[pacote_movimento_id]
    )
    avaliacoes = relationship(
        "BanhoTosaAvaliacao", back_populates="atendimento", cascade="all, delete-orphan"
    )
    etapas = relationship(
        "BanhoTosaEtapa", back_populates="atendimento", cascade="all, delete-orphan"
    )
    insumos_usados = relationship(
...
class BanhoTosaEtapa(BaseTenantModel):
    ...
    __table_args__ = (
        Index("ix_bt_etapas_atendimento_tipo", "tenant_id", "atendimento_id", "tipo"),
    )
    ...
    recurso_id = Column(
        Integer, ForeignKey("banho_tosa_recursos.id"), nullable=True, index=True
    )
...
class BanhoTosaInsumoPrevisto(BaseTenantModel):
    ...
    servico_id = Column(
        Integer,
        ForeignKey("banho_tosa_servicos.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    porte_id = Column(
