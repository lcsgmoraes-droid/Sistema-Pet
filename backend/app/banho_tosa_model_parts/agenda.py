...
class BanhoTosaAgendamento(BaseTenantModel):
    ...
    responsavel_agendamento_user_id = Column(
        Integer, ForeignKey("users.id"), nullable=True, index=True
    )
    profissional_principal_id = Column(
    ...
    profissional_principal = relationship(
        "Cliente", foreign_keys=[profissional_principal_id]
    )
    ...
    atendimento = relationship(
        "BanhoTosaAtendimento", back_populates="agendamento", uselist=False
    )
...
class BanhoTosaAgendamentoServico(BaseTenantModel):
    ...
    __table_args__ = (
        Index("ix_bt_ag_servicos_agendamento", "tenant_id", "agendamento_id"),
    )

    agendamento_id = Column(
        Integer,
        ForeignKey("banho_tosa_agendamentos.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    servico_id = Column(
