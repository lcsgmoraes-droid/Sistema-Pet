...
def gerar_venda_atendimento(
    db: Session, tenant_id, user_id: int, atendimento_id: int
) -> dict:
    ...
    if atendimento.status in {"cancelado", "no_show"}:
        raise HTTPException(
            status_code=422, detail="Atendimento cancelado/no-show nao pode gerar venda"
        )
    if atendimento.pacote_credito_id:
        raise HTTPException(
            status_code=422,
            detail="Atendimento ja foi quitado por pacote e nao deve gerar nova venda",
        )
    ...
    if not itens:
        raise HTTPException(
            status_code=422,
            detail="Atendimento nao possui valor ou servicos para gerar venda",
        )
    ...
    venda = (
        db.query(Venda)
        .filter(Venda.id == venda_dict["id"], Venda.tenant_id == tenant_id)
        .first()
    )
...
def _montar_itens_venda(db: Session, tenant_id, atendimento) -> list[dict]:
    ...
    if agendamento:
        for servico in agendamento.servicos or []:
            subtotal = _subtotal(
                servico.quantidade, servico.valor_unitario, servico.desconto
            )
            if subtotal <= 0:
                continue
            itens.append(
                _item_servico(
                    servico.nome_servico_snapshot,
                    servico.quantidade,
    ...
    if not itens:
        valor = Decimal(
            str(getattr(agendamento, "valor_previsto", 0) if agendamento else 0)
        )
        if valor > 0:
            itens.append(
                _item_servico(
                    f"Banho & Tosa - Atendimento #{atendimento.id}",
                    1,
                    valor,
                    0,
                    valor,
...
def _item_servico(
    nome, quantidade, valor_unitario, desconto, subtotal, pet_id: int
) -> dict:
    return {
        "tipo": "servico",
...
def _subtotal(quantidade, valor_unitario, desconto) -> Decimal:
    return max(
        (Decimal(str(quantidade or 0)) * Decimal(str(valor_unitario or 0)))
        - Decimal(str(desconto or 0)),
        Decimal("0"),
    )
...
def _resposta_venda(atendimento_id: int, venda: Venda, ja_existia: bool) -> dict:
    return {
        ...
        "mensagem": "Venda ja estava vinculada ao atendimento."
        if ja_existia
        else "Venda gerada para cobranca no PDV.",
    }
