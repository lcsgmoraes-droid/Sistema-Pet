...
def _sincronizar_sequence_postgres(
    db: Session, tabela: str, coluna: str = "id"
) -> None:
...
def ensure_especies_racas_tables_exist(db: Session) -> None:
    ...
    db.execute(
        text(
            "ALTER TABLE racas ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()"
        )
    )
    coluna_legada_especie = db.execute(
        text("""
        SELECT EXISTS (
            SELECT 1
    ...
    db.execute(
        text("CREATE INDEX IF NOT EXISTS ix_racas_especie_id ON racas (especie_id)")
    )
    db.execute(text("CREATE INDEX IF NOT EXISTS ix_racas_especie ON racas (especie)"))
    db.execute(
        text("CREATE INDEX IF NOT EXISTS ix_racas_tenant_id ON racas (tenant_id)")
    )
...
@router.post(
    "/especies", response_model=EspecieResponse, status_code=status.HTTP_201_CREATED
)
def criar_especie(
    ...
    existe = (
        db.query(Especie)
        .filter(Especie.tenant_id == tenant_id, Especie.nome.ilike(especie_data.nome))
        .first()
    )
    ...
    nova_especie = Especie(
        tenant_id=tenant_id, nome=especie_data.nome, ativo=especie_data.ativo
    )
...
def atualizar_especie(
    ...
    old_data = {"nome": especie.nome, "ativo": especie.ativo}
...
def deletar_especie(
    ...
    racas_ativas = (
        db.query(Raca).filter(Raca.especie_id == especie_id, Raca.ativo).count()
    )
...
def criar_raca(
    ...
    especie = (
        db.query(Especie)
        .filter(Especie.id == raca_data.especie_id, Especie.tenant_id == tenant_id)
        .first()
    )
...
def atualizar_raca(
    ...
    if not raca:
        ...
        # Salvar dados antigos para audit log
    old_data = {"nome": raca.nome, "especie_id": raca.especie_id, "ativo": raca.ativo}
