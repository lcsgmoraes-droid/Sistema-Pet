...
class OpportunityEventPayload(BaseModel):
    ...
    cliente_id: Optional[int] = Field(
        None, description="ID do cliente se houver (pode ser venda sem cadastro)"
    )
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp UTC do evento"
    )
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "metadata": {"version": "1.0", "confidence_score": 0.85},
            }
        }
...
class OpportunityEventResponse(BaseModel):
    ...
    success: bool = Field(..., description="True se evento foi registrado com sucesso")

    event_id: Optional[str] = Field(
        None, description="ID do evento registrado (se sucesso)"
    )

    message: Optional[str] = Field(
        None, description="Mensagem de status ou erro (não crítico)"
    )
