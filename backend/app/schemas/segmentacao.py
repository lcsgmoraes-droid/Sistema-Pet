...
class MetricasCliente(BaseModel):
    ...
    compras_90d: int = Field(
        ..., description="Quantidade de compras nos últimos 90 dias"
    )
    ...
    compras_90d_anteriores: int = Field(
        ..., description="Quantidade de compras no período anterior (90-180 dias)"
    )
    total_historico: float = Field(..., description="Total gasto desde sempre")
    total_compras_historico: int = Field(
        ..., description="Total de compras desde sempre"
    )
...
class SegmentoResponse(BaseModel):
    ...
    segmento: str = Field(
        ...,
        description="Segmento principal: VIP, Recorrente, Novo, Inativo, Endividado, Risco, Regular",
    )
    tags: List[str] = Field(
        default_factory=list, description="Lista de tags/segmentos aplicáveis"
    )
...
class RecalcularTodosRequest(BaseModel):
    ...
    limit: Optional[int] = Field(
        None, description="Limite de clientes a processar (None = todos)", ge=1
    )
...
class RecalcularTodosResponse(BaseModel):
    ...
    distribuicao_segmentos: Dict[str, int] = Field(
        default_factory=dict, description="Contagem de clientes por segmento"
    )
