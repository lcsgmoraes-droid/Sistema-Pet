...
class RacaoCalculadoraInput(BaseModel):
    ...
    especie: Literal["cao", "gato"] = Field(..., description="Espécie do animal")

    peso_kg: float = Field(
        ..., gt=0, description="Peso do animal em kg (deve ser maior que 0)"
    )
...
class RacaoCalculadoraOutput(BaseModel):
    ...
    custo_diario: float = Field(..., description="Custo diário de alimentação em reais")
