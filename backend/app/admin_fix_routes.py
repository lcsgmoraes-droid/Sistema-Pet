    ...
    for table_name in tables_to_fix:
        try:
            ...
            seq_result = db.execute(
                text(
                    f"SELECT setval(pg_get_serial_sequence('{table_name}', 'id'), :next_id, false)"
                ),
                {"next_id": next_id},
            )
            db.commit()
            ...
            results[table_name] = {"status": "❌ ERRO", "error": str(e)}

    return {"message": "Sequências corrigidas", "results": results}
