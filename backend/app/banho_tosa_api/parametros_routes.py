...
def listar_parametros_porte(
    ...
    query = db.query(BanhoTosaParametroPorte).filter(
        BanhoTosaParametroPorte.tenant_id == tenant_id
    )
    if ativos_only:
        query = query.filter(BanhoTosaParametroPorte.ativo.is_(True))
    return query.order_by(
        BanhoTosaParametroPorte.peso_min_kg.asc().nullslast(),
        BanhoTosaParametroPorte.porte.asc(),
    ).all()


@router.post(
    "/parametros-porte", response_model=BanhoTosaParametroPorteResponse, status_code=201
)
def criar_parametro_porte(
    ...
    if existente:
        raise HTTPException(
            status_code=409, detail="Ja existe um parametro para esse porte"
        )
...
@router.patch(
    "/parametros-porte/{porte_id}", response_model=BanhoTosaParametroPorteResponse
)
