...
def listar_servicos(
    ...
    return query.order_by(
        BanhoTosaServico.categoria.asc(), BanhoTosaServico.nome.asc()
    ).all()
...
def criar_servico(
    ...
    if existente:
        raise HTTPException(
            status_code=409, detail="Ja existe um servico de Banho & Tosa com esse nome"
        )
...
def atualizar_servico(
    ...
    if not servico:
        raise HTTPException(
            status_code=404, detail="Servico de Banho & Tosa nao encontrado"
        )
    ...
    if "nome" in payload:
        ...
        if duplicado:
            raise HTTPException(
                status_code=409,
                detail="Ja existe um servico de Banho & Tosa com esse nome",
            )
        payload["nome"] = nome
