...
class InsightEngine:
    ...
    def gerar_insights_clientes(self, user_id: int) -> List[Insight]:
        ...
        try:
            insights_atrasados = regra_cliente_recorrente_atrasado(
                self.clientes_rm, user_id=user_id
            )
        ...
        try:
            insights_inativos = regra_cliente_inativo(self.clientes_rm, user_id=user_id)
    ...
    def gerar_insights_produtos(
        self, user_id: int, produto_id: int, min_confianca: float = 50.0
    ) -> List[Insight]:
        """
    ...
    def gerar_insights_produtos_geral(
        self, user_id: int, limit_produtos: int = 5
    ) -> List[Insight]:
        """
        ...
        top_produtos = self.produtos_rm.top_produtos(
            limit=limit_produtos, user_id=user_id
        )
    ...
    def gerar_insights_kits(self, user_id: int) -> List[Insight]:
        ...
        try:
            insights = regra_kit_mais_vantajoso(
                self.kits_rm, self.produtos_rm, user_id=user_id
            )
            return insights
        except Exception as e:
    ...
    def filtrar_por_severidade(
        self, insights: List[Insight], severidade: SeveridadeInsight
    ) -> List[Insight]:
        """
    ...
    def filtrar_por_tipo(
        self, insights: List[Insight], tipo: TipoInsight
    ) -> List[Insight]:
        """
    ...
    def agrupar_por_entidade(self, insights: List[Insight]) -> Dict[str, List[Insight]]:
        """
    ...
    def resumo_executivo(
        self, insights: List[Insight], top_n: int = 5
    ) -> Dict[str, Any]:
        """
