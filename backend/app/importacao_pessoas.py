...
async def criar_template_pessoas(current_user=Depends(get_current_user)):
    ...
    # Instrução
    ws["A1"] = (
        "INSTRUÇÕES: Preencha os dados abaixo para importar Pessoas (Clientes, Fornecedores, Veterinários)"
    )
            ...
            try:
                ...
                if cpf:
                    pessoa_existente = (
                        db.query(Cliente)
                        .filter(
                            Cliente.cpf == cpf.strip(), Cliente.tenant_id == tenant_id
                        )
                        .first()
                    )
                elif cnpj:
                    pessoa_existente = (
                        db.query(Cliente)
                        .filter(
                            Cliente.cnpj == cnpj.strip(), Cliente.tenant_id == tenant_id
                        )
                        .first()
                    )
                elif codigo:
