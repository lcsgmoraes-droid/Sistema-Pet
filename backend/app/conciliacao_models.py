...
from .conciliacao_recebimento_models import (
    ConciliacaoMetrica,
...
class EmpresaParametros(TenantScoped, Base):
    ...
    # Vencimentos padrão
    dias_vencimento_cartao_debito = Column(Integer, default=1, comment="D+1 = amanhã")
    dias_vencimento_cartao_credito_av = Column(
        Integer, default=30, comment="D+30 para crédito à vista"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "taxas_estimadas": {
                ...
                "credito_parcelado_mdr": float(
                    self.taxa_mdr_credito_parcelado_estimada
                ),
            },
        }
...
class AdquirenteTemplate(TenantScoped, Base):
    ...
    # Identificação
    nome = Column(
        String(100), nullable=False, comment="Stone, Cielo, Rede, Getnet, etc"
    )
    tipo_arquivo = Column(
        String(50), nullable=False, comment="recebimentos, pagamentos, vendas"
    )
    ...
    mapeamento = Column(
        JSONB, nullable=False, comment="Mapeamento nome_campo: nome_coluna_csv"
    )

    # Transformações opcionais (formato de datas, decimais, etc)
    transformacoes = Column(
        JSONB, nullable=True, comment="Regras de transformação por campo"
    )
...
class ArquivoEvidencia(TenantScoped, Base):
    ...
    tipo_arquivo = Column(
        String(50), nullable=False, comment="ofx, recebimentos, pagamentos, vendas"
    )
    adquirente = Column(String(100), nullable=True, comment="Stone, Cielo, etc")

    # Armazenamento
    caminho_storage = Column(
        String(500), nullable=False, comment="Caminho no storage/S3"
    )
    tamanho_bytes = Column(BigInteger, nullable=True)
    hash_md5 = Column(
        String(32), nullable=False, index=True, comment="Detectar duplicatas"
    )
    ...
    def to_dict(self):
        return {
            ...
            "tamanho_mb": round(self.tamanho_bytes / 1024 / 1024, 2)
...
class ConciliacaoImportacao(TenantScoped, Base):
    ...
    # Referências
    arquivo_evidencia_id = Column(
        Integer,
        ForeignKey("arquivos_evidencia.id", ondelete="RESTRICT"),
        nullable=False,
    )
    adquirente_template_id = Column(
        Integer,
        ForeignKey("adquirentes_templates.id", ondelete="RESTRICT"),
        nullable=True,
    )
    ...
    data_referencia = Column(
        Date, nullable=False, index=True, comment="Data do movimento/crédito"
    )
    ...
    resumo = Column(
        JSONB, nullable=True, comment="Resumo da importação (formato livre)"
    )
    ...
    validacoes_ofx = relationship(
        "ConciliacaoValidacao",
        foreign_keys="ConciliacaoValidacao.importacao_ofx_id",
        back_populates="importacao_ofx",
    )
    validacoes_pagamentos = relationship(
        "ConciliacaoValidacao",
        foreign_keys="ConciliacaoValidacao.importacao_pagamentos_id",
        back_populates="importacao_pagamentos",
    )
    validacoes_recebimentos = relationship(
        "ConciliacaoValidacao",
        foreign_keys="ConciliacaoValidacao.importacao_recebimentos_id",
        back_populates="importacao_recebimentos",
    )
...
class ConciliacaoLote(BaseTenantModel):
    ...
    # Vínculos
    importacao_pagamentos_id = Column(
        Integer,
        ForeignKey("conciliacao_importacoes.id", ondelete="SET NULL"),
        nullable=True,
    )
    importacao_ofx_id = Column(
        Integer,
        ForeignKey("conciliacao_importacoes.id", ondelete="SET NULL"),
        nullable=True,
    )
    movimentacao_bancaria_id = Column(
        Integer,
        ForeignKey("movimentacoes_bancarias.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    identificador_lote = Column(
        String(255), nullable=True, comment="ID fornecido pela operadora (se existir)"
    )
    ...
    modalidade = Column(
        String(50), nullable=True, comment="Antecipação, Débito, Crédito"
    )
...
class ConciliacaoValidacao(BaseTenantModel):
    ...
    importacao_ofx_id = Column(
        Integer,
        ForeignKey("conciliacao_importacoes.id", ondelete="CASCADE"),
        nullable=True,
    )
    importacao_pagamentos_id = Column(
        Integer,
    ...
    importacao_recebimentos_id = Column(
        Integer,
        ForeignKey("conciliacao_importacoes.id", ondelete="CASCADE"),
        nullable=True,
    )
    ...
    confianca = Column(
        String(20), nullable=False, index=True, comment="ALTA, MEDIA, BAIXA"
    )
    pode_processar = Column(
        Boolean, default=True, comment="Sempre True - sistema nunca bloqueia"
    )
    ...
    # Relationships
    importacao_ofx = relationship(
        "ConciliacaoImportacao",
        foreign_keys=[importacao_ofx_id],
        back_populates="validacoes_ofx",
    )
    importacao_pagamentos = relationship(
        "ConciliacaoImportacao",
        foreign_keys=[importacao_pagamentos_id],
        back_populates="validacoes_pagamentos",
    )
    importacao_recebimentos = relationship(
        "ConciliacaoImportacao",
        foreign_keys=[importacao_recebimentos_id],
        back_populates="validacoes_recebimentos",
    )
    ...
    def to_dict(self):
        return {
            ...
            "totais": {
                ...
                "pagamentos": float(self.total_pagamentos)
                if self.total_pagamentos
                else None,
                "recebimentos": float(self.total_recebimentos)
                if self.total_recebimentos
                else None,
            },
            "diferencas": {
                "ofx_vs_pagamentos": float(self.diferenca_ofx_pagamentos),
                "pagamentos_vs_recebimentos": float(
                    self.diferenca_pagamentos_recebimentos
                ),
...
class ConciliacaoLog(BaseTenantModel):
    ...
    conciliacao_validacao_id = Column(
        Integer, nullable=True
    )  # was: ForeignKey('conciliacao_validacoes.id', ondelete='CASCADE')
    ...
    status_acao = Column(
        String(50), default="sucesso", comment="sucesso, erro, parcial, revertido"
    )

    # Detalhes (JSONB flexível)
    arquivos_utilizados = Column(
        JSONB, nullable=True, comment="Lista de arquivos usados"
    )
    ...
    motivo = Column(
        Text, nullable=True, comment="Justificativa de reversão ou confirmação"
    )
