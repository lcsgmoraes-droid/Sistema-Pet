...
class WhatsAppAnalyticsService:
    ...
    def get_dashboard_metrics(
        ...
        message_query = (
            ...
            .join(WhatsAppSession, WhatsAppMessage.session_id == WhatsAppSession.id)
            .filter(
                WhatsAppSession.tenant_id == self.tenant_id,
        ...
        handoff_query = (
            ...
            .join(WhatsAppSession, WhatsAppHandoff.session_id == WhatsAppSession.id)
            .filter(
                WhatsAppSession.tenant_id == self.tenant_id,
        ...
        avg_response_time_query = (
            self.db.query(
                func.avg(
                    ...
                    - func.lag(func.extract("epoch", WhatsAppMessage.created_at)).over(
                        partition_by=WhatsAppMessage.session_id,
        ...
        intent_stats = (
            self.db.query(
                WhatsAppMessage.intent, func.count(WhatsAppMessage.id).label("count")
            )
            .join(WhatsAppSession, WhatsAppMessage.session_id == WhatsAppSession.id)
            .filter(
                WhatsAppSession.tenant_id == self.tenant_id,
            ...
            .group_by(WhatsAppMessage.intent)
        ...
        top_intents = [
            {"intent": row.intent, "count": row.count} for row in intent_stats
        ]
        ...
        peak_hours_query = (
            ...
            .group_by(extract("hour", WhatsAppMessage.created_at))
            .order_by(extract("hour", WhatsAppMessage.created_at))
            .all()
        )

        peak_hours = [
            {"hour": int(row.hour), "count": row.count} for row in peak_hours_query
        ]

        # 8. Sentiment analysis distribution
        sentiment_stats = (
            self.db.query(
                WhatsAppMessage.sentiment, func.count(WhatsAppMessage.id).label("count")
            )
            .join(WhatsAppSession, WhatsAppMessage.session_id == WhatsAppSession.id)
            .filter(
                WhatsAppSession.tenant_id == self.tenant_id,
            ...
            .group_by(WhatsAppMessage.sentiment)
            .all()
        )
        ...
        # 10. Tools usage
        tools_usage_query = (
            self.db.query(WhatsAppMessage.tools_used)
            .join(WhatsAppSession, WhatsAppMessage.session_id == WhatsAppSession.id)
            .filter(
                WhatsAppSession.tenant_id == self.tenant_id,
    ...
    def get_agent_performance(
        ...
        performance = (
            ...
            .group_by(WhatsAppAgent.id, WhatsAppAgent.name, WhatsAppAgent.email)
            .all()
        )
    ...
    def get_nps_score(
        ...
        # Query para ratings
        ratings = (
            self.db.query(WhatsAppSession.rating)
            .filter(
                WhatsAppSession.tenant_id == self.tenant_id,
