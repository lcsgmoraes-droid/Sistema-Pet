...
class ContextManager:
    ...
    def _load_session_history(
        self, db: Session, session: WhatsAppSession, context: ConversationContext
    ) -> ConversationContext:
        """Carrega histórico de mensagens da sessão"""
        try:
            messages = (
                db.query(WhatsAppMessage)
                .filter(WhatsAppMessage.session_id == session.id)
            ...
            logger.info(
                f"Carregadas {len(messages)} mensagens para sessão {session.id}"
            )
    ...
    def _identify_customer(
        self, db: Session, phone_number: str, tenant_id: str
