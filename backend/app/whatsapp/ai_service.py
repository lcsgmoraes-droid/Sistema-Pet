...
class AIService:
    ...
    def __init__(self, db: Session, tenant_id: str):
        ...
        self.formatter = ResponseFormatter(
            loja_nome=self.config.bot_name if self.config else "Pet Shop"
        )
    ...
    def _load_config(self) -> Optional[TenantWhatsAppConfig]:
        ...
        if not config:
            logger.error(
                f"Configuração WhatsApp não encontrada para tenant {self.tenant_id}"
            )
    ...
    async def process_message(
        self, message: str, phone_number: str, session_id: Optional[str] = None
    ) -> Dict[str, Any]:
        with whatsapp_tenant_context(self.tenant_id):
            return await self._process_message_with_context(
                message, phone_number, session_id
            )
        ...
        try:
            ...
            sentiment_result = self.sentiment_analyzer.analyze(
                ...
                conversation_history=[
                    m["content"] for m in context.messages if m["role"] == "user"
                ],
            )
            logger.info(
                f"😊 Sentimento: {sentiment_result['label']} (score: {sentiment_result['score']})"
            )
            ...
            if session and sentiment_result.get("should_handoff"):
                ...
                logger.warning(
                    f"🚨 Handoff criado: {handoff.id} (reason: {handoff.reason})"
                )
    ...
    def _should_auto_respond(
        self, context: ConversationContext, intent: IntentType
    ) -> tuple[bool, str]:
        """
    ...
    def _is_within_working_hours(self) -> bool:
        ...
        if (
            not self.config
            or not self.config.working_hours_start
            or not self.config.working_hours_end
        ):
            return True
        ...
        try:
            ...
            if tool_calls:
                ...
                for tool_call in tool_calls:
                    ...
                    logger.info(
                        f"Executando tool: {function_name} com args: {function_args}"
                    )
    ...
    def _build_system_prompt(
        self, intent: IntentType, context: ConversationContext
    ) -> str:
...
"""
        ...
        if context.customer_data:
            ...
            if customer.get("pets"):
                base_prompt += (
                    f"- Pets: {', '.join([p['name'] for p in customer['pets']])}\n"
                )
