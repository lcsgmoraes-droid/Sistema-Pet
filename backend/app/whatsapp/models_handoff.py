...
class WhatsAppHandoff(TenantScoped, Base):
    ...
    notes = relationship(
        "WhatsAppInternalNote", back_populates="handoff", cascade="all, delete-orphan"
    )
...
class WhatsAppInternalNote(TenantScoped, Base):
    ...
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False)
    handoff_id = Column(
        UUID(as_uuid=True), ForeignKey("whatsapp_handoffs.id"), nullable=False
    )
    session_id = Column(String, ForeignKey("whatsapp_ia_sessions.id"), nullable=False)
    agent_id = Column(
        UUID(as_uuid=True), ForeignKey("whatsapp_agents.id"), nullable=False
    )

    note = Column(Text, nullable=False)
    note_type = Column(
        String(50), default="general"
    )  # general, important, follow_up, warning
