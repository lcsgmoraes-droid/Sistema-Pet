...
class WhatsAppAnalyticsService:
    ...
    def get_dashboard_metrics(
        ...
        if total_sessions > 0:
            auto_resolution_rate = (
                (total_sessions - total_handoffs) / total_sessions
            ) * 100
        ...
        top_intents = (
            ...
            .group_by(WhatsAppMessage.intent_detected)
        ...
        peak_hours = (
            ...
            .group_by(func.extract("hour", WhatsAppMessage.created_at))
            .order_by(func.count(WhatsAppMessage.id).desc())
            .limit(5)
            .all()
        )
        ...
        estimated_cost = (
            total_tokens * 0.375
        ) / 1_000_000  # Média entre input e output
        ...
        return {
            ...
            "messages": {
                ...
                "avg_per_conversation": round(total_messages / total_sessions, 2)
                if total_sessions > 0
                else 0,
            },
            "response_time": {
            ...
            "costs": {
                ...
                "per_conversation": round(estimated_cost / total_sessions, 4)
                if total_sessions > 0
                else 0,
                "currency": "USD",
            ...
            "intents": [
                {"intent": i.intent_detected, "count": i.count} for i in top_intents
            ],
    ...
    def get_conversation_trends(
        ...
        daily_conversations = (
            ...
            .group_by(func.date(WhatsAppSession.started_at))
            .order_by(func.date(WhatsAppSession.started_at))
            .all()
        )
        ...
        return {
            ...
            "daily_conversations": [
                {"date": str(d.date), "count": d.count} for d in daily_conversations
            ],
            "daily_messages": [
                {"date": str(d.date), "count": d.count} for d in daily_messages
            ],
        }
    ...
    def get_handoff_analysis(
        ...
        handoffs_by_reason = (
            self.db.query(
                WhatsAppHandoff.reason, func.count(WhatsAppHandoff.id).label("count")
            )
            .join(WhatsAppSession)
            .filter(
                and_(
            ...
            .group_by(WhatsAppHandoff.reason)
            .all()
        )
        ...
        handoffs_by_priority = (
            self.db.query(
                WhatsAppHandoff.priority, func.count(WhatsAppHandoff.id).label("count")
            )
            .join(WhatsAppSession)
            .filter(
                and_(
            ...
            .group_by(WhatsAppHandoff.priority)
            .all()
        )
        ...
        return {
            ...
            "by_reason": {
                ...
                for reason, count in handoffs_by_reason
            },
            "by_priority": {
                priority: count for priority, count in handoffs_by_priority
            },
    ...
    def get_cost_analysis(
        ...
        costs = (
            self.db.query(
                ...
                func.sum(
                    WhatsAppMessage.tokens_input + WhatsAppMessage.tokens_output
                ).label("total_tokens"),
            ...
            "by_direction": [
                {
                    ...
                    "estimated_cost": round(
                        ((c.total_tokens or 0) * 0.375) / 1_000_000, 4
                    ),
                }
                for c in costs
            ],
    ...
    def get_agent_performance(
        ...
        for agent in agents:
            ...
            # Taxa de resolução
            resolution_rate = (
                (resolved_handoffs / total_handoffs * 100) if total_handoffs > 0 else 0
            )
            ...
            avg_time = (
                self.db.query(
                    func.avg(
                        func.extract(
                            "epoch",
                            WhatsAppHandoff.resolved_at - WhatsAppHandoff.created_at,
                        )
                        / 60
                    )
                )
