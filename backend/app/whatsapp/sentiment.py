...
class SentimentAnalyzer:
    ...
    def analyze(self, message: str, conversation_history: List[str] = None) -> Dict:
        ...
        if label in ["very_negative", "negative"]:
            if score <= Decimal("-0.6"):
                ...
                logger.warning(
                    f"Sentiment trigger: score={score}, keywords={matched_keywords}"
                )
        ...
        if conversation_history:
            ...
            repeated_count = sum(
                1 for msg in conversation_history if msg.lower() == message_lower
            )
            if repeated_count >= 2:
                should_handoff = True
    ...
    def analyze_conversation_trend(self, messages: List[Dict]) -> Dict:
        ...
        if should_handoff:
            logger.warning(
                f"Conversation deteriorating: recent={recent_avg:.2f}, older={older_avg:.2f}"
            )
