...
class MetricsCollector:
    ...
    def record_human_handoff(self, reason: str, session_id: str):
    ...
    def record_api_cost(
        self, cost_usd: float, tokens_input: int, tokens_output: int, model: str
    ):
...
class MetricsAnalyzer:
    ...
    def get_summary(
        ...
        # Custo total
        total_cost = (
            self.db.query(func.sum(WhatsAppMetric.value))
            .filter(
                WhatsAppMetric.tenant_id == self.tenant_id,
        ...
        if conversations_resolved > 0:
            auto_resolved = self._count_auto_resolved_conversations(
                start_date, end_date
            )
        ...
        return {
            ...
            "rates": {
                ...
                "handoff_rate": round((human_handoffs / messages_processed * 100), 2)
                if messages_processed > 0
                else 0,
            },
            "performance": {"avg_response_time_seconds": round(avg_response_time, 2)},
            "insights": {"top_intents": top_intents, "peak_hours": peak_hours},
        }
    ...
    def _get_top_intents(
        ...
        try:
            ...
            return [
                {"intent": intent, "count": count} for intent, count in sorted_intents
            ]
        except Exception as e:
        ...
        try:
            ...
            for metric in metrics:
                try:
                    ...
                    model_costs[model]["tokens_input"] += metadata.get(
                        "tokens_input", 0
                    )
                    model_costs[model]["tokens_output"] += metadata.get(
                        "tokens_output", 0
                    )
            ...
            return {"by_model": model_costs, "total_tokens": total_tokens}
        except Exception as e:
