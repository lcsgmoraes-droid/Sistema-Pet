...
async def _usuario_whatsapp_security(
    user_and_tenant=Depends(get_current_user_and_tenant),
) -> User:
    return user_and_tenant[0]


async def _tenant_whatsapp_security(
    user_and_tenant=Depends(get_current_user_and_tenant),
):
...
def check_consent(
    ...
    **Retorna:**
    ...
    has_consent = lgpd_service.check_consent(
        subject_id=check_data.subject_id, consent_type=check_data.consent_type
    )
        ...
        "requests": [
            {
                ...
                "status": req.status,
                "reason": req.reason,
                "created_at": req.request_date.isoformat()
                if req.request_date
                else None,
    ...
    **Inclui:**
    ...
    try:
        data = lgpd_service.export_user_data(
            subject_id=export_data.subject_id, subject_type=export_data.subject_type
        )
