...
class WhatsAppSession(TenantScoped, Base):
    ...
    handoffs = relationship(
        "WhatsAppHandoff", back_populates="session", cascade="all, delete-orphan"
    )
...
class WhatsAppMessage(TenantScoped, Base):
    ...
    intent_detected = Column(
        String(100)
    )  # consulta_produto, criar_pedido, suporte, etc
...
class WhatsAppMetric(TenantScoped, Base):
    ...
    )  # message_count, ai_call, conversion, etc
