...
class HandoffManager:
    ...
    def should_handoff(
        ...
        if sentiment_result.get("should_handoff"):
            ...
            logger.info(
                f"Sentiment trigger handoff: reason={reason}, priority={priority}"
            )
        ...
        if session.message_count >= 10:
            logger.warning(
                f"Timeout: session {session.id} has {session.message_count} messages"
            )
    ...
    def create_handoff(
        ...
        logger.info(
            f"Handoff created: {handoff.id} (reason={reason}, priority={priority})"
        )
    ...
    def assign_to_agent(
        self, handoff_id: str, agent_id: str, auto: bool = False
    ) -> WhatsAppHandoff:
        """
        ...
        if handoff.status not in ["pending", "assigned"]:
            raise ValueError(
                f"Handoff {handoff_id} cannot be assigned (status={handoff.status})"
            )
        ...
        agent = (
            ...
            .filter(
                WhatsAppAgent.id == agent_id, WhatsAppAgent.tenant_id == self.tenant_id
            )
            .first()
        )
        ...
        logger.info(
            f"Handoff {handoff_id} assigned to agent {agent.name} ({'auto' if auto else 'manual'})"
        )
    ...
    def resolve_handoff(
        ...
        logger.info(
            f"Handoff {handoff_id} resolved by agent {agent_id} ({handoff.resolution_time_seconds}s)"
        )
    ...
    def _try_auto_assign(self, handoff: WhatsAppHandoff):
        ...
        available_agents = [
            a for a in agents if a.current_chats < a.max_concurrent_chats
        ]
