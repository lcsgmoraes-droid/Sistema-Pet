...
class DateRangeQuery(BaseModel):
    ...
    start_date: Optional[datetime] = Field(
        None, description="Data inicial (ISO format). Padrão: 30 dias atrás"
    )
    end_date: Optional[datetime] = Field(
        None, description="Data final (ISO format). Padrão: agora"
    )
    ...
    **Período padrão:** Últimos 30 dias
    ...
    if start_date:
        try:
            ...
            raise HTTPException(
                400, "Formato de data inválido para start_date. Use YYYY-MM-DD"
            )
    ...
    if end_date:
        try:
            ...
            raise HTTPException(
                400, "Formato de data inválido para end_date. Use YYYY-MM-DD"
            )
...
def get_conversation_trends(
    interval: str = Query(
        "day", description="Intervalo de agrupamento: hour, day, week, month"
    ),
    start_date: Optional[str] = Query(None, description="Data início (YYYY-MM-DD)"),
    end_date: Optional[str] = Query(None, description="Data fim (YYYY-MM-DD)"),
    db: Session = Depends(get_session),
    ...
    **Intervalos disponíveis:** hour, day, week, month
    ...
    if interval not in ["hour", "day", "week", "month"]:
        raise HTTPException(400, "Intervalo inválido. Use: hour, day, week ou month")
...
def get_agent_performance(
    ...
    **Útil para:** Avaliar produtividade, identificar treinamentos necessários
    ...
    try:
        performance = analytics.get_agent_performance(start_date=start, end_date=end)
...
def get_handoff_analysis(
    ...
    **Útil para:** Identificar quando a IA precisa de ajuda, otimizar automação
    ...
    try:
        handoff_data = analytics.get_handoff_analysis(start_date=start, end_date=end)
        return handoff_data
    except Exception as e:
...
def get_cost_analysis(
    ...
    **Útil para:** Controle de budget, otimização de prompts
    ...
    try:
        cost_data = analytics.get_cost_analysis(start_date=start, end_date=end)
        return cost_data
    except Exception as e:
...
def get_quick_summary(
    db: Session = Depends(get_session), current_user: User = Depends(_usuario_analytics)
):
    """
    ...
    try:
        ...
        return {"period": "Últimas 24 horas", "metrics": metrics["overview"]}
    except Exception as e:
