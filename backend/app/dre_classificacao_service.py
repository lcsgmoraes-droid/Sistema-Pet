...
class ClassificadorDRE:
    ...
    def analisar_lancamento(
        ...
        if (
            tipo == "pagar"
            and hasattr(lancamento, "fornecedor")
            and lancamento.fornecedor
        ):
        ...
        if (
            tipo == "receber"
            and hasattr(lancamento, "venda_id")
            and lancamento.venda_id
        ):
            sugestoes.extend(self._aplicar_regras_venda(lancamento))

        if (
            tipo == "pagar"
            and hasattr(lancamento, "nota_entrada_id")
            and lancamento.nota_entrada_id
        ):
        ...
        tem_beneficiario = (
            hasattr(lancamento, "fornecedor") and lancamento.fornecedor
        ) or (hasattr(lancamento, "cliente") and lancamento.cliente)
        if tem_beneficiario:
    ...
    def _aplicar_regras_venda(self, lancamento: ContaReceber) -> List[Dict]:
        ...
        regras = (
            self.db.query(RegraClassificacaoDRE)
            .filter(
                RegraClassificacaoDRE.tenant_id == self.tenant_id,
                RegraClassificacaoDRE.tipo_regra
                == TipoRegraClassificacao.VENDA_AUTOMATICA,
            ...
            sugestoes.append(
                {
                    ...
                    "subcategoria_nome": regra.dre_subcategoria.nome
                    if regra.dre_subcategoria
                    else "N/A",
    ...
    def _aplicar_regras_palavra_chave(self, lancamento) -> List[Dict]:
        ...
        for regra in regras:
            ...
            matches = [
                palavra for palavra in palavras if palavra.lower() in descricao_lower
            ]
    ...
    def _aplicar_regras_tipo_documento(self, lancamento) -> List[Dict]:
        ...
        regras = (
            ...
            .filter(
                RegraClassificacaoDRE.tenant_id == self.tenant_id,
                RegraClassificacaoDRE.tipo_regra
                == TipoRegraClassificacao.TIPO_DOCUMENTO,
                RegraClassificacaoDRE.ativo.is_(True),
            )
            .all()
        )
    ...
    def _aplicar_regras_combo(self, lancamento) -> List[Dict]:
        ...
        for regra in regras:
            ...
            if "beneficiario" in criterios:
                ...
                if (
                    beneficiario
                    and criterios["beneficiario"].lower() in beneficiario.lower()
                ):
                    score += 1
            ...
            if "tipo_documento" in criterios:
                ...
                if hasattr(lancamento, "tipo_documento") and lancamento.tipo_documento:
                    if (
                        criterios["tipo_documento"].lower()
                        == lancamento.tipo_documento.lower()
                    ):
                        score += 1
    ...
    def _sugerir_por_historico(self, lancamento, tipo: str) -> List[Dict]:
        ...
        resultados = (
            historicos.with_entities(
                HistoricoClassificacao.dre_subcategoria_id,
                func.count(HistoricoClassificacao.id).label("qtd"),
            )
            .group_by(HistoricoClassificacao.dre_subcategoria_id)
            .order_by(func.count(HistoricoClassificacao.id).desc())
            .limit(3)
            .all()
        )
        ...
        for subcat_id, qtd in resultados:
            if qtd >= 2:  # Mínimo 2 ocorrências para sugerir
                subcategoria = (
                    self.db.query(DRESubcategoria)
                    .filter(DRESubcategoria.id == subcat_id)
                    .first()
                )
    ...
    def aplicar_classificacao(
        ...
        try:
            ...
            if regra_id:
                regra = (
                    self.db.query(RegraClassificacaoDRE)
                    .filter(RegraClassificacaoDRE.id == regra_id)
                    .first()
                )
            ...
            logger.info(
                f"✅ Classificação aplicada: {tipo} #{lancamento_id} → {subcategoria.nome}"
            )
    ...
    def _avaliar_criar_regra_automatica(
        self, lancamento, tipo: str, dre_subcategoria_id: int
    ):
        """
        ...
        count = (
            self.db.query(HistoricoClassificacao)
            .filter(
                HistoricoClassificacao.tenant_id == self.tenant_id,
                HistoricoClassificacao.tipo_lancamento == tipo,
        ...
        if count >= 3:
            ...
            regra_existente = (
                ...
                .filter(
                    ...
                    RegraClassificacaoDRE.criterios["beneficiario"].astext
                    == beneficiario,
                )
                .first()
            )
    ...
    def listar_pendentes(
        ...
        """
        resultado = {"total_pendentes": 0, "contas_pagar": [], "contas_receber": []}
        ...
        if tipo is None or tipo == "pagar":
            ...
            pendentes_pagar = (
                self.db.query(ContaPagar)
                .outerjoin(Cliente, ContaPagar.fornecedor_id == Cliente.id)
                .filter(
                    ContaPagar.tenant_id == self.tenant_id,
                    ...
                    or_(
                        ...
                        ContaPagar.fornecedor_id.is_(
                            None
                        ),  # Sem fornecedor = controla DRE
                    ),
                )
                .order_by(ContaPagar.data_vencimento.desc())
                .limit(limit)
                .all()
            )
        ...
        if tipo is None or tipo == "receber":
            ...
            pendentes_receber = (
                self.db.query(ContaReceber)
                .outerjoin(Cliente, ContaReceber.cliente_id == Cliente.id)
                .filter(
                    ContaReceber.tenant_id == self.tenant_id,
                ...
                resultado["contas_receber"].append(
                    {
                        ...
                        "data_vencimento": cr.data_vencimento.isoformat()
        ...
        resultado["total_pendentes"] = len(resultado["contas_pagar"]) + len(
