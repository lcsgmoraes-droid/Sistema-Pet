...
def resumir_movimentos_devolucao(movimentos):
    ...
    for mov in movimentos:
        ...
        item = produtos.setdefault(produto_id, {
            "produto_id": produto_id,
            "produto_nome": getattr(mov.produto, "nome", None) or f"Produto #{produto_id}",
        ...
        valor = _decimal(mov.valor_total) if mov.valor_total is not None else quantidade * _decimal(mov.custo_unitario)
    ...
    for item in produtos.values():
        ...
        itens.append({chave: float(valor) if isinstance(valor, Decimal) else valor
...
def buscar_resumos_devolucao(db, *, tenant_id, conta_ids):
    ...
    return {conta_id: resumir_movimentos_devolucao(itens)
...
def preparar_devolucao(itens_disponiveis, itens_solicitados):
    ...
    for pedido in itens_solicitados:
        ...
        if (not quantidade.is_finite() or quantidade <= 0
            ...
            raise HTTPException(400, "Informe uma quantidade positiva com ate 3 casas decimais.")
        if quantidade > _decimal(item["quantidade_disponivel"]):
            raise HTTPException(400, f"Quantidade a devolver ultrapassa o restante de {item['produto_nome']}.")
        ...
        if valor < 0:
            raise HTTPException(400, "O valor das devolucoes anteriores precisa ser conferido.")
        preparados.append({
            "produto_id": produto_id,
            "produto_nome": item["produto_nome"],
            "quantidade": float(quantidade),
            "valor_total": valor,
            "custo_unitario": _decimal(item["valor_total"]) / _decimal(item["quantidade"]),
        })
    if not preparados:
        raise HTTPException(400, "Informe a quantidade de pelo menos um produto a devolver.")
