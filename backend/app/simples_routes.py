...
def buscar_fechamento(
    ...
    if not registro:
        ...
        faturamento_sistema = (
            float(periodo_dre.receita_bruta)
            if periodo_dre and periodo_dre.receita_bruta
            else 0.0
        )
        imposto_estimado = (
            float(periodo_dre.impostos) if periodo_dre and periodo_dre.impostos else 0.0
        )
        ...
        if categoria_das:
            das_pago = (
                db.query(
                    func.coalesce(func.sum(ContaPagar.valor_original), Decimal("0"))
                )
                .filter(
    ...
    aliquota_efetiva_pct = (
        float(registro.aliquota_efetiva * 100) if registro.aliquota_efetiva else None
    )
    aliquota_sugerida_pct = (
        float(registro.aliquota_sugerida * 100) if registro.aliquota_sugerida else None
    )
    ...
    if payload.imposto_real is not None:
        ...
        if not registro:
            registro = SimplesNacionalMensal(
                tenant_id=tenant_id, mes=payload.mes, ano=payload.ano
            )
            db.add(registro)
...
def reabrir_mes(
    ...
    resultado = reabrir_simples_mensal(db=db, tenant_id=tenant_id, mes=mes, ano=ano)

    if not resultado["sucesso"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=resultado["motivo"]
        )
