...
# ============================================================================
# POPULAR FORNECEDORES: vincula produtos das notas ao fornecedor da nota
# ============================================================================

@router.post("/{nota_id}/popular-fornecedores")
def popular_fornecedores_nota(
    nota_id: int,
    db: Session = Depends(get_session),
    user_and_tenant = Depends(get_current_user_and_tenant)
):
    """
    Percorre todos os itens vinculados da nota e cria/atualiza o vínculo
    produto-fornecedor na tabela produto_fornecedores.
    Útil para notas já processadas que não tiveram o fornecedor populado.
    """
    ...
    if not nota.fornecedor_id:
        raise HTTPException(status_code=422, detail="Esta nota não tem fornecedor vinculado")
    ...
    for item in nota.itens:
        ...
        vinculo = db.query(ProdutoFornecedor).filter(
            ProdutoFornecedor.produto_id == item.produto_id,
            ProdutoFornecedor.fornecedor_id == nota.fornecedor_id
        ).first()
        ...
        if not vinculo:
            ...
            db.add(novo)
            vinculados += 1
            logger.info(f"✅ Produto {item.produto_id} → fornecedor {nota.fornecedor_id}")
        else:
            vinculo.preco_custo = item.valor_unitario
            vinculo.ativo = True
            atualizados += 1
    ...
    return {
        "message": "Fornecedores populados com sucesso",
        "fornecedor_id": nota.fornecedor_id,
