        ...
        movimentacoes.append(
            {
                ...
                "data": conta.data_vencimento.isoformat()
...
def obter_saldo_atual(
    ...
    contas = (
        ...
        .filter(ContaBancaria.user_id == current_user.id, ContaBancaria.ativa.is_(True))
        .all()
    )
...
def analisar_gastos(
    ...
    gastos_periodo = (
        db.query(func.sum(ContaPagar.valor_original))
        .filter(
            and_(
                ContaPagar.user_id == current_user.id,
                ContaPagar.data_vencimento >= data_ini,
    ...
    return [
        {
            ...
            "descricao": c.descricao,
            "valor": float(c.valor_original),
            "proxima_data": c.proxima_recorrencia.isoformat()
            if c.proxima_recorrencia
            else None,
            "tipo_recorrencia": c.tipo_recorrencia,
            "dias_ate": (c.proxima_recorrencia - hoje).days
            if c.proxima_recorrencia
            else 0,
        }
...
def projecao_saldo(
    ...
    # Média de entradas
    total_entradas = (
        db.query(func.sum(ContaReceber.valor_original))
        .filter(
            and_(
                ContaReceber.user_id == current_user.id,
    ...
    return {
        ...
        "diasAteNegativo": dias_ate_negativo
        if dias_ate_negativo > 0 and dias_ate_negativo <= dias
        else 0,
        "projecao_dias": dias,
    }
