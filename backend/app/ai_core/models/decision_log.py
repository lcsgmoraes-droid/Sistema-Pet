...
class FeedbackLog(BaseTenantModel):
    ...
    # Decisão original
    decision_id = Column(
        Integer, ForeignKey("ai_decision_logs.id"), nullable=False, unique=True
    )
...
class ReviewQueueModel(BaseTenantModel):
    ...
    decision_log_id = Column(
        Integer, ForeignKey("ai_decision_logs.id"), nullable=False, index=True
    )
...
class LearningPatternModel(BaseTenantModel):
    ...
    )  # categoria_por_descricao, produto_por_contexto
