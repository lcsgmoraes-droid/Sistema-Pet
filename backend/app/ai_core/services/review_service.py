...
class ReviewService:
    ...
    def get_pending_reviews(
        ...
        results = query.order_by(ReviewQueueModel.created_at.asc()).limit(limit).all()

        # Ordenar por prioridade manualmente (SQLAlchemy não tem CASE fácil)
        results = sorted(
            results, key=lambda x: (priority_order.get(x.priority, 999), x.created_at)
        )

        return [self._model_to_domain(r) for r in results]

    def submit_review(self, feedback: HumanReviewFeedback) -> DecisionReviewedEvent:
        """
        ...
        queue_entry = (
            ...
            .filter(ReviewQueueModel.request_id == feedback.request_id)
            .first()
        )
        ...
        decision_log = (
            ...
            .filter(DecisionLog.id == queue_entry.decision_log_id)
            .first()
        )
    ...
    def get_review_by_request_id(self, request_id: str) -> Optional[ReviewQueueEntry]:
        """
        Busca uma entrada específica na fila por request_id.
        """
        queue_entry = (
            self.db.query(ReviewQueueModel)
            .filter(ReviewQueueModel.request_id == request_id)
            .first()
        )
    ...
    def get_review_stats(self, tenant_id: int) -> Dict[str, Any]:
        ...
        for status in DecisionReviewStatus:
            count = (
                self.db.query(func.count(ReviewQueueModel.id))
                .filter(
                    and_(
                        ReviewQueueModel.tenant_id == tenant_id,
        ...
        if reviewed:
            times = [
                (r.reviewed_at - r.created_at).total_seconds() / 60 for r in reviewed
            ]
    ...
    def _calculate_priority(
        self, confidence_score: int, confidence_level: str
    ) -> ReviewPriority:
        """
