...
class TrustService:
    ...
    def generate_trust_report(
        ...
        metrics = self.metrics_service.get_metrics(
            tenant_id=tenant_id, decision_type=decision_type, period=period
        )
        ...
        suggested_review_threshold = self._suggest_review_threshold(
            metrics, maturity_level
        )
    ...
    def _calculate_trust_score(
        self, metrics: AIPerformanceMetrics, maturity: AIMaturityLevel
    ) -> int:
        """
        ...
        gap_normalized = max(
            0, 100 - metrics.confidence_accuracy_gap * 5
        )  # Gap de 20 = 0 pontos
    ...
    def _identify_strengths(self, metrics: AIPerformanceMetrics) -> List[str]:
        ...
        if metrics.automation_rate >= 70:
            strengths.append(
                f"Alto nível de automação ({metrics.automation_rate:.1f}%)"
            )

        if metrics.confidence_accuracy_gap < 10:
            strengths.append(
                f"Bem calibrada (gap de confiança: {metrics.confidence_accuracy_gap:.1f})"
            )
        ...
        if metrics.avg_processing_time_ms < 100:
            strengths.append(
                f"Processamento rápido ({metrics.avg_processing_time_ms:.1f}ms)"
            )
    ...
    def _identify_weaknesses(self, metrics: AIPerformanceMetrics) -> List[str]:
        ...
        if metrics.confidence_accuracy_gap > 20:
            weaknesses.append(
                f"Mal calibrada (gap: {metrics.confidence_accuracy_gap:.1f})"
            )

        if metrics.review_pressure > 60:
            weaknesses.append(
                f"Alta pressão de revisão ({metrics.review_pressure:.1f}%)"
            )
    ...
    def _identify_risks(self, metrics: AIPerformanceMetrics) -> List[str]:
        ...
        if metrics.rejection_rate > 20:
            risks.append(
                "ALTO: Taxa de rejeição muito alta - IA fazendo sugestões ruins"
            )
    ...
    def _generate_recommendations(
        ...
        if maturity == AIMaturityLevel.LEARNING:
            recommendations.append(
                "Manter revisão humana obrigatória para todas as decisões"
            )
            ...
            recommendations.append(
                "Pode executar automaticamente decisões com confiança > 85%"
            )
            ...
            recommendations.append(
                "Considerar aumentar threshold de automação para 80%"
            )
        ...
        if metrics.confidence_accuracy_gap > 15:
            recommendations.append(
                f"Recalibrar modelo - gap de {metrics.confidence_accuracy_gap:.1f} é alto"
            )
        ...
        if metrics.rejection_rate > 15:
            recommendations.append(
                "Revisar lógica de engines - muitas sugestões inadequadas"
            )
    ...
    def _create_no_data_report(
        self, tenant_id: int, decision_type: Optional[str], period: MetricPeriod
    ) -> AITrustReport:
