...
from app.ai_core.domain.guardrails import AIGuardrails, GuardrailSeverity
...
class SafetyService:
    ...
    def evaluate_safety(
        self, tenant_id: int, decision_type: Optional[str] = None, period: str = "daily"
    ) -> List[AIAlertEvent]:
        """
        ...
        violations = self.guardrails.evaluate_all(
            metrics=metrics_dict, previous_trust_score=previous_trust_score
        )

        if not violations:
            # Tudo OK, registrar sucesso no circuit breaker
            circuit = AICircuitBreaker(
                db=self.db, tenant_id=tenant_id, decision_type=decision_type
            )
    ...
    def _create_alert(
        ...
        if guardrail_type == "approval_rate_min":
            ...
            raise ValueError(f"Unsupported guardrail type: {guardrail_type}")
        ...
        if severity == GuardrailSeverity.WARNING:
            ...
            threshold_value = (
                threshold.emergency_threshold or threshold.critical_threshold
            )
    ...
    def resolve_alert(
        self, alert_id: int, resolved_by: int, resolution_notes: str
    ) -> None:
        """
        Marca alerta como resolvido (ação administrativa).
        """
        alert = (
            self.db.query(AIGuardrailViolationLog)
            .filter(AIGuardrailViolationLog.id == alert_id)
            .first()
        )
