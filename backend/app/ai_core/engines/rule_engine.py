...
class RuleEngine(DecisionEngine):
    ...
    async def decide(
        self, context: DecisionContext, user_patterns: Optional[List] = None
    ) -> DecisionResult:
        """
        ...
        tipo_lancamento = context.primary_data.get(
        ...
        reasons.append(
            f"Palavra-chave '{matched_keyword}' indica categoria '{matched_rule['categoria_nome']}'"
        )
        ...
        if tipo_lancamento == matched_rule["tipo_esperado"]:
            confidence += 10.0
            reasons.append(
                f"Tipo de lançamento '{tipo_lancamento}' consistente com a categoria"
            )
        ...
        if user_patterns:
            for pattern in user_patterns:
                ...
                pattern_categoria = pattern.get("output_preference", {}).get(
                    "categoria_nome"
                )
                if pattern_categoria == matched_rule["categoria_nome"]:
                    ...
                    reasons.append(
                        f"Padrão aprendido: categoria aplicada {occurrences}x anteriormente"
                    )
                    evidence.append(
                        Evidence(
                            source="user_pattern",
    ...
    def _find_alternatives(
        self, descricao: str, matched_rule: Dict
    ) -> List[Alternative]:
