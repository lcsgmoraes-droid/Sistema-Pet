...
class AIGovernanceExample:
    ...
    async def analise_completa_tenant(self, tenant_id: int) -> Dict[str, Any]:
        ...
        trends = self.metrics_service.get_trends(
            tenant_id=tenant_id, decision_type=None
        )
        ...
        trust_report = self.trust_service.generate_trust_report(
            tenant_id=tenant_id, period=MetricPeriod.MONTHLY
        )
    ...
    async def monitorar_maturidade(self, tenant_id: int):
        ...
        for period, label in periods:
            ...
            logger.info(
                f"  Pode aumentar automação: {'✅ SIM' if trust_report.can_increase_automation else '❌ NÃO'}"
            )
            print()
    ...
    async def recomendar_ajustes(self, tenant_id: int):
        ...
        logger.info(
            f"  • Confiança mínima para automação: {trust_report.suggested_min_confidence}%"
        )
        logger.info(
            f"  • Threshold de revisão obrigatória: {trust_report.suggested_review_threshold}%"
        )
        logger.info(
            f"  • Aumentar automação: {'✅ Recomendado' if trust_report.can_increase_automation else '❌ Não recomendado'}"
        )
        print()
    ...
    def _print_metrics_summary(self, metrics):
        ...
        logger.info(
            f"  Decisões revisadas: {metrics.decisions_reviewed} ({metrics.review_pressure:.1f}%)"
        )
        logger.info(
            f"  Automatizadas: {metrics.decisions_auto_executed} ({metrics.automation_rate:.1f}%)"
        )
        print()
        ...
        logger.info(f"     Confiança média: {metrics.avg_confidence_all:.1f}")
        logger.info(
            f"     Gap confiança-acurácia: {metrics.confidence_accuracy_gap:.1f}"
        )
        print()
...
async def exemplo_politica_adaptativa():
    ...
    adaptive_policy = DecisionPolicy(
        adaptive_mode=True, metrics_service=metrics_service, trust_service=trust_service
    )
