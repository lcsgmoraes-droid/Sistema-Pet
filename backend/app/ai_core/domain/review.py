...
class ReviewQueueEntry(BaseModel):
    ...
    ai_decision: Dict[str, Any] = Field(..., description="Decisão sugerida pela IA")
    ai_explanation: str = Field(..., description="Explicação da IA")
...
class HumanReviewFeedback(BaseModel):
    ...
    corrected_decision: Optional[Dict[str, Any]] = Field(
        None, description="Decisão correta (substitui a decisão da IA)"
    )

    # Comentário explicativo (opcional)
    comment: Optional[str] = Field(
        None, max_length=1000, description="Comentário do revisor sobre a decisão"
    )
