...
class DecisionReviewedEvent(BaseModel):
    ...
    decision_type: str = Field(
        ..., description="Tipo de decisão (ex: categorizar_lancamento)"
    )
    ...
    reviewer_name: Optional[str] = Field(
        None, description="Nome do revisor (para auditoria)"
    )

    # Ação tomada
    action_taken: str = Field(..., description="approved | corrected | rejected")

    # Dados originais da IA
    original_decision: Dict[str, Any] = Field(..., description="Decisão original da IA")
    confidence_score_original: int = Field(
        ..., ge=0, le=100, description="Confiança original da IA"
    )

    # Dados corrigidos (se action=corrected)
    corrected_data: Optional[Dict[str, Any]] = Field(
        None, description="Decisão correta fornecida pelo humano"
    )

    # Comentário
    comment: Optional[str] = Field(None, description="Comentário do revisor")

    # Contexto adicional
    processing_context: Optional[Dict[str, Any]] = Field(
        None, description="Contexto adicional para o LearningService"
    )
...
class DecisionAppliedEvent(BaseModel):
    ...
    review_event_id: Optional[str] = Field(
        None, description="ID do DecisionReviewedEvent relacionado"
    )
    ...
    # Aplicação
    applied_by: int = Field(
        ..., description="Quem aplicou (pode ser sistema ou humano)"
    )
...
class AIAlertEvent(BaseModel):
    ...
    # Severidade
    severity: str = Field(..., description="warning | critical | emergency")

    # Contexto
    tenant_id: int = Field(..., description="Tenant afetado")
    decision_type: Optional[str] = Field(
        None, description="Tipo de decisão (None = todos os tipos)"
    )

    # Violação
    guardrail_type: str = Field(..., description="Tipo de guardrail violado")
    current_value: float = Field(..., description="Valor atual da métrica")
    threshold_violated: float = Field(..., description="Limite que foi violado")
    ...
    previous_trust_score: Optional[float] = Field(
        None, description="Trust score anterior (para alertas de queda)"
    )

    # Circuit breaker
    circuit_breaker_triggered: bool = Field(
        default=False, description="Se circuit breaker foi acionado"
    )
