...
class Evidence(BaseModel):
    ...
    source: str = Field(
        ..., description="Fonte (ex: 'historico_usuario', 'regra_padrao')"
    )
...
class DecisionResult(BaseModel):
    ...
    confidence_score: int = Field(
        ..., ge=0, le=100, description="Score de confiança (0-100) - ÚNICO no sistema"
    )
    ...
    engine_used: str = Field(
        ..., description="Motor usado (rule_engine, llm_engine, etc)"
    )
    processing_time_ms: float = Field(..., description="Tempo de processamento")

    # Ações sugeridas
    suggested_actions: Optional[List[str]] = Field(
        None, description="Ações que o sistema pode tomar (se confiança permitir)"
    )
    requires_human_review: bool = Field(
        default=False, description="Se True, deve passar por validação humana"
    )

    # Debugging
    debug_info: Optional[Dict[str, Any]] = Field(
        None, description="Informações técnicas (apenas em dev/debug)"
    )
