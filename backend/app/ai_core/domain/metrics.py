...
class AIPerformanceMetrics(BaseModel):
    ...
    decision_type: Optional[str] = Field(
        None, description="Tipo de decisão (ou None=todos)"
    )
    ...
    decisions_auto_executed: int = Field(
        0, description="Decisões executadas automaticamente"
    )
    ...
    avg_confidence_approved: float = Field(
        0.0, description="Confiança média das aprovadas"
    )
    avg_confidence_corrected: float = Field(
        0.0, description="Confiança média das corrigidas"
    )

    # Tempo de processamento
    avg_processing_time_ms: float = Field(
        0.0, description="Tempo médio de processamento"
    )
    avg_review_time_minutes: float = Field(
        0.0, description="Tempo médio de revisão humana"
    )
...
class AITrustReport(BaseModel):
    ...
    trust_score: int = Field(
        ..., ge=0, le=100, description="Score de confiança geral (0-100)"
    )
    ...
    recommendations: List[str] = Field(
        default_factory=list, description="Recomendações para melhorar"
    )

    suggested_min_confidence: int = Field(
        ..., ge=0, le=100, description="Confiança mínima sugerida para automação"
    )

    suggested_review_threshold: int = Field(
        ..., ge=0, le=100, description="Threshold sugerido para revisão obrigatória"
    )
