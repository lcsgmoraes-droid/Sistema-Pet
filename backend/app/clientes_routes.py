...
def update_cliente(
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        f"[DEBUG update_cliente] ID={cliente_id}, tenant_id={tenant_id}, data_fechamento_comissao={cliente_data.data_fechamento_comissao}"
    )
    logger.info(
        f"[DEBUG] entregador_padrao={cliente_data.entregador_padrao}, gera_conta_pagar_custo_entrega={cliente_data.gera_conta_pagar_custo_entrega}"
    )
...
async def get_vendas_em_aberto(
    ...
    logger.info(
        f"ðŸ” DEBUG vendas-em-aberto: cliente_id={cliente_id}, user_id={current_user.id}"
    )
...
def obter_timeline_cliente(
    ...
    pet_id: Optional[int] = Query(
        None, description="Filtrar eventos de um pet especÃ­fico"
    ),
...
def obter_timeline_fornecedor(
    ...
    if not fornecedor:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Fornecedor nÃ£o encontrado"
        )
