...
class StoneTransaction(BaseTenantModel):
    ...
    # IDs de referência
    stone_payment_id = Column(
        String(100), unique=True, index=True, nullable=False
    )  # ID retornado pela Stone
    external_id = Column(
        String(100), unique=True, index=True, nullable=False
    )  # ID do nosso sistema
    ...
    )  # pending, approved, cancelled, refunded, failed
    stone_status = Column(
        String(50)
    )  # Status original da Stone (pode ter mais detalhes)
    ...
    conta_receber = relationship(
        "ContaReceber", foreign_keys=[conta_receber_id], backref="stone_transactions"
    )
...
class StoneTransactionLog(BaseTenantModel):
    ...
    transaction_id = Column(
        Integer, ForeignKey("stone_transactions.id"), nullable=False, index=True
    )

    # Tipo de evento
    event_type = Column(
        String(50), nullable=False
    )  # webhook, status_change, api_call, error
...
class StoneConfig(BaseTenantModel):
    ...
    # ── Stone Conciliação ──────────────────────────────────────────────────────
    # Client ID e Secret fornecidos pela Stone no programa API Conciliação
    conciliacao_client_id = Column(String(200), nullable=True)
    conciliacao_client_secret = Column(String(200), nullable=True)
    # Stone Code (código de afiliação) do estabelecimento
    affiliation_code = Column(String(100), nullable=True)
    # CNPJ do estabelecimento (usado no pedido de consentimento)
    documento = Column(String(20), nullable=True)
    # Credenciais recebidas quando o consentimento é aceito (via webhook)
    conciliacao_username = Column(String(200), nullable=True)
    # Senha CRIPTOGRAFADA (AES-256-CBC) — nunca armazenar em texto plano
    conciliacao_password_enc = Column(String(500), nullable=True)
    ...
    def to_dict(self):
        ...
        return {
            ...
            "active": self.active,
            # Conciliação (não-sensíveis)
            "affiliation_code": self.affiliation_code,
            "documento": self.documento,
            "conciliacao_username": self.conciliacao_username,
            "conciliacao_configurado": bool(
                self.conciliacao_username and self.conciliacao_password_enc
            ),
            # POS
