            ...
            if prod1.porte_animal_id and prod2.porte_animal_id:
                ...
                if prod1.porte_animal_id != prod2.porte_animal_id:
                    porte1 = (
                        ...
                        .filter(PorteAnimal.id == prod1.porte_animal_id)
                        .first()
                    )
                    porte2 = (
                        db.query(PorteAnimal)
                        .filter(PorteAnimal.id == prod2.porte_animal_id)
                        .first()
                    )
                    if porte1 and porte2:
                        ...
                        if porte1.nome != "Todos" and porte2.nome != "Todos":
                            ...
                            if (
                                porte1.nome in portes_ordem
                                and porte2.nome in portes_ordem
                            ):
                                diferenca = abs(
                                    portes_ordem[porte1.nome]
                                    - portes_ordem[porte2.nome]
                                )
                                if diferenca > 1:
                ...
                duplicatas.append(
                    DuplicataDetectada(
                        produto_1={
                            ...
                            "estoque": float(prod1.estoque_atual)
                            if prod1.estoque_atual
                            else 0,
                        },
                        produto_2={
                            "id": prod2.id,
                            ...
                            "estoque": float(prod2.estoque_atual)
                            if prod2.estoque_atual
                            else 0,
                        },
...
async def ignorar_duplicata(
    ...
    prod1 = (
        db.query(Produto)
        .filter(Produto.id == produto_id_1, Produto.tenant_id == tenant_id)
        .first()
    )

    prod2 = (
        db.query(Produto)
        .filter(Produto.id == produto_id_2, Produto.tenant_id == tenant_id)
        .first()
    )
...
async def mesclar_produtos(
    ...
    prod_manter = (
        db.query(Produto)
        .filter(Produto.id == produto_id_manter, Produto.tenant_id == tenant_id)
        .first()
    )

    prod_remover = (
        db.query(Produto)
        .filter(Produto.id == produto_id_remover, Produto.tenant_id == tenant_id)
        .first()
    )
    ...
    for produto in produtos:
        ...
        if produto.fase_publico_id:
            fase = (
                ...
                .filter(FasePublico.id == produto.fase_publico_id)
                .first()
            )
        ...
        if produto.porte_animal_id:
            porte = (
                db.query(PorteAnimal)
                .filter(PorteAnimal.id == produto.porte_animal_id)
                .first()
            )
        ...
        if (
            len(partes_nome) >= 3
            and nome_sugerido.lower() != nome_atual.lower()
            and confianca >= 0.5
        ):
