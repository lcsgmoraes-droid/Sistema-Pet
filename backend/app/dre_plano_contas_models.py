...
class DRECategoria(BaseTenantModel):
    ...
    natureza = Column(
        Enum(NaturezaDRE, values_callable=lambda x: [e.value for e in x]),
        nullable=False,
    )
    ativo = Column(Boolean, default=True)

    subcategorias = relationship(
        "DRESubcategoria", back_populates="categoria", cascade="all, delete-orphan"
    )
...
class DRESubcategoria(BaseTenantModel):
    ...
    categoria_id = Column(Integer, ForeignKey("dre_categorias.id"), nullable=False)
    ...
    escopo_rateio = Column(Enum(EscopoRateio, name="escoporateio"), nullable=False)

    ativo = Column(Boolean, default=True)
    custo_pe = Column(
        String(10), nullable=True
    )  # 'fixo' | 'variavel' | null (para Ponto de Equilíbrio)
    categoria_financeira_id = Column(
        Integer,
        ForeignKey("categorias_financeiras.id", ondelete="SET NULL"),
        nullable=True,
    )
