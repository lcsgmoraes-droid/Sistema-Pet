...
class SugestoesRequest(BaseModel):
    ...
    cliente_id: Optional[int] = Field(
        None, gt=0, description="ID do cliente identificado"
    )
    cliente_nome: Optional[str] = Field(None, max_length=200)
    itens: List[ItemVendaRequest] = Field(
        default_factory=list, description="Produtos já adicionados"
    )
    ...
    contexto_extra: Optional[Dict[str, Any]] = Field(
        None, description="Contexto adicional"
    )
...
class SugestaoResponse(BaseModel):
    ...
    explicacao: Optional[str] = Field(
        None, description="Explicação detalhada (opcional)"
    )
    origem: str = Field("insight", description="Origem: insight ou ai")
    confianca: float = Field(
        ..., ge=0.0, le=1.0, description="Nível de confiança (0-1)"
    )
...
class SugestoesResponse(BaseModel):
    ...
    processamento_ms: Optional[int] = Field(
        None, description="Tempo de processamento em ms"
    )
...
def criar_pdv_context(request: SugestoesRequest, tenant_id: int) -> PDVContext:
    """
    Cria PDVContext a partir do request.
    """
    itens_pdv = [converter_item_request_para_pdv(item) for item in request.itens]
...
def criar_pdv_context_preview(request: PreviewRequest, tenant_id: int) -> PDVContext:
    """
...
async def gerar_sugestoes_pdv(
    ...
    try:
        ...
        sugestoes_response = [converter_sugestao_para_response(s) for s in sugestoes]
        ...
        logger.info(
            f"[PDV AI] Geradas {len(sugestoes_response)} sugestões em {tempo_ms}ms"
        )
        ...
        logger.error(f"[PDV AI] Erro ao gerar sugestões: {str(e)}", exc_info=True)
...
async def preview_sugestoes_pdv(
    ...
    try:
        logger.info(
            f"[PDV AI Preview] Simulação de sugestões (itens={len(request.itens)})"
        )
        ...
        logger.error(f"[PDV AI Preview] Erro ao gerar preview: {str(e)}", exc_info=True)
