...
def _normalizar_insumos(insumos: Optional[object]) -> list[dict]:
    ...
    for item in insumos:
        ...
        custo_total = _round_money(
            item.get("custo_total") or (custo_unitario * quantidade)
        )
        normalizados.append(
            {
                **item,
...
def coletar_produto_ids_extrato(
    *,
    ...
    ids: set[int] = set()
    for procedimento in list(procedimentos_consulta or []) + list(
        procedimentos_internacao or []
    ):
...
def _linhas_procedimento_consulta(
    procedimento, produtos_por_id: dict[int, object]
) -> list[dict]:
        ...
        linhas.append(
            _linha_extrato(
                ...
                nome=_normalizar_texto(
                    _get(procedimento, "nome"), "Procedimento da consulta"
                ),
                quantidade=1,
                unidade="servico",
                ...
                estoque_movimentacao_ids=_get(procedimento, "estoque_movimentacao_ids")
                or [],
    ...
    for insumo in insumos:
        ...
        linhas.append(
            _linha_extrato(
                ...
                nome=_normalizar_texto(
                    insumo.get("nome") or produto.get("nome"),
                    f"Produto {insumo['produto_id']}",
                ),
                quantidade=quantidade,
                ...
                estoque_movimentacao_ids=_get(procedimento, "estoque_movimentacao_ids")
                or [],
                observacoes=insumo.get("observacoes")
                or _get(procedimento, "observacoes"),
            )
        )
    return linhas


def _linhas_procedimento_internacao(
    procedimento: dict, produtos_por_id: dict[int, object]
) -> list[dict]:
    ...
    data_hora = (
        procedimento.get("horario_execucao")
        or procedimento.get("data_hora")
        or procedimento.get("horario_agendado")
    )
    nome = (
        procedimento.get("medicamento")
        or procedimento.get("nome")
        or "Procedimento da internacao"
    )
    linhas: list[dict] = []
    ...
    if tem_valor_cobrado or not insumos:
        linhas.append(
            _linha_extrato(
                ...
                quantidade=procedimento.get("quantidade_executada")
                or procedimento.get("quantidade_prevista")
                or 1,
                ...
                estoque_movimentacao_ids=procedimento.get("estoque_movimentacao_ids")
                or [],
                observacoes=procedimento.get("observacao_execucao")
                or procedimento.get("observacoes_agenda"),
            )
        )
    ...
    for insumo in insumos:
        ...
        linhas.append(
            _linha_extrato(
                ...
                or [],
                observacoes=insumo.get("observacoes")
                or procedimento.get("observacao_execucao"),
            )
        )
    return linhas
...
def _calcular_totais(linhas: list[dict]) -> dict:
    ...
    custo_total = _round_money(
        sum(_as_float(linha.get("custo_total")) or 0 for linha in contabilizadas)
    )
    preco_total = _round_money(
        sum(_as_float(linha.get("preco_total")) or 0 for linha in contabilizadas)
    )
    ...
    *,
    ...
    return {
        "contexto": {
            ...
            "pet_id": _get(consulta, "pet_id")
            if consulta is not None
            else (_get(internacoes_lista[0], "pet_id") if internacoes_lista else None),
...
def gerar_excel_extrato_bytes(extrato: dict, colunas: Optional[object] = None) -> bytes:
    ...
    for indice, coluna in enumerate(colunas_exportacao, start=1):
        ws.column_dimensions[get_column_letter(indice)].width = EXTRATO_COLUNAS_META[
            coluna
        ]["excel_width"]
...
def gerar_pdf_extrato_bytes(extrato: dict, colunas: Optional[object] = None) -> bytes:
    ...
    texto_pequeno = ParagraphStyle(
        "ExtratoVetSmall", parent=styles["Normal"], fontSize=7, leading=9
    )
    ...
    table_data = [
        [EXTRATO_COLUNAS_META[coluna]["titulo"] for coluna in colunas_exportacao]
    ]
    for linha in extrato.get("linhas") or []:
        row = []
    ...
    col_widths = [
        EXTRATO_COLUNAS_META[coluna]["pdf_width_mm"] * mm
        for coluna in colunas_exportacao
    ]
