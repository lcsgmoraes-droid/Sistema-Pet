...
def _verificar_duplicata(
    db: Session, model, tenant_id, nome: str, id_excluir: Optional[int] = None
):
...
async def criar_linha_racao(
    ...
    if _verificar_duplicata(db, LinhaRacao, tenant_id, dados.nome):
        raise HTTPException(
            status_code=400, detail=f"Já existe uma linha com o nome '{dados.nome}'"
        )
...
async def atualizar_linha_racao(
    ...
    linha = (
        db.query(LinhaRacao)
        .filter(LinhaRacao.id == linha_id, LinhaRacao.tenant_id == tenant_id)
        .first()
    )
    ...
    if _verificar_duplicata(db, LinhaRacao, tenant_id, dados.nome, linha_id):
        raise HTTPException(
            status_code=400, detail=f"Já existe outra linha com o nome '{dados.nome}'"
        )
...
async def criar_porte(
    ...
    if _verificar_duplicata(db, PorteAnimal, tenant_id, dados.nome):
        raise HTTPException(
            status_code=400, detail=f"Já existe um porte com o nome '{dados.nome}'"
        )
...
async def atualizar_porte(
    ...
    porte = (
        ...
        .filter(PorteAnimal.id == porte_id, PorteAnimal.tenant_id == tenant_id)
        .first()
    )
    ...
    if _verificar_duplicata(db, PorteAnimal, tenant_id, dados.nome, porte_id):
        raise HTTPException(
            status_code=400, detail=f"Já existe outro porte com o nome '{dados.nome}'"
        )
...
async def criar_fase(
    ...
    if _verificar_duplicata(db, FasePublico, tenant_id, dados.nome):
        raise HTTPException(
            status_code=400, detail=f"Já existe uma fase com o nome '{dados.nome}'"
        )
...
async def atualizar_fase(
    ...
    fase = (
        ...
        .filter(FasePublico.id == fase_id, FasePublico.tenant_id == tenant_id)
        .first()
    )
    ...
    if _verificar_duplicata(db, FasePublico, tenant_id, dados.nome, fase_id):
        raise HTTPException(
            status_code=400, detail=f"Já existe outra fase com o nome '{dados.nome}'"
        )
...
async def criar_tratamento(
    ...
    if _verificar_duplicata(db, TipoTratamento, tenant_id, dados.nome):
        raise HTTPException(
            status_code=400, detail=f"Já existe um tratamento com o nome '{dados.nome}'"
        )
...
async def atualizar_tratamento(
    ...
    tratamento = (
        ...
        .filter(
            TipoTratamento.id == tratamento_id, TipoTratamento.tenant_id == tenant_id
        )
        .first()
    )
    ...
    if _verificar_duplicata(db, TipoTratamento, tenant_id, dados.nome, tratamento_id):
        raise HTTPException(
            status_code=400,
            detail=f"Já existe outro tratamento com o nome '{dados.nome}'",
        )
...
async def criar_sabor(
    ...
    if _verificar_duplicata(db, SaborProteina, tenant_id, dados.nome):
        raise HTTPException(
            status_code=400, detail=f"Já existe um sabor com o nome '{dados.nome}'"
        )
...
async def atualizar_sabor(
    ...
    sabor = (
        ...
        .filter(SaborProteina.id == sabor_id, SaborProteina.tenant_id == tenant_id)
        .first()
    )
    ...
    if _verificar_duplicata(db, SaborProteina, tenant_id, dados.nome, sabor_id):
        raise HTTPException(
            status_code=400, detail=f"Já existe outro sabor com o nome '{dados.nome}'"
        )
...
async def criar_apresentacao(
    ...
    if existe:
        raise HTTPException(
            status_code=400, detail=f"Já existe uma apresentação com {dados.peso_kg}kg"
        )
...
async def atualizar_apresentacao(
    ...
    if existe:
        raise HTTPException(
            status_code=400,
            detail=f"Já existe outra apresentação com {dados.peso_kg}kg",
        )
