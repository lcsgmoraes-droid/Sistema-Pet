...
def atualizar_config_fiscal(
    ...
    logger.info(
        f"🔍 Dados recebidos para atualização: {dados.model_dump(exclude_unset=True)}"
    )
    ...
    logger.info(
        f"💰 Alíquota vigente após atualização: {config.aliquota_simples_vigente}"
    )
    ...
    # Se não é Simples ou simples_ativo=False, zerar campos
        ...
        logger.info(
            f"⚠️ ZERANDO alíquotas - Regime: {config.regime_tributario}, Simples Ativo: {config.simples_ativo}"
        )
    ...
    logger.info(
        f"💾 Salvando no banco - Alíquota final: {config.aliquota_simples_vigente}"
    )
...
def _config_estoque_response(tenant: Tenant) -> ConfigEstoqueResponse:
    return ConfigEstoqueResponse(
        ...
        bloquear_validade_ecommerce=bool(
            getattr(tenant, "bloquear_validade_ecommerce", True)
        ),
