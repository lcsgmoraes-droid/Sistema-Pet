...
def _payload_grupo(db: Session, grupo: FornecedorGrupo, tenant_id) -> dict:
    ...
    if grupo.fornecedor_principal_id:
        principal = next(
            (
                ...
                if fornecedor.id == grupo.fornecedor_principal_id
            ),
            None,
        )
    ...
    return {
        ...
        "fornecedores": [
            _payload_fornecedor(fornecedor) for fornecedor in fornecedores
        ],
...
def _buscar_fornecedores_validos(
    db: Session, fornecedor_ids: List[int], tenant_id
) -> List[Cliente]:
    ids_unicos = sorted(
        {int(fornecedor_id) for fornecedor_id in fornecedor_ids if fornecedor_id}
    )
    if not ids_unicos:
        return []
    ...
    faltantes = [
        fornecedor_id
        for fornecedor_id in ids_unicos
        if fornecedor_id not in encontrados
    ]
    if faltantes:
        raise HTTPException(
            status_code=400,
...
def criar_grupo_fornecedor(
    ...
    _user, tenant_id = user_and_tenant
    fornecedores = _buscar_fornecedores_validos(
        db, payload.fornecedor_ids or [], tenant_id
    )
    fornecedor_ids = {fornecedor.id for fornecedor in fornecedores}

    if (
        payload.fornecedor_principal_id
        and payload.fornecedor_principal_id not in fornecedor_ids
    ):
        raise HTTPException(
            status_code=400,
    ...
    grupo = FornecedorGrupo(
        ...
        fornecedor_principal_id=payload.fornecedor_principal_id
        or (fornecedores[0].id if fornecedores else None),
...
def atualizar_grupo_fornecedor(
    ...
    if "fornecedor_ids" in dados:
        fornecedores = _buscar_fornecedores_validos(
            db, dados.get("fornecedor_ids") or [], tenant_id
        )
        fornecedor_ids = {fornecedor.id for fornecedor in fornecedores}
        principal_id = dados.get(
            "fornecedor_principal_id", grupo.fornecedor_principal_id
        )
