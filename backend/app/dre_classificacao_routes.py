...
class AplicarClassificacaoRequest(BaseModel):
    ...
    canal: Optional[str] = Field(
        None, description="loja_fisica, mercado_livre, shopee, amazon"
    )
    regra_id: Optional[int] = Field(
        None, description="ID da regra que originou (se aplicável)"
    )
    forma_classificacao: str = Field(
        "manual", description="manual, automatico_regra, sugestao_aceita"
    )
...
def sugerir_classificacao(
    ...
    **Exemplo:**
    ...
    if request.tipo not in ["pagar", "receber"]:
        raise HTTPException(
            status_code=400, detail="Tipo deve ser 'pagar' ou 'receber'"
        )

    try:
        classificador = ClassificadorDRE(db, tenant_id)
        sugestoes = classificador.analisar_lancamento(
            tipo=request.tipo, lancamento_id=request.lancamento_id
        )

        logger.info(
            f"📋 {len(sugestoes)} sugestões encontradas para {request.tipo} #{request.lancamento_id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao analisar lançamento: {str(e)}"
        )
    ...
    **Exemplo:**
    ...
    try:
        ...
        if sucesso:
            ...
            raise HTTPException(
                status_code=400, detail="Não foi possível aplicar a classificação"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao aplicar classificação: {str(e)}"
        )
    ...
    **Exemplo:**
    ...
    for item in request.classificacoes:
        try:
            ...
            logger.error(
                f"❌ Erro ao aplicar classificação lote #{item.lancamento_id}: {e}"
            )
            resultados.append(
                ResultadoAplicacao(
                    sucesso=False,
    ...
    logger.info(
        f"📦 Lote aplicado: {sum(1 for r in resultados if r.sucesso)}/{len(resultados)} sucessos"
    )
...
def listar_pendentes(
    tipo: Optional[str] = Query(
        None, description="Filtrar por tipo: 'pagar', 'receber' ou None (ambos)"
    ),
    ...
    **Use Case:**
    ...
    if tipo and tipo not in ["pagar", "receber"]:
        raise HTTPException(
            status_code=400, detail="Tipo deve ser 'pagar', 'receber' ou None"
        )
    ...
    try:
        ...
        logger.info(
            f"📋 Lançamentos pendentes: {pendentes['total_pendentes']} encontrados"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao listar pendentes: {str(e)}"
        )
...
def auto_classificar_pendentes(
    ...
    apenas_alta_confianca: bool = Query(
        True, description="Aplicar apenas sugestões com confiança >= 90%"
    ),
    db: Session = Depends(get_session),
    user_and_tenant=Depends(get_current_user_and_tenant),
):
    ...
    **Retorna:**
    ...
    logger.info(
        f"🤖 Auto-classificação: {total_classificado}/{total_processado} lançamentos"
    )
