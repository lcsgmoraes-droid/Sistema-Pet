...
def _registrar_lote_entrada(
    *,
    ...
    nome_lote = (
        numero_lote or f"{codigo_produto}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    )
...
def entrada_estoque(
    ...
    logger.info(
        f"📥 Entrada de estoque - Produto {entrada.produto_id}, Qtd: {entrada.quantidade}"
    )

    # Buscar produto
    produto = (
        db.query(Produto)
        .filter(Produto.id == entrada.produto_id, Produto.tenant_id == tenant_id)
        .first()
    )
    if not produto:
    ...
    if _produto_e_granel(produto) and entrada.motivo not in {
        "conversao_granel",
        "balanco",
    }:
        raise HTTPException(
    ...
    ultima_entrada = (
        ...
        .order_by(desc(EstoqueMovimentacao.created_at))
        .first()
    )

    custo_anterior = (
        ultima_entrada.custo_unitario if ultima_entrada else produto.preco_custo
    )
    ...
    logger.info(
        f"✅ Entrada registrada - Custo: R$ {entrada.custo_unitario or 0:.2f} (Anterior: R$ {custo_anterior or 0:.2f})"
    )
    ...
    logger.info(
        f"✅ Entrada registrada - Estoque: {estoque_anterior} → {produto.estoque_atual}"
    )
    ...
    if estoque_anterior <= 0 and produto.estoque_atual > 0:
        try:
            ...
            notificar_clientes_estoque_disponivel(
                db, str(tenant_id), produto.id, produto.nome
            )
    ...
    if produto.tipo_produto == "KIT" and produto.tipo_kit == "FISICO":
        ...
        logger.info(
            f"🧩 KIT FÍSICO - ENTRADA: Consumindo {len(componentes)} componentes do kit '{produto.nome}' (montando {entrada.quantidade} kits)"
        )
        ...
        for comp in componentes:
            ...
            if componente_produto:
                ...
                logger.info(
                    f"   ↳ {componente_produto.nome}: {estoque_ant_comp} → {componente_produto.estoque_atual} (-{quantidade_componente}) [consumido para montagem]"
                )

        db.commit()
        logger.info(
            f"✅ KIT FÍSICO: {len(componentes_sensibilizados)} componentes consumidos na montagem"
        )
    ...
    try:
        sincronizar_bling_background(
            produto.id, produto.estoque_atual, "entrada_estoque"
        )
