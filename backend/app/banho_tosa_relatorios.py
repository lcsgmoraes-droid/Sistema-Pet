...
def gerar_relatorio_operacional(
    db: Session, tenant_id, data_inicio: date, data_fim: date
) -> dict:
    ...
    ocupacao = _agregar_ocupacao(
        db, tenant_id, data_inicio, data_fim, agendamentos, recursos
    )
    nps = calcular_nps_periodo(db, tenant_id, inicio, fim)

    resumo = _montar_resumo(
        atendimentos, agendamentos, snapshots, desperdicios, ocupacao, nps
    )
    return {
...
def _agregar_margem_por_servico(atendimentos, snapshots) -> list[dict]:
    ...
    for atendimento in atendimentos:
        ...
        for servico in agendamento.servicos:
            ...
            _somar_grupo(
                grupos[chave], chave, nome, snapshots.get(atendimento.id), proporcao
            )
    ...
    for atendimento in atendimentos:
        porte = (
            (
                ...
                or getattr(atendimento.pet, "porte", None)
                or "nao_informado"
            )
            .strip()
            .lower()
...
def _agregar_produtividade(atendimentos) -> list[dict]:
        ...
        for etapa in atendimento.etapas or []:
            ...
            grupo = grupos.setdefault(
                etapa.responsavel_id,
                {
                    ...
                    "responsavel_nome": etapa.responsavel.nome
    ...
    return sorted(
        (serializar_produtividade(item) for item in grupos.values()),
        key=lambda item: item["minutos_trabalhados"],
        reverse=True,
    )
        ...
        for insumo in atendimento.insumos_usados or []:
            ...
            grupo = grupos.setdefault(
                insumo.produto_id,
                {
                    ...
                    "produto_nome": insumo.produto.nome
                    if insumo.produto
                    else "Produto",
                    "unidade": getattr(insumo.produto, "unidade", None),
                    "quantidade_desperdicio": Decimal("0"),
                    "custo_desperdicio": Decimal("0"),
                },
            )
            grupo["quantidade_desperdicio"] += quantidade
            grupo["custo_desperdicio"] += quantidade * dec(
                insumo.custo_unitario_snapshot
            )
    return sorted(
        grupos.values(), key=lambda item: item["custo_desperdicio"], reverse=True
    )


def _agregar_ocupacao(
    db: Session, tenant_id, data_inicio: date, data_fim: date, agendamentos, recursos
):
    ...
    grupos = {
        recurso.id: grupo_ocupacao(recurso, minutos_dia * dias) for recurso in recursos
    }
    for agendamento in agendamentos:
        if not agendamento.recurso_id or agendamento.recurso_id not in grupos:
            continue
        grupos[agendamento.recurso_id]["minutos_ocupados"] += minutos_agendamento(
            agendamento
        )
...
def _montar_resumo(
    atendimentos, agendamentos, snapshots, desperdicios, ocupacao, nps: dict
) -> dict:
    receita = sum(
        (dec(item.valor_cobrado) for item in snapshots.values()), Decimal("0")
    )
    custo = sum((dec(item.custo_total) for item in snapshots.values()), Decimal("0"))
    margem = receita - custo
    desperdicio_valor = sum(
        (dec(item["custo_desperdicio"]) for item in desperdicios), Decimal("0")
    )
    desperdicio_qtd = sum(
        (dec(item["quantidade_desperdicio"]) for item in desperdicios), Decimal("0")
    )
...
def _somar_grupo(
    grupo, chave: str, nome: str, snapshot, proporcao: Decimal = Decimal("1")
):
    grupo["chave"] = chave
    grupo["nome"] = nome
        ...
        itens.append(
            {
                **grupo,
                "margem_valor": margem,
                "margem_percentual": percentual(margem, grupo["receita"]),
            }
        )
...
def _listar_recursos(db: Session, tenant_id):
    return (
        ...
        .filter(
            BanhoTosaRecurso.tenant_id == tenant_id, BanhoTosaRecurso.ativo.is_(True)
        )
        .all()
    )
