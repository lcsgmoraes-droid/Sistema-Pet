...
def registrar_vacina(
    ...
    if tenant_id is None:
        ...
        if not pet_ref or not pet_ref.tenant_id:
            raise HTTPException(
                status_code=400, detail="Pet inválido para registro de vacina"
            )
        tenant_id = pet_ref.tenant_id
    ...
    if body.consulta_id:
        _bloquear_lancamento_em_consulta_finalizada(
            consulta_ok, "novo registro de vacina vinculado"
        )
    ...
    if user_id is None:
        raise HTTPException(
            status_code=401, detail="Usuário inválido para registrar vacina"
        )
...
def curva_peso(
    ...
    registros = (
        ...
        .order_by(PesoRegistro.data)
        .all()
    )
    return [
        {"data": r.data, "peso_kg": r.peso_kg, "consulta_id": r.consulta_id}
        for r in registros
    ]
...
@router.get(
    "/catalogo/calendario-preventivo", summary="Calendário preventivo por espécie"
)
