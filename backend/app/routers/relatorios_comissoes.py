...
def relatorio_margem_produto(
    ...
    logger.warning("[RELATORIO-MARGEM-PRODUTO] Request received")
    ...
    logger.warning("[RELATORIO-MARGEM-PRODUTO] Query prepared")
    result = execute_tenant_safe(db, query, params_dict, tenant_id=tenant_id)
    resultados = result.fetchall()
    logger.warning(
        f"🔍 [RELATORIO-MARGEM-PRODUTO] Resultados encontrados: {len(resultados)}"
    )
...
def relatorio_produtos_prejudiciais(
    ...
    dados_margem = relatorio_margem_produto(
        data_inicio, data_fim, None, None, db, user_and_tenant
    )
    ...
    for produto in produtos:
        ...
        if produto["percentual_comissao"] > 20:
            alertas.append(
                f"Comissão de {produto['percentual_comissao']:.1f}% sobre venda"
            )
            nivel_gravidade = max(nivel_gravidade, 1)

        if 0 <= produto["percentual_margem"] < 5:
            alertas.append(
                f"Margem líquida de apenas {produto['percentual_margem']:.1f}%"
            )
            nivel_gravidade = max(nivel_gravidade, 1)
            ...
            prejudiciais.append(
                {
                    **produto,
                    ...
                    "impacto_financeiro": abs(produto["margem_liquida"])
                    if produto["margem_liquida"] < 0
                    else (produto["margem_bruta"] - produto["margem_liquida"]),
                }
            )
    ...
    ranking = [
        {
            ...
            "percentual_total": round((float(row[2] or 0) / total_geral * 100), 2)
            if total_geral > 0
            else 0,
        }
        for idx, row in enumerate(resultados)
...
def visao_dre(
        ...
        dados_mensais.append(
            {
                ...
                "margem_bruta": margem_bruta,
                "margem_liquida": margem_liquida,
                "percentual_comissao_sobre_receita": round(
                    (comissao / receita * 100), 2
                )
                if receita > 0
                else 0,
                "percentual_margem_bruta": round((margem_bruta / receita * 100), 2)
                if receita > 0
                else 0,
                "percentual_margem_liquida": round((margem_liquida / receita * 100), 2)
                if receita > 0
                else 0,
    ...
    total_ano["margem_liquida"] = (
        total_ano["receita_bruta"]
        - total_ano["custo_total"]
        - total_ano["despesa_comissao"]
    )
    total_ano["percentual_comissao"] = round(
        (total_ano["despesa_comissao"] / total_ano["receita_bruta"] * 100)
        if total_ano["receita_bruta"] > 0
        else 0,
        2,
    )
...
def exportar_csv(
    tipo: str = Query(
        ...,
        pattern="^(margem-produto|produtos-prejudiciais|ranking-funcionarios|ranking-produtos)$",
    ),
    data_inicio: Optional[date] = None,
    data_fim: Optional[date] = None,
    db: Session = Depends(get_session),
    ...
    if tipo == "margem-produto":
        dados = relatorio_margem_produto(
            data_inicio, data_fim, None, None, db, user_and_tenant
        )
        linhas = dados["dados"]
        campos = [
            ...
            "percentual_comissao",
            "percentual_margem",
            "alerta_margem_negativa",
        ]

    elif tipo == "produtos-prejudiciais":
        dados = relatorio_produtos_prejudiciais(
            data_inicio, data_fim, 20, db, user_and_tenant
        )
        linhas = dados["dados"]
        campos = [
            "produto_nome",
        ...
        dados = ranking_funcionarios(data_inicio, data_fim, 10, db, user_and_tenant)
        ...
        dados = ranking_produtos(data_inicio, data_fim, 10, db, user_and_tenant)
        linhas = dados["ranking"]
        campos = [
            "posicao",
            "produto_nome",
