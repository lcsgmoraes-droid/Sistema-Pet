...
class ExportadorDRE:
    ...
    def exportar_pdf(
        ...
        dre = (
            ...
            .filter(
                DREPeriodo.id == dre_periodo_id, DREPeriodo.usuario_id == usuario_id
            )
            .first()
        )
        ...
        # Título
        elementos.append(
            Paragraph("Demonstração do Resultado do Exercício (DRE)", titulo_style)
        )
        ...
        dados_dre = [
            ...
            [
                "(-) Deduções da Receita",
                f"({dre.deducoes_receita:,.2f})",
                f"({dre.deducoes_receita / dre.receita_bruta * 100:.1f}%)"
                if dre.receita_bruta > 0
                else "0%",
            ],
            [
                "(=) Receita Líquida",
                f"{dre.receita_liquida:,.2f}",
                f"{dre.receita_liquida / dre.receita_bruta * 100:.1f}%"
                if dre.receita_bruta > 0
                else "100%",
            ],
            ["", "", ""],
            ...
            [
                ...
                f"({dre.custo_produtos_vendidos:,.2f})",
                f"({dre.custo_produtos_vendidos / dre.receita_bruta * 100:.1f}%)"
                if dre.receita_bruta > 0
                else "0%",
            ],
            [
                "(=) LUCRO BRUTO",
                f"{dre.lucro_bruto:,.2f}",
                f"{dre.margem_bruta_percent:.1f}%",
            ],
            ["", "", ""],
            ["DESPESAS OPERACIONAIS", "", ""],
            [
                "Despesas com Vendas",
                f"({dre.despesas_vendas:,.2f})",
                f"({dre.despesas_vendas / dre.receita_bruta * 100:.1f}%)"
                if dre.receita_bruta > 0
                else "0%",
            ],
            [
                "Despesas Administrativas",
                f"({dre.despesas_administrativas:,.2f})",
                f"({dre.despesas_administrativas / dre.receita_bruta * 100:.1f}%)"
                if dre.receita_bruta > 0
                else "0%",
            ],
            [
                "Despesas Financeiras",
                f"({dre.despesas_financeiras:,.2f})",
                f"({dre.despesas_financeiras / dre.receita_bruta * 100:.1f}%)"
                if dre.receita_bruta > 0
                else "0%",
            ],
            [
                "Outras Despesas",
                f"({dre.outras_despesas:,.2f})",
                f"({dre.outras_despesas / dre.receita_bruta * 100:.1f}%)"
                if dre.receita_bruta > 0
                else "0%",
            ],
            [
                "(=) Total Despesas Operacionais",
                f"({dre.total_despesas_operacionais:,.2f})",
                f"({dre.total_despesas_operacionais / dre.receita_bruta * 100:.1f}%)"
                if dre.receita_bruta > 0
                else "0%",
            ],
            ["", "", ""],
            [
                "(=) LUCRO OPERACIONAL",
                f"{dre.lucro_operacional:,.2f}",
                f"{dre.margem_operacional_percent:.1f}%",
            ],
        ]
        ...
        if dre.impostos and dre.impostos > 0:
            dados_dre.extend(
                [
                    ...
                    ["IMPOSTOS", "", ""],
                    [
                        f"(-) Impostos ({dre.regime_tributario or ''})",
                        f"({dre.impostos:,.2f})",
                        f"({dre.aliquota_efetiva_percent:.1f}%)",
                    ],
                ]
            )
        ...
        dados_dre.extend(
            [
                ...
                [
                    "(=) LUCRO LÍQUIDO",
                    f"{dre.lucro_liquido:,.2f}",
                    f"{dre.margem_liquida_percent:.1f}%",
                ],
            ]
        )
        ...
        elementos.append(
            Paragraph("<b>Indicadores de Performance</b>", styles["Heading2"])
        )
        ...
        if incluir_produtos:
            ...
            if produtos:
                ...
                elementos.append(
                    Paragraph(
                        "<b>Top 10 Produtos Mais Rentáveis</b>", styles["Heading2"]
                    )
                )
                elementos.append(Spacer(1, 0.1 * inch))
    ...
    def exportar_excel(self, dre_periodo_id: int, usuario_id: int) -> BytesIO:
        """
