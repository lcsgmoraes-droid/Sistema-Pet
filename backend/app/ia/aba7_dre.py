...
class DREService:
    ...
    def _calcular_analise_produtos(
        ...
        for venda in vendas:
            ...
            for item in itens:
                ...
                produtos_stats[item.produto_id]["receita"] += (
                ...
                if produto and produto.preco_custo:
                    produtos_stats[item.produto_id]["custo"] += (
                        produto.preco_custo * item.quantidade
                    )
        ...
        for produto_id, stats in sorted(
            ...
            if margem > 30:
                ...
                recomendacao = (
                    "Margem baixa. Avalie possibilidade de reajuste de preço."
                )
            else:
    ...
    def _calcular_analise_categorias(
        ...
        for prod in produtos:
            ...
            if cat not in categorias_stats:
                categorias_stats[cat] = {"quantidade": 0, "receita": 0, "custo": 0}
        ...
        for categoria, stats in categorias_stats.items():
            ...
            margem = (lucro / stats["receita"] * 100) if stats["receita"] > 0 else 0
            participacao = (
                (stats["receita"] / receita_total_geral * 100)
    ...
    def _gerar_insights(self, dre_periodo_id: int, usuario_id: int, dre: DREPeriodo):
        ...
        indices = (
            self.db.query(IndicesMercado)
            .filter(IndicesMercado.setor == "pet_shop", IndicesMercado.ativo.is_(True))
            .first()
        )
        ...
        if not indices:
            ...
            logger.warning(
                "⚠️  Índices de mercado não encontrados, usando valores padrão"
            )
            indices = type(
                "obj",
                (object,),
        ...
        if dre.margem_liquida_percent < indices.margem_liquida_ideal_min:
            ...
            impacto_valor = (
                (diferenca / 100) * dre.receita_liquida
        ...
        if (
            dre.margem_liquida_percent >= indices.margem_liquida_ideal_min
            and dre.margem_liquida_percent <= indices.margem_liquida_ideal_max
            and dre.status != "prejuizo"
        ):
            insights.append(
                {
                    "tipo": "recomendacao",
        ...
        for insight_data in insights:
            insight = DREInsight(
                dre_periodo_id=dre_periodo_id, usuario_id=usuario_id, **insight_data
            )
            self.db.add(insight)
    ...
    def obter_produtos_rentabilidade(
        self, dre_periodo_id: int, usuario_id: int
    ) -> List[DREProduto]:
    ...
    def obter_insights(self, dre_periodo_id: int, usuario_id: int) -> List[DREInsight]:
    ...
    def comparar_periodos(
        self, usuario_id: int, dre1_id: int, dre2_id: int
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "variacoes": {
                "receita_percent": calc_variacao(
                    dre1.receita_liquida, dre2.receita_liquida
                ),
                "lucro_percent": calc_variacao(dre1.lucro_liquido, dre2.lucro_liquido),
                "margem_percent": calc_variacao(
                    dre1.margem_liquida_percent, dre2.margem_liquida_percent
                ),
            },
        }
