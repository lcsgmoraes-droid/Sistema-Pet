...
class DRECanalService:
    ...
    def calcular_dre_por_canal(
        ...
        total_receita_periodo = (
            self.db.query(func.sum(Venda.valor_total))
            .filter(
                Venda.user_id == usuario_id,
                Venda.criado_em >= data_inicio,
                Venda.criado_em <= data_fim,
        ...
        proporcao_canal = (
            receita_bruta / total_receita_periodo if total_receita_periodo > 0 else 0
        )

        # Despesas totais do período
        despesas_vendas_total = (
