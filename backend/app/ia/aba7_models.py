...
class DREPeriodo(TenantScoped, Base):
    ...
    usuario_id = Column(
        Integer, index=True
    )  # dono legado (mantido p/ compat. de leitura)
    ...
    # ===== CANAL DE VENDA =====
    canal = Column(
        String(50), default="loja_fisica"
    )  # 'loja_fisica', 'mercado_livre', 'shopee', 'amazon', 'todos'
    ...
    impostos_detalhamento = Column(
        Text
    )  # JSON com detalhamento (PIS, COFINS, IRPJ, etc)
    ...
    # Relacionamentos
    historico_atualizacoes = relationship(
        "HistoricoAtualizacaoDRE",
        back_populates="dre_periodo",
...
class IndicesMercado(BaseTenantModel):
    ...
    )  # 'pet_shop', 'varejo', 'servicos', 'restaurante'
