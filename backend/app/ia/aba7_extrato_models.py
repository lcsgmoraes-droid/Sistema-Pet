...
class PadraoCategoriacaoIA(BaseTenantModel):
    ...
    beneficiario_pattern = Column(
        String(500)
    )  # Texto ou regex (ex: "Mercio%" ou "ENERGISA%")
    ...
    lancamentos_aplicados = relationship(
        "LancamentoImportado", back_populates="padrao_sugerido"
    )
...
class LancamentoImportado(BaseTenantModel):
    ...
    padrao_sugerido_id = Column(
        Integer, ForeignKey("padroes_categorizacao_ia.id"), nullable=True
    )
    confianca_ia = Column(Float, default=0.0)  # 0.0 a 1.0
    categorias_alternativas = Column(
        JSON, nullable=True
    )  # TOP 3 sugestões: [{id, nome, confianca}]
    ...
    conta_receber_id = Column(
        Integer, ForeignKey("contas_receber.id", ondelete="SET NULL"), nullable=True
    )
    linkagem_automatica = Column(Boolean, default=False)
    confianca_linkagem = Column(Float, nullable=True)

    # Lançamento criado
    lancamento_manual_id = Column(
        Integer, ForeignKey("lancamentos_manuais.id"), nullable=True
    )
    ...
    padrao_sugerido = relationship(
        "PadraoCategoriacaoIA", back_populates="lancamentos_aplicados"
    )
    categoria_sugerida = relationship(
        "CategoriaFinanceira", foreign_keys=[categoria_financeira_id]
    )
    categoria_validada = relationship(
        "CategoriaFinanceira", foreign_keys=[categoria_usuario_id]
    )
...
class HistoricoAtualizacaoDRE(BaseTenantModel):
    ...
    tipo_alteracao = Column(
        String(50)
    )  # 'devolucao', 'nota_atrasada', 'correcao_manual'
    descricao = Column(Text)

    # Valores antes/depois
    valores_anteriores = Column(
        JSON
    )  # {receita_bruta: 26500, lucro_liquido: 3430, ...}
