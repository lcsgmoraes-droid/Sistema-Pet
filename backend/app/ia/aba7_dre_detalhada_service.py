...
class DREDetalhadaService:
    ...
    def calcular_dre_por_canal(
        self, usuario_id: int, data_inicio: date, data_fim: date, canal: str
    ) -> DREDetalheCanal:
        """
        ...
        if tenant_id is None:
            raise ValueError(
                "tenant_id e obrigatorio para calcular DRE detalhado por canal"
            )
        ...
        despesas_vendas_especificas = (
            self.db.query(func.sum(LancamentoManual.valor))
            .filter(
                ...
                LancamentoManual.categoria.ilike(
                    f"%{canal}%"
                ),  # Categorias que mencionam o canal
            )
            .scalar()
            or 0
        )
        ...
        alocacoes = (
            self.db.query(AlocacaoDespesaCanal)
            .filter(
        ...
        for alocacao in alocacoes:
            if alocacao.modo_alocacao == "manual" and alocacao.alocacao_manual:
                ...
                if canal in manual:
                    ...
                    if alocacao.categoria_despesa.lower() in [
                        "salario",
                        "salário",
                        "folha",
                        "pessoal",
                    ...
                    elif alocacao.categoria_despesa.lower() in [
                        "aluguel",
                        "luz",
                        "água",
                        "agua",
            ...
            elif alocacao.modo_alocacao == "proporcional" and canal in json.loads(
                alocacao.canais_afetados or "[]"
            ):
                ...
                if receita_bruta > 0 and alocacao.usar_faturamento:
                    ...
                    total_receita = (
                        self.db.query(func.sum(Venda.total))
                        .filter(
                            Venda.user_id == usuario_id,
                            Venda.tenant_id == tenant_id,
                            Venda.data_venda >= data_inicio,
    ...
    def calcular_dre_consolidado(
        self, usuario_id: int, data_inicio: date, data_fim: date, canais: List[str]
    ) -> Dict:
        """
        ...
        margem_bruta_consolidada = (
            (lucro_bruto_total / receita_total_liquida * 100)
            if receita_total_liquida > 0
            else 0
        )
        margem_operacional_consolidada = (
            (lucro_operacional_total / receita_total_liquida * 100)
            if receita_total_liquida > 0
            else 0
        )
        margem_liquida_consolidada = (
            (lucro_liquido_total / receita_total_liquida * 100)
            if receita_total_liquida > 0
            else 0
        )
        ...
        return {
            ...
            "despesas": {
                ...
                "totais": {
                    ...
                    "administrativas": sum(
                        d["administrativas"] for d in despesas_detalhadas
                    ),
    ...
    def salvar_alocacao_despesa(
        ...
        canais: List[str],
        alocacao_manual: Optional[
            Dict
        ] = None,  # Se modo='manual': {canal: {valor, percentual}}
        ...
        if tenant_id is None:
            raise ValueError(
                "tenant_id e obrigatorio para salvar alocacao de despesa por canal"
            )
        sync_rls_tenant(self.db, tenant_id)
