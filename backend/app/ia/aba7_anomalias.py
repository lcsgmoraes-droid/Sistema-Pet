...
class DetectorAnomalias:
    ...
    def detectar_anomalias_periodo(
        self, usuario_id: int, dre_periodo_id: int
    ) -> List[Dict]:
        """
        ...
        # 1. Buscar DRE atual
        dre_atual = (
            self.db.query(DREPeriodo).filter(DREPeriodo.id == dre_periodo_id).first()
        )
        ...
        if dre_atual.impostos and dre_atual.impostos > 0:
            impostos_hist = [
                d.impostos for d in dres_historico if d.impostos and d.impostos > 0
            ]
        ...
        for anomalia in anomalias:
            self._salvar_insight(
                dre_periodo_id=dre_periodo_id, usuario_id=usuario_id, anomalia=anomalia
            )
    ...
    def _salvar_insight(self, dre_periodo_id: int, usuario_id: int, anomalia: Dict):
