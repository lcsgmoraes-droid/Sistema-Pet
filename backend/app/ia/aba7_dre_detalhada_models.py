...
class DREDetalheCanal(BaseTenantModel):
    ...
    )  # loja_fisica, mercado_livre, shopee, amazon
    ...
    despesas_vendas = Column(
        Float, default=0
    )  # Comissões, taxas marketplace (já específicas do canal)
    despesas_pessoal = Column(
        Float, default=0
    )  # Salários, encargos (alocado manualmente)
    despesas_administrativas = Column(
        Float, default=0
    )  # Água, luz, internet, telefone, aluguel (alocado manualmente)
    despesas_financeiras = Column(
        Float, default=0
    )  # Juros, taxas bancárias (alocado manualmente)
...
class DREConsolidado(BaseTenantModel):
    ...
    receita_por_canal = Column(
        Text
    )  # JSON: {"loja_fisica": 10000, "mercado_livre": 5000}
    ...
    despesas_pessoal = Column(
        Float, default=0
    )  # Soma das despesas com pessoal de todos os canais
    ...
    despesas_por_canal = Column(
        Text
    )  # JSON: {"loja_fisica": 3000, "mercado_livre": 1500, "shopee": 1500}
...
class AlocacaoDespesaCanal(BaseTenantModel):
    ...
    usar_faturamento = Column(
        Boolean, default=True
    )  # Se True, usa receita de cada canal; se False, percentual manual
