...
class Conversa(BaseTenantModel):
    ...
    )  # FK removida temporariamente
    titulo = Column(
        String(200), nullable=True
    )  # Gerado automaticamente pela primeira mensagem
    ...
    mensagens = relationship(
        "MensagemChat", back_populates="conversa", cascade="all, delete-orphan"
    )
...
class MensagemChat(BaseTenantModel):
    ...
    id = Column(Integer, primary_key=True, index=True)
    conversa_id = Column(
        Integer, ForeignKey("conversas_ia.id"), nullable=False, index=True
    )
...
class ContextoFinanceiro(BaseTenantModel):
    ...
    tipo_contexto = Column(
        String(50), nullable=False
    )  # 'indices', 'projecoes', 'alertas', 'transacoes'
