...
_DATA_EXTRATO_TEXTO_RE = re.compile(r"^\d{2}[\/\-\.]\d{2}[\/\-\.]\d{2,4}$")
...
class ExtratoParser:
    ...
    def _parse_excel(self, arquivo: bytes) -> List[Dict]:
        ...
        try:
            ...
            return self._parse_dataframe_transacoes(
                df, "Não foi possível detectar colunas de data, descrição ou valor"
            )
    ...
    def _parse_csv(self, arquivo: bytes) -> List[Dict]:
        ...
        try:
            ...
            return self._parse_dataframe_transacoes(
                df, "Não foi possível detectar colunas necessárias"
            )
    ...
    def _parse_pdf(self, arquivo: bytes) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    "PDF parsing requer: pip install pdf2image pytesseract Pillow"
                )
    ...
    def _parse_dataframe_transacoes(self, df, erro_colunas: str) -> List[Dict]:
        ...
        for _, row in df.iterrows():
            try:
                transacao = self._transacao_de_linha(
                    row, col_data, col_descricao, col_valor
                )
                if transacao:
    ...
    def _transacao_de_linha(
        self, row, col_data: str, col_descricao: str, col_valor: str
    ) -> Optional[Dict]:
    ...
    def _extrair_transacoes_texto(self, texto: str) -> List[Dict]:
        ...
        for linha in texto.splitlines():
            ...
            if len(partes) < 3 or not _DATA_EXTRATO_TEXTO_RE.fullmatch(partes[0]):
                continue
            ...
            if data and descricao and valor > 0:
                ...
                if any(
                    ...
                    for kw in ["pag", "debito", "compra", "saque"]
                ):
                    tipo = "saida"
