...
def dashboard_vet(
    ...
    agendamentos_hoje = (
        db.query(func.count(AgendamentoVet.id))
        .filter(
    ...
    consultas_hoje = (
        db.query(func.count(ConsultaVet.id))
        .filter(
    ...
    internados = (
        db.query(func.count(InternacaoVet.id))
        .filter(
    ...
    vacinas_vencendo_30d = (
        db.query(func.count(VacinaRegistro.id))
        .filter(
    ...
    retornos_30d = sum(
        1 for c in consultas_30d if (c.tipo or "").strip().lower() == "retorno"
    )
    ...
    tempo_medio_atendimento_min = (
        round(sum(duracoes_min) / len(duracoes_min), 1) if duracoes_min else 0.0
    )
    procedimentos_30d = (
    ...
    financeiro_30d = [
        _resumo_financeiro_procedimento(
            procedimento.valor, procedimento.insumos, regra_financeira
        )
        for procedimento in procedimentos_30d
    ]
    faturamento_procedimentos_30d = _round_money(
        sum(item["valor_cobrado"] for item in financeiro_30d)
    )
    custo_procedimentos_30d = _round_money(
        sum(item["custo_total"] for item in financeiro_30d)
    )
    margem_procedimentos_30d = _round_money(
        sum(item["margem_valor"] for item in financeiro_30d)
    )
    margem_percentual_procedimentos_30d = (
        round((margem_procedimentos_30d / faturamento_procedimentos_30d) * 100, 2)
        if faturamento_procedimentos_30d > 0
        else 0.0
    )
    repasse_empresa_procedimentos_30d = _round_money(
        sum(item["repasse_empresa_valor"] for item in financeiro_30d)
    )
    receita_tenant_procedimentos_30d = _round_money(
        sum(item["receita_tenant_valor"] for item in financeiro_30d)
    )
    entrada_empresa_procedimentos_30d = _round_money(
        sum(item["entrada_empresa_valor"] for item in financeiro_30d)
    )
...
def relatorio_clinico_vet(
    ...
    consultas_finalizadas = sum(
        1 for c in consultas if (c.status or "").strip().lower() == "finalizada"
    )
    ...
    top_diagnosticos = [
        ...
        for nome, qtd in sorted(
            diagnosticos_count.items(), key=lambda item: item[1], reverse=True
        )[:top]
    ]

    procedimentos_periodo = (
        db.query(ProcedimentoConsulta)
        .join(ConsultaVet, ConsultaVet.id == ProcedimentoConsulta.consulta_id)
        .filter(
            ProcedimentoConsulta.tenant_id == tenant_id,
    ...
    for procedimento in procedimentos_periodo:
        ...
        item = procedimentos_resumo.setdefault(
            chave,
            {
                "nome": chave,
                "quantidade": 0,
    ...
    top_procedimentos = [
        {
            ...
            "margem_percentual": round(
                (item["margem_total"] / item["valor_total"]) * 100, 2
            )
    ...
    return {
        ...
        "financeiro_procedimentos": {
            ...
            "margem_percentual": round(
                (total_procedimentos_margem / total_procedimentos_valor) * 100, 2
            )
            if total_procedimentos_valor > 0
            else 0.0,
        },
        "top_diagnosticos": [
...
def exportar_relatorio_clinico_csv(
    ...
    conteudo.append(
        [
            ...
            f"{dados['financeiro_procedimentos']['faturamento_total']:.2f}",
        ]
    )
    conteudo.append(
        [
            "Custo procedimentos",
            f"{dados['financeiro_procedimentos']['custo_total']:.2f}",
        ]
    )
    conteudo.append(
        [
            "Margem procedimentos",
            f"{dados['financeiro_procedimentos']['margem_total']:.2f}",
        ]
    )
    conteudo.append(
        [
            "Margem % procedimentos",
            f"{dados['financeiro_procedimentos']['margem_percentual']:.2f}",
        ]
    )
    conteudo.append(
        ["Modo operacional", dados["financeiro_procedimentos"]["modo_operacional"]]
    )
    conteudo.append(
        [
            "Comissão empresa %",
            f"{dados['financeiro_procedimentos']['comissao_empresa_pct']:.2f}",
        ]
    )
    conteudo.append(
        [
            "Entrada empresa",
            f"{dados['financeiro_procedimentos']['entrada_empresa_total']:.2f}",
        ]
    )
    conteudo.append(
        [
            "Repasse empresa",
            f"{dados['financeiro_procedimentos']['repasse_empresa_total']:.2f}",
        ]
    )
    conteudo.append(
        [
            "Receita líquida vet",
            f"{dados['financeiro_procedimentos']['receita_tenant_total']:.2f}",
        ]
    )
    conteudo.append([])
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=relatorio_clinico_vet_{dias}d.csv"
        },
    )
...
def validar_assinatura_consulta(
    ...
    return {
        ...
        "finalizado_em": c.finalizado_em,
        "motivo": "OK"
        if hash_recalculado == c.hash_prontuario
        else "Hash divergente: possível alteração após finalização.",
    }
...
def baixar_prontuario_pdf(
    ...
    if c.status != "finalizada":
        raise HTTPException(
            status_code=400,
            detail="A consulta precisa estar finalizada para gerar o prontuário em PDF",
        )
    ...
    url_validacao = (
        f"{str(request.base_url).rstrip('/')}/vet/consultas/{consulta_id}/assinatura"
    )
...
def baixar_prescricao_pdf(
    ...
    url_validacao = (
        f"{str(request.base_url).rstrip('/')}/vet/consultas/{p.consulta_id}/assinatura"
    )
