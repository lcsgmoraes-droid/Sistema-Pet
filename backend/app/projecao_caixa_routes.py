...
def buscar_projecao(
    ...
    projecao = projetar_caixa(db=db, tenant_id=tenant_id, meses_a_frente=meses_a_frente)

    return {"sucesso": True, "projecao": projecao, "meses_projetados": len(projecao)}
...
def buscar_resumo_projecao(
    ...
    resumo = obter_resumo_projecao(
        db=db, tenant_id=tenant_id, meses_a_frente=meses_a_frente
    )
