...
def conversion_rate_by_type(
    ...
    try:
        with _session_for_tenant(tenant_id) as session:
            ...
            total_convertidas = (
                session.query(func.count(OpportunityEvent.id))
                .filter(
                    and_(
                        OpportunityEvent.tenant_id == tenant_id,
                        OpportunityEvent.event_type
                        == OpportunityEventTypeEnum.CONVERTIDA,
                        OpportunityEvent.created_at >= start_date,
                        OpportunityEvent.created_at <= end_date,
                    )
                )
...
def top_products_converted(tenant_id: UUID, limit: int = 10) -> List[Dict[str, Any]]:
    """
    ...
    try:
        with _session_for_tenant(tenant_id) as session:
            query = (
                select(
                    OpportunityEvent.extra_data["produto_sugerido_id"].astext.label(
                        "produto_id"
                    ),
                    func.count(OpportunityEvent.id).label("count"),
                )
        ...
        return [{"produto_id": row[0], "conversoes": row[1]} for row in results]
...
def top_products_ignored(tenant_id: UUID, limit: int = 10) -> List[Dict[str, Any]]:
    """
    ...
    try:
            ...
            query = (
                ...
                .where(
                    and_(
                        OpportunityEvent.tenant_id == tenant_id,
                        OpportunityEvent.event_type
                        == OpportunityEventTypeEnum.REJEITADA,
                    )
                )
                .group_by(OpportunityEvent.extra_data["produto_sugerido_id"].astext)
                .order_by(func.count(OpportunityEvent.id).desc())
        ...
        return [{"produto_id": row[0], "rejeicoes": row[1]} for row in results]
            ...
            query = (
                select(
                    ...
                    func.sum(
                        func.cast(
                            OpportunityEvent.event_type
                            == OpportunityEventTypeEnum.REFINADA,
                            sqlalchemy.Integer,
                        )
                    ).label("refinadas"),
                    func.sum(
...
def get_metrics_dashboard_summary(tenant_id: UUID, days: int = 30) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if total_events > 0:
            taxas = {
                "conversao": round(
                    (events_by_type.get("oportunidade_convertida", 0) / total_events)
                    * 100,
                    2,
                ),
                "refinacao": round(
                    (events_by_type.get("oportunidade_refinada", 0) / total_events)
                    * 100,
                    2,
                ),
                "rejeicao": round(
                    (events_by_type.get("oportunidade_rejeitada", 0) / total_events)
                    * 100,
                    2,
                ),
            }
