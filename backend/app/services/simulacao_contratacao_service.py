    ...
    return {
        ...
        "analise": {
            "percentual_encargos": float(
                (total_inss + total_fgts) / total_salarios * 100
            )
            if total_salarios > 0
            else 0,
            "percentual_provisoes": float(
                (total_ferias + total_terco + total_13) / total_salarios * 100
            )
            if total_salarios > 0
            else 0,
            "custo_total_vs_salario": float(total_geral / total_salarios * 100)
            if total_salarios > 0
            else 0,
        },
    }
...
def comparar_cenarios(cenario_atual: Dict, cenario_com_contratacao: Dict) -> Dict:
    """
    ...
    diferenca_custo_mensal = cenario_com_contratacao["totais"][
        "media_mensal"
    ...
    return {
        ...
        "percentual_aumento": (
            float(
                ...
                / cenario_atual.get("custo_mensal_atual", 1)
                * 100
            )
