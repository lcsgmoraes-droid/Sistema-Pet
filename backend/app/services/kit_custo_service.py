...
class KitCustoService:
    ...
    def calcular_custo_kit(
        kit_id: int, db: Session, user_id: Optional[int] = None
    ) -> Decimal:
        """
        ...
        try:
            ...
            componentes = (
                ...
                .filter(ProdutoKitComponente.kit_id == kit_id)
                .all()
            )
            kit_e_granel = (
            ...
            if kit_e_granel:
                ...
                produto_comp = (
                    db.query(Produto)
                    .filter(Produto.id == componente.produto_componente_id)
                    .first()
                )
                if not produto_comp:
                    return Decimal(str(kit.preco_custo or 0))
            ...
            logger.debug(f"🔍 Encontrados {len(componentes)} componente(s) no KIT")
            ...
            logger.debug(f"✅ Custo total do produto composto: R$ {custo_total:.2f}")
            ...
            logger.error(
                f"❌ Erro ao calcular custo do KIT {kit_id}: {str(e)}", exc_info=True
            )
            ...
            raise ValueError(f"Erro ao calcular custo do KIT: {str(e)}")
    ...
    @staticmethod
    def recalcular_kits_que_usam_produto(
    ...
    def validar_componentes_kit(kit_id: int, db: Session, user_id: int) -> dict:
        ...
        resultado = {"valido": True, "total_componentes": 0, "erros": [], "avisos": []}

        try:
            # Buscar KIT
            kit = (
                db.query(Produto)
                .filter(Produto.id == kit_id, Produto.user_id == user_id)
                .first()
            )
