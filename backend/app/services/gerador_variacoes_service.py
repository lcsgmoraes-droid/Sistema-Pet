...
class GeradorVariacoesService:
    ...
    def generate_variacoes(
        produto_pai_id: int, db: Session, user_id: int
    ) -> List[Produto]:
        """
        ...
        logger.info(
            f"🔄 Iniciando geração de variações para produto_pai_id={produto_pai_id}"
        )
        ...
        produto_pai = (
            db.query(Produto)
            .filter(Produto.id == produto_pai_id, Produto.user_id == user_id)
            .first()
        )
        ...
        try:
            for combinacao in combinacoes:
                ...
                if GeradorVariacoesService._variacao_existe(
                    ...
                    logger.debug(
                        f"⏭️ Variação já existe: {[op.valor for op in combinacao]}"
                    )
                    continue
    ...
    def _variacao_existe(
        produto_pai_id: int, combinacao: Tuple[ProdutoAtributoOpcao, ...], db: Session
    ) -> bool:
        """
        ...
        for variacao in variacoes:
            ...
            atributos_variacao = (
                db.query(ProdutoVariacaoAtributo)
                .filter(ProdutoVariacaoAtributo.variacao_id == variacao.id)
                .all()
            )
    ...
    def _criar_variacao(
        ...
        for i, opcao in enumerate(combinacao):
            ...
            vinculo = ProdutoVariacaoAtributo(
                variacao_id=variacao.id, atributo_id=atributo.id, opcao_id=opcao.id
            )
