...
class VariacaoLixeiraService:
    ...
    @staticmethod
    def excluir_variacao(variacao_id: int, db: Session, user_id: int) -> Produto:
        """
        ...
        variacao = (
            db.query(Produto)
            .filter(Produto.id == variacao_id, Produto.user_id == user_id)
            .first()
        )
    ...
    @staticmethod
    def restaurar_variacao(variacao_id: int, db: Session, user_id: int) -> Produto:
        """
    ...
    def listar_variacoes_ativas(
        ...
        logger.info(
            f"📋 {len(variacoes)} variações ativas encontradas para produto_pai_id={produto_pai_id}"
        )
    ...
    def listar_variacoes_excluidas(
        ...
        variacoes = (
            ...
            .order_by(Produto.deleted_at.desc())
            .all()
        )

        logger.info(
            f"🗑️ {len(variacoes)} variações na lixeira encontradas para produto_pai_id={produto_pai_id}"
        )

        return variacoes

    @staticmethod
    def contar_variacoes(produto_pai_id: int, db: Session, user_id: int) -> dict:
        """
        ...
        resultado = {"ativas": ativas, "excluidas": excluidas, "total": total}
