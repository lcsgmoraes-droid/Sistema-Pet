...
def gerar_provisao_simples_por_nf(
    ...
    if not config or not config.simples_ativo:
        return {"sucesso": False, "motivo": "Simples Nacional não está ativo"}

    if not config.aliquota_simples_vigente:
        return {"sucesso": False, "motivo": "Alíquota não configurada"}
    ...
    periodo = buscar_periodo_dre_do_tenant(
        db, tenant_id, competencia_mes, competencia_ano
    )
    ...
    if not periodo:
        ...
        if not usuario_id:
            ...
            usuario = db.query(User).filter(User.tenant_id == tenant_id).first()
        ...
        if competencia_mes == 12:
            ...
            data_fim = (
                datetime(competencia_ano, competencia_mes + 1, 1) - timedelta(days=1)
            ).date()
    ...
    if periodo.lucro_operacional is not None and periodo.impostos is not None:
        ...
        if periodo.receita_liquida and periodo.receita_liquida > 0:
            periodo.margem_liquida_percent = float(
                (periodo.lucro_liquido / periodo.receita_liquida * 100).quantize(
                    Decimal("0.01")
                )
            )
