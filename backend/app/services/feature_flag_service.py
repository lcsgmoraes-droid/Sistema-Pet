...
def is_feature_enabled(
    db: Session, tenant_id: UUID, feature_key: str, use_cache: bool = True
) -> bool:
    """
...
def set_feature_flag(
    db: Session, tenant_id: UUID, feature_key: str, enabled: bool
) -> FeatureFlag:
    """
    ...
    feature_flag = (
        db.query(FeatureFlag)
        .filter(
            FeatureFlag.tenant_id == tenant_id, FeatureFlag.feature_key == feature_key
        )
        .first()
    )
...
def get_all_feature_flags(db: Session, tenant_id: UUID) -> Dict[str, bool]:
    ...
    flags = db.query(FeatureFlag).filter(FeatureFlag.tenant_id == tenant_id).all()
