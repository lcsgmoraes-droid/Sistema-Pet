...
class SegmentacaoService:
    ...
    def calcular_metricas_cliente(
        cliente_id: int, tenant_id: UUID, db: Session
    ) -> Dict:
        """
        ...
        data_limite_180d = hoje - timedelta(
            days=SegmentacaoService.PERIODO_ATUAL_DIAS * 2
        )
        ...
        vendas_anteriores = (
            db.query(func.count(Venda.id).label("quantidade"))
            .filter(
                Venda.cliente_id == cliente_id,
                Venda.tenant_id == tenant_id,
                Venda.status.in_(["finalizada"]),
        ...
        if ultima_venda:
            ultima_compra_date = (
                ultima_venda.data_venda.date()
                if isinstance(ultima_venda.data_venda, datetime)
                else ultima_venda.data_venda
            )
        ...
        if primeira_venda:
            primeira_compra_date = (
                primeira_venda.data_venda.date()
                if isinstance(primeira_venda.data_venda, datetime)
                else primeira_venda.data_venda
            )
    ...
    def aplicar_regras_segmentacao(metricas: Dict) -> Tuple[str, List[str]]:
        ...
        # Regra 1: VIP
        if (
            metricas["total_compras_90d"] >= SegmentacaoService.VIP_TOTAL_MINIMO
            or metricas["ticket_medio"] >= SegmentacaoService.VIP_TICKET_MEDIO_MINIMO
        ):
            tags.append("VIP")
        ...
        # Regra 3: Novo
        if (
            metricas["primeira_compra_dias"] <= SegmentacaoService.NOVO_DIAS_MAXIMOS
            and metricas["primeira_compra_dias"] > 0
        ):
            tags.append("Novo")
        ...
        if (
            ...
            and metricas["compras_90d_anteriores"] > 0
            and metricas["compras_90d"] < metricas["compras_90d_anteriores"]
        ):
            tags.append("Risco")
    ...
    def recalcular_segmento_cliente(
        ...
        metricas = SegmentacaoService.calcular_metricas_cliente(
            cliente_id=cliente_id, tenant_id=tenant_id, db=db
        )

        # 3. Aplicar regras de segmentação
        segmento_principal, tags = SegmentacaoService.aplicar_regras_segmentacao(
            metricas
        )
        ...
        existing = db.execute(
            check_query, {"cliente_id": cliente_id, "tenant_id": tenant_id}
        ).fetchone()
    ...
    def recalcular_todos_segmentos(
        tenant_id: UUID, db: Session, limit: Optional[int] = None
    ) -> Dict:
        """
        ...
        for cliente in clientes:
            try:
                resultado = SegmentacaoService.recalcular_segmento_cliente(
                    cliente_id=cliente.id, tenant_id=tenant_id, db=db
                )
    ...
    def obter_segmento_cliente(
        ...
        result = db.execute(
            query, {"cliente_id": cliente_id, "tenant_id": tenant_id}
        ).fetchone()
    ...
    def listar_segmentos(
        tenant_id: UUID, db: Session, segmento_filtro: Optional[str] = None
    ) -> List[Dict]:
        """
