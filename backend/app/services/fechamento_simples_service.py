...
def fechar_simples_mensal(
    ...
    if not registro:
        registro = SimplesNacionalMensal(tenant_id=tenant_id, mes=mes, ano=ano)
        db.add(registro)
    ...
    if faturamento_base and faturamento_base > 0 and registro.imposto_real:
        ...
        registro.aliquota_sugerida = Decimal(str(sugestao / 100)).quantize(
            Decimal("0.0001")
        )
    ...
    return {
        ...
        "faturamento_contador": float(registro.faturamento_contador)
        if registro.faturamento_contador
        else None,
        ...
        "diferenca_imposto": float(registro.diferenca_imposto)
        if registro.diferenca_imposto
        else None,
        "aliquota_efetiva_decimal": float(registro.aliquota_efetiva)
        if registro.aliquota_efetiva
        else None,
        "aliquota_efetiva_percentual": float(registro.aliquota_efetiva * 100)
        if registro.aliquota_efetiva
        else None,
        "aliquota_sugerida_percentual": float(registro.aliquota_sugerida * 100)
        if registro.aliquota_sugerida
        else None,
        "fechado": registro.fechado,
    }


def reabrir_simples_mensal(db: Session, tenant_id, mes: int, ano: int) -> dict:
    """
    ...
    if not registro:
        return {"sucesso": False, "motivo": f"Fechamento {mes}/{ano} não encontrado"}

    registro.fechado = False
    db.commit()

    return {"sucesso": True, "mes": mes, "ano": ano, "fechado": False}


def atualizar_faturamento_contador(
    db: Session, tenant_id, mes: int, ano: int, faturamento: Decimal
) -> dict:
    """
