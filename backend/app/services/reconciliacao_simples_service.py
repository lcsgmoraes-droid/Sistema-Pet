...
def reconciliar_das_simples(
    ...
    periodo = buscar_periodo_dre_do_tenant(
        db, tenant_id, mes_competencia, ano_competencia
    )
    ...
    return {
        ...
        if diferenca > 0
        else ("negativo" if diferenca < 0 else "sem_ajuste"),
        ...
        "aliquota_atualizada_automaticamente": sugestao_aliquota
        != float(config.aliquota_simples_vigente)
        if config and sugestao_aliquota
        else False,
    }
