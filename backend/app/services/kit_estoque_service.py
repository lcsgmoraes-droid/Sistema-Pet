...
class KitEstoqueService:
    ...
    def calcular_estoque_virtual_kit(
        ...
        if kit.tipo_produto not in ("KIT", "VARIACAO"):
            raise ValueError(
                f"Produto #{kit_id} não é KIT/VARIACAO-KIT (tipo atual: {kit.tipo_produto})"
            )
        ...
        estoque_virtual = (
            min(kits_possiveis_por_componente) if kits_possiveis_por_componente else 0
        )
    ...
    def validar_componentes_kit(
        db: Session, kit_id: int, componentes: List[Dict]
        ...
        for idx, comp in enumerate(componentes):
            ...
            if produto_id == kit_id:
                return (
                    False,
                    f"Componente #{idx + 1}: KIT não pode conter a si mesmo (recursão não permitida)",
                )
            ...
            if not produto:
                return (
                    False,
                    f"Componente #{idx + 1}: produto_id={produto_id} não encontrado",
                )
                ...
                resultado.append(
                    {
                        ...
                        "produto_peso_embalagem": produto.peso_embalagem,
                        "produto_e_granel": bool(getattr(produto, "e_granel", False)),
