...
class OFXParser:
            ...
            for prefix in ["ofx:", ""]:
                ...
                for stmttrn in root.findall(f".//{prefix}STMTTRN"):
                    ...
                    if dtposted is not None:
                        transacao.data_movimento = OFXParser._parse_data_ofx(
                            dtposted.text
                        )
    ...
    def parse_arquivo(caminho_arquivo: str) -> ExtratoOFX:
        ...
        raise ValueError(
            f"Não foi possível decodificar o arquivo com encodings: {encodings}"
        )
...
def validar_extrato(extrato: ExtratoOFX) -> Dict[str, any]:
    ...
    sem_descricao = [
        t for t in extrato.transacoes if not t.memo and not t.nome_beneficiario
    ]
    if sem_descricao:
