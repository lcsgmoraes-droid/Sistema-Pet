...
class RegraClassificacaoDRE(BaseTenantModel):
    ...
    canal = Column(
        String(50), nullable=True, comment="Se aplicável, forçar canal específico"
    )
    ...
    aplicacoes_sucesso = Column(
        Integer, default=0, comment="Quantas vezes foi aplicada com sucesso"
    )
    aplicacoes_rejeitadas = Column(
        Integer, default=0, comment="Quantas vezes usuário rejeitou a sugestão"
    )

    # Flags
    ativo = Column(Boolean, default=True)
    sugerir_apenas = Column(
        Boolean,
        default=False,
        comment="True = apenas sugere, False = aplica automaticamente",
    )

    # Auditoria
    criado_por_user_id = Column(
        Integer, nullable=True, comment="User que criou (se manual)"
    )

    # Relationships
    dre_subcategoria = relationship(
        "DRESubcategoria", foreign_keys=[dre_subcategoria_id]
    )
...
class HistoricoClassificacao(BaseTenantModel):
    ...
    lancamento_id = Column(
        Integer, nullable=False, comment="ID do contas_pagar ou contas_receber"
    )
    ...
    regra_aplicada_id = Column(
        Integer, ForeignKey("regras_classificacao_dre.id"), nullable=True
    )
    ...
    # Feedback do usuário
    usuario_aceitou = Column(
        Boolean, default=True, comment="False se usuário rejeitou/alterou"
    )
    ...
    regra_aplicada = relationship(
        "RegraClassificacaoDRE", foreign_keys=[regra_aplicada_id]
    )
