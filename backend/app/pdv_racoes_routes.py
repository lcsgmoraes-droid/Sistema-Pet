...
async def verificar_alergia_produto(
    ...
    pets = db.query(Pet).filter(Pet.cliente_id == cliente_id, Pet.ativo.is_(True)).all()
...
async def obter_produtos_similares(
    ...
    for produto in produtos_candidatos:
        ...
        if produto.peso_embalagem and produto_base.peso_embalagem:
            diff_peso = (
                abs(produto.peso_embalagem - produto_base.peso_embalagem)
                / produto_base.peso_embalagem
            )
            if diff_peso <= 0.2:
                score += 10
    ...
    for item in similares:
        ...
        sugestoes_lista.append(
            ProdutoSimilar(
                ...
                estoque_quantidade=float(prod.estoque_atual)
                if prod.estoque_atual
                else 0,
            )
        )
...
async def obter_cross_sell(
    produtos_carrinho: List[int] = Query(
    ...
    for produto_id in produtos_carrinho:
        ...
        for rel in produtos_relacionados:
            prod_relacionado = (
                db.query(Produto).filter(Produto.id == rel.produto_id).first()
            )
            if not prod_relacionado:
                continue

            marca = (
                db.query(Marca).filter(Marca.id == prod_relacionado.marca_id).first()
            )
            ...
            sugestoes.append(
                {
                    ...
                    "estoque_quantidade": float(prod_relacionado.estoque_atual)
                    if prod_relacionado.estoque_atual
                    else 0,
                }
            )

        if sugestoes:
            sugestoes_por_produto.append(
                CrossSellResponse(
        ...
        resultado.append(
            {
                ...
                "categoria": next(
                    (
                        c.nome
                        for c in categorias_complementares
