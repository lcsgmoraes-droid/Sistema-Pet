...
def recalcular_segmento_cliente(
    ...
    try:
        ...
        resultado["mensagem"] = (
            f"Segmento do cliente {resultado['cliente_nome']} recalculado com sucesso"
        )
...
def recalcular_todos_segmentos(
    ...
    try:
        resultado = SegmentacaoService.recalcular_todos_segmentos(
            tenant_id=tenant_id, db=db, limit=request.limit
        )
...
def listar_segmentos(
    ...
    try:
        segmentos = SegmentacaoService.listar_segmentos(
            tenant_id=tenant_id, db=db, segmento_filtro=segmento
        )

        return {"total": len(segmentos), "segmentos": segmentos}
...
def obter_estatisticas_segmentos(
    ...
    try:
        ...
        ultima_result = db.execute(
            ultima_atualizacao_query, {"tenant_id": tenant_id}
        ).fetchone()
