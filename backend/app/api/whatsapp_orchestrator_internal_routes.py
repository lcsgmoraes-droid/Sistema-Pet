...
from app.whatsapp.tool_utils import _normalize_text
...
def _customer_phone_digits(value: str) -> str:
    return "".join(character for character in str(value or "") if character.isdigit())


def _serialize_latest_purchase(db: Session, tenant_id: str, customer_id: int):
...
def _serialize_latest_delivery(db: Session, tenant_id: str, customer_id: int):
...
def get_catalog_data(
    ...
    """Consulta o catálogo real sem permitir qualquer gravação."""
...
def get_customer_context_data(
    ...
    """Consulta cliente e histórico necessário ao atendimento, sem gravar dados."""
    ...
    try:
        ...
        if customer_id is not None:
            ...
            phone_digits = _customer_phone_digits(phone)
            if len(phone_digits) >= 8:
                ...
                matches = [
                    ...
                    if any(
                        _customer_phone_digits(value).endswith(phone_digits[-8:])
        ...
        return {
            ...
            "latest_purchase": _serialize_latest_purchase(db, tenant_id, customer.id),
            "latest_delivery": _serialize_latest_delivery(db, tenant_id, customer.id),
            "benefits": {},
        }
    finally:
        clear_current_tenant()
...
def get_store_context_data(
    ...
    """Consulta nome e horário cadastrados da loja, sem gravar dados."""
