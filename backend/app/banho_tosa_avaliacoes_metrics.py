...
def resumir_nps(avaliacoes) -> dict:
    ...
    notas_servico = [
        Decimal(item.nota_servico)
        for item in avaliacoes
        if item.nota_servico is not None
    ]
    return {
        "avaliacoes": total,
        ...
        "nota_servico_media": sum(notas_servico, Decimal("0")) / len(notas_servico)
        if notas_servico
        else Decimal("0"),
    }
...
def _nps(total: int, promotores: int, detratores: int) -> Decimal:
    if not total:
        return Decimal("0")
    return (Decimal(promotores) / Decimal(total) * Decimal("100")) - (
        Decimal(detratores) / Decimal(total) * Decimal("100")
    )
