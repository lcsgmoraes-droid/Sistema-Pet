...
@router.get(
    "/parceiros",
    response_model=List[PartnerLinkResponse],
    summary="Lista parcerias do tenant atual",
)
def listar_parceiros(
    ...
    for link in links:
        vet_tenant = (
            db.query(Tenant).filter(Tenant.id == str(link.vet_tenant_id)).first()
        )
        result.append(
            PartnerLinkResponse(
                id=link.id,
                ...
                comissao_empresa_pct=float(link.comissao_empresa_pct)
                if link.comissao_empresa_pct
                else None,
                ativo=link.ativo,
...
@router.post(
    ...
    status_code=201,
    summary="Cria vínculo com veterinário parceiro",
)
def criar_parceiro(
    ...
    if not vet_tenant:
        raise HTTPException(
            status_code=404, detail="Tenant do veterinário não encontrado."
        )
    ...
    if str(payload.vet_tenant_id) == str(tenant_id):
        raise HTTPException(
            status_code=400,
            detail="O tenant parceiro não pode ser o mesmo tenant atual.",
        )
    ...
    if existente:
        raise HTTPException(
            status_code=409,
            detail="Já existe um vínculo com este veterinário parceiro.",
        )
...
@router.patch(
    "/parceiros/{link_id}",
    response_model=PartnerLinkResponse,
    summary="Atualiza vínculo de parceria",
)
def atualizar_parceiro(
    ...
    if not link:
        raise HTTPException(
            status_code=404, detail="Vínculo de parceria não encontrado."
        )
...
@router.delete(
    "/parceiros/{link_id}", status_code=204, summary="Remove vínculo de parceria"
)
...
@router.get(
    "/relatorios/repasse", summary="Relatório de repasse veterinário por período"
)
def relatorio_repasse(
    ...
    for c in contas:
        tipo = (
            "repasse_empresa"
            if "-REPASSE-EMPRESA" in (c.documento or "")
            else "liquido_vet"
        )
        ...
        items.append(
            {
                ...
                "data_recebimento": c.data_recebimento.isoformat()
...
@router.post(
    "/relatorios/repasse/{conta_id}/baixar",
    summary="Dá baixa (recebimento) em um lançamento de repasse",
)
def baixar_repasse(
    conta_id: int,
