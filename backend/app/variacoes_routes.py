...
class VariationAttributesSchema(BaseModel):
    ...
    attributes: Dict[str, str] = Field(
        ..., description="Dicionário de atributos: {'cor': 'azul', 'tamanho': 'G'}"
    )

    class Config:
        json_schema_extra = {"example": {"attributes": {"cor": "Azul", "tamanho": "G"}}}
...
class VariationCreateSchema(BaseModel):
    ...
    nome_complemento: Optional[str] = Field(
        None, description="Complemento do nome (ex: '- Azul G')"
    )
    variation_attributes: Dict[str, str] = Field(
        ..., description="Atributos da variação"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "variation_attributes": {"cor": "Azul", "tamanho": "G"},
                "preco_venda": 89.90,
                "preco_custo": 45.00,
                "estoque_atual": 10,
...
def validate_sku_uniqueness(
    sku: str, tenant_id: str, db: Session, exclude_id: Optional[int] = None
):
    ...
    if query.first():
        raise HTTPException(
            status_code=400, detail=f"SKU '{sku_normalizado}' ja cadastrado"
        )
...
@router.post(
    "/{parent_id}/variacoes", response_model=VariationResponseSchema, status_code=201
)
def criar_variacao(
    parent_id: int,
...
def listar_variacoes(
    ...
    variacoes = (
        ...
        .order_by(Produto.variation_signature)
        .all()
    )
...
def atualizar_variacao(
    ...
    **Nota:** Não permite alterar variation_attributes (assinatura é imutável)
    ...
    if dados.nome_complemento is not None:
        produto_pai = (
            db.query(Produto).filter(Produto.id == variacao.produto_pai_id).first()
        )
    ...
    if dados.estoque_atual is not None:
        try:
            ...
            sincronizar_bling_background(
                variacao.id, variacao.estoque_atual, "edicao_variacao"
            )
        except Exception:
...
def converter_produto_em_pai(
    ...
    **Ações:**
    ...
    produto = (
        db.query(Produto)
        .filter(Produto.id == parent_id, Produto.tenant_id == tenant_id)
        .first()
    )
    ...
    if produto.produto_pai_id:
        raise HTTPException(
            status_code=400, detail="Este produto é uma variação, não pode ser pai"
        )
