...
class AIEngine(IAIEngine):
    ...
    def __init__(
        self, prompt_builder: AIPromptBuilder = None, enable_cost_control: bool = True
    ):
        """
    ...
    async def generate_response(
        ...
        if self.cost_controller:
            ...
            ) * AISettings.OPENAI_GPT4_PRICE_PER_1K_OUTPUT
            ...
            if not can_proceed:
                logger.warning(
                    f"[AIEngine] Limite de custo excedido para tenant {tenant_id}: {reason}"
                )
        ...
        try:
            return await self._generate_with_provider(
                prompt=prompt, context=context, objetivo=objetivo, tenant_id=tenant_id
            )
    ...
    async def _generate_with_provider(
        self, prompt: str, context: Dict[str, Any], objetivo: str, tenant_id: int
    ) -> AIResponse:
        """
        ...
        return self._convert_provider_response(
            provider_response=provider_response, context=context, tenant_id=tenant_id
        )
    ...
    def _convert_provider_response(
        self, provider_response, context: Dict[str, Any], tenant_id: int
    ) -> AIResponse:
        """
    ...
    def _analyze_context_mock(self, context: Dict[str, Any], objetivo: str) -> str:
        """
    ...
    def _generate_explanation_mock(self, context: Dict[str, Any], objetivo: str) -> str:
        """
