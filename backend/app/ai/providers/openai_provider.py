...
class OpenAIProvider(IAIProvider):
    ...
    def __init__(
        self, api_key: Optional[str] = None, model: str = None, max_retries: int = 2
    ):
        """
    ...
    async def generate(self, request: ProviderRequest) -> ProviderResponse:
        ...
        try:
            ...
            response = await asyncio.wait_for(
                self._call_openai_api(request), timeout=request.timeout_seconds
            )
    ...
    def _calculate_cost(self, prompt_tokens: int, completion_tokens: int) -> float:
        ...
        if "gpt-4" in self.model.lower():
            input_cost = (
                prompt_tokens / 1000
            ) * AISettings.OPENAI_GPT4_PRICE_PER_1K_INPUT
            output_cost = (
            ...
            input_cost = (
                prompt_tokens / 1000
            ) * AISettings.OPENAI_GPT35_PRICE_PER_1K_INPUT
            output_cost = (
                completion_tokens / 1000
            ) * AISettings.OPENAI_GPT35_PRICE_PER_1K_OUTPUT
