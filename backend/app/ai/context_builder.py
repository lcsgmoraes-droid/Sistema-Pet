...
class ContextBuilder:
    ...
    async def build_context(
        self, tenant_id: str, session_id: str, message: str
    ) -> Dict[str, Any]:
        with whatsapp_tenant_context(tenant_id):
            return await self._build_context_with_context(
    ...
    async def _build_context_with_context(
        ...
        try:
            ...
            cliente = (
                await self._get_cliente_info(tenant_id, session.cliente_id)
            ...
            logger.info(
                f"✅ Contexto construído: {len(produtos)} produtos, cliente={cliente is not None}"
            )
    ...
    async def _get_tenant_config(self, tenant_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "working_hours": {
                "start": config.working_hours_start.isoformat()
                if config.working_hours_start
                else None,
                "end": config.working_hours_end.isoformat()
                if config.working_hours_end
                else None,
            },
    ...
    async def _get_cliente_info(
        ...
        return {
            ...
            "endereco": f"{cliente.endereco}, {cliente.bairro}"
            if cliente.endereco
            else None,
            "ultimo_pedido": {
                "id": ultimo_pedido.id,
    ...
    async def _get_conversation_history(
        ...
        messages = (
            ...
            .filter(WhatsAppMessage.session_id == session_id)
