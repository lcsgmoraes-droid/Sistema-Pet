...
class CostController:
    ...
    def __init__(self, daily_limit_usd: float = None, warning_threshold: float = None):
        """
    ...
    def _get_usage(self, tenant_id: int, date_obj: date = None) -> TenantUsage:
        ...
        if cache_key not in self._usage_cache:
            self._usage_cache[cache_key] = TenantUsage(
                tenant_id=tenant_id, date=date_obj
            )

        return self._usage_cache[cache_key]

    def check_can_proceed(
        self, tenant_id: int, estimated_tokens: int, estimated_cost: float
    ...
    def record_usage(self, tenant_id: int, tokens_used: int, cost_usd: float):
        """
        Registra uso de IA.
    ...
    def get_usage_summary(self, tenant_id: int) -> dict:
        ...
        return {
            ...
            "is_near_limit": usage.is_near_limit(
                self.daily_limit_usd, self.warning_threshold
            ),
