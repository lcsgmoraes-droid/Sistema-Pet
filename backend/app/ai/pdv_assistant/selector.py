...
class PDVInsightSelector:
    ...
    def selecionar_insights_para_pdv(
        self, pdv_context: PDVContext, insights_disponiveis: List[Insight]
    ) -> List[Insight]:
        """
        ...
        insights_tenant = [
            i for i in insights_disponiveis if i.tenant_id == pdv_context.tenant_id
        ]

        # 2. Filtrar apenas tipos relevantes para PDV
        insights_relevantes = [
            i for i in insights_tenant if i.tipo in self.TIPOS_RELEVANTES_PDV
        ]

        # 3. Filtrar por contexto específico
        insights_contextuais = self._filtrar_por_contexto(
            pdv_context, insights_relevantes
        )

        # 4. Priorizar insights
        insights_priorizados = self._priorizar_insights(
            pdv_context, insights_contextuais
        )
    ...
    def _filtrar_por_contexto(
        self, pdv_context: PDVContext, insights: List[Insight]
    ) -> List[Insight]:
        """
        ...
        for insight in insights:
            ...
            if insight.tipo in [
                ...
                if cliente_id_insight and cliente_id_insight != pdv_context.cliente_id:
                    logger.debug(
                        "[PDVInsightSelector] Ignorando insight - cliente diferente"
                    )
                    continue
    ...
    def _priorizar_insights(
        ...
        insights_ordenados = sorted(insights, key=calcular_score, reverse=True)
