...
class PDVAIService:
    ...
    async def sugerir_para_pdv(self, pdv_context: PDVContext) -> List[PDVSugestao]:
        """
        ...
        try:
            ...
            insights_selecionados = self.insight_selector.selecionar_insights_para_pdv(
                pdv_context, insights_disponiveis
            )
            ...
            # 4. Gerar sugestões
            sugestoes = await self._gerar_sugestoes(pdv_context, insights_selecionados)

            # 5. Ordenar por prioridade
            sugestoes_ordenadas = self._ordenar_sugestoes(sugestoes)

            logger.info(f"[PDVAIService] Geradas {len(sugestoes_ordenadas)} sugestões")

            return sugestoes_ordenadas

        except Exception as e:
            logger.error(
                f"[PDVAIService] Erro ao gerar sugestões: {str(e)}", exc_info=True
            )
    ...
    def _buscar_insights(self, pdv_context: PDVContext) -> List[Insight]:
        ...
        try:
            ...
            logger.error(
                f"[PDVAIService] Erro ao buscar insights: {str(e)}", exc_info=True
            )
            return []

    async def _gerar_sugestoes(
        self, pdv_context: PDVContext, insights_selecionados: List[Insight]
    ) -> List[PDVSugestao]:
        """
    ...
    async def _converter_insight_em_sugestao(
        self, pdv_context: PDVContext, insight: Insight
    ...
    def _gerar_mensagem_sugestao(
        ...
        try:
            ...
            if insight.tipo == TipoInsight.PRODUTOS_COMPRADOS_JUNTOS:
                ...
                if len(produtos) >= 2:
                    return (
                        f"{produtos[0]} costuma ser comprado junto com {produtos[1]}."
                    )
            ...
            logger.error(
                f"[PDVAIService] Erro ao gerar mensagem: {str(e)}", exc_info=True
            )
            return None
    ...
    def _ordenar_sugestoes(self, sugestoes: List[PDVSugestao]) -> List[PDVSugestao]:
        """
        ...
        return sorted(
            sugestoes,
            key=lambda s: (ordem_prioridade.get(s.prioridade, 0), s.confianca),
            reverse=True,
        )
