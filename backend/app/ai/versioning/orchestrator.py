...
class RolloutOrchestrator:
    ...
    def _get_initial_tenants(self, plan: RolloutPlan) -> List[str]:
        ...
        if strategy == RolloutStrategy.IMMEDIATE:
            ...
            if plan.config.canary_tenant_ids:
                ...
                return _secure_random.sample(all_tenants, count)
    ...
    def check_rollout_health(self, plan_id: UUID) -> Dict:
        ...
        avg_fallback_rate = (
            sum(m.fallback_rate for m in metrics) / len(metrics) if metrics else 0
        )
    ...
    def proceed_next_step(self, plan_id: UUID) -> RolloutPlan:
        ...
        new_tenants = _secure_random.sample(
            available, min(count - len(plan.current_tenant_ids), len(available))
        )
