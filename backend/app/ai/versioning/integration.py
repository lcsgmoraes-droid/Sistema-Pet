...
class VersionedDecisionService:
    ...
    def make_decision(
        ...
        result["_version_metadata"] = {
            ...
            "component_versions": {
                ct.value: cv.version for ct, cv in behavior_version.components.items()
            },
...
class VersionAwareAuditService:
    ...
    def get_version_performance_stats(
        ...
        total = len(decisions)
        successes = sum(
            1 for d in decisions if d["decision"].get("action") != "fallback_to_human"
        )

        avg_confidence = (
            sum(d["decision"].get("confidence_score", 0) for d in decisions) / total
        )

        avg_trust = sum(d["decision"].get("trust_score", 0) for d in decisions) / total
