...
def example_complete_change_flow():
    ...
    logger.info(
        f"  Required roles: {[r.value for r in approval_flow.applicable_rule.required_roles]}"
    )
    logger.info(f"  Min approvals: {approval_flow.applicable_rule.min_approvals}")
    logger.info(
        f"  Require all roles: {approval_flow.applicable_rule.require_all_roles}"
    )
    ...
    logger.info(
        f"  Approvals received: {status['approved_count']}/{status['min_approvals']}"
    )
    ...
    for approval in history:
        ...
        logger.info(
            f"    Timestamp: {approval.approved_at.strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    logger.info(
        f"✓ Aprovações: {len(history)}/{approval_flow.applicable_rule.min_approvals}"
    )
