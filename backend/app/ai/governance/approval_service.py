...
class ApprovalService:
    ...
    def create_approval_flow(
        ...
        if not rule:
            raise ValueError(
                f"No approval rule for impact level {change_request.impact_level}"
            )
    ...
    def submit_approval(
        ...
        existing_approval = next(
            (a for a in flow.approvals if a.approver_role == approver_role), None
        )
            ...
            if flow.is_approved:
                ...
                rejection = next(
                    (
                        ...
                        if a.decision == ApprovalDecision.REJECTED
                    ),
                    None,
                )
                reason = (
                    rejection.comments if rejection else "Rejected during approval flow"
                )
