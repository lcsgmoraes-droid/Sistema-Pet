...
def detectar_operadora_csv(
    conteudo: str, nome_arquivo: str = ""
    ...
    if ";" in primeira_linha:
        ...
        colunas_presentes = sum(
            1 for col in colunas_stone_recebimentos if col in colunas
        )
    ...
    if "," in primeira_linha:
        ...
        if (
            sum(
                1
                for col in colunas_rede
                if any(cr in col for cr in ["NSU", "CV", "VALOR BRUTO"])
            )
            >= 2
        ):
            return {
...
def detectar_operadora_ofx(conteudo: str) -> Optional[Dict[str, Any]]:
    ...
    if "STONE" in conteudo_upper or "<ORG>STONE" in conteudo_upper:
        return {"operadora": "Stone", "tipo_arquivo": "extrato_ofx", "confianca": 1.0}

    # PagBank (antigo PagSeguro)
    if "PAGBANK" in conteudo_upper or "PAGSEGURO" in conteudo_upper:
        return {"operadora": "PagBank", "tipo_arquivo": "extrato_ofx", "confianca": 1.0}
...
def detectar_operadora_automatico(
    conteudo: str, nome_arquivo: str = "", tipo_arquivo: str = ""
) -> Optional[Dict[str, Any]]:
    """
...
01/02/2024;15/02/2024;Cartão de crédito;100,00;3,79;0,00;96,21;Pago;JOAO SILVA;12345678;TID123;1;1;ABC123;Presencial;VISA;12345678;0,00;0,00"""
    ...
    resultado = detectar_operadora_csv(
        csv_stone_relatorio, "Stone_Relatorio_02_2024.csv"
    )
...
1820,00;0,00;0,00;0,00;1820,00;paid;TID123;NSU456;AUTH789;10/02/2024;10/02/2024;Visa;Débito;1;POS;ABC123;;0,00;Taxa Zero"""
    ...
    resultado = detectar_operadora_csv(
        csv_stone_comprovante, "Comprovante_Pagamento.csv"
    )
