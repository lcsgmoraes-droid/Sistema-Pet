...
def atualizar_dre_por_lancamento(
    ...
    *,
    ...
    if subcategoria.categoria.natureza == NaturezaDRE.RECEITA:
        ...
        dre_detalhe.receita_liquida = (
            dre_detalhe.receita_bruta - dre_detalhe.deducoes_receita
        )
        ...
        if "venda" in nome_lower or "comiss" in nome_lower or "taxa" in nome_lower:
            ...
            "pessoal" in nome_lower or "salario" in nome_lower or "folha" in nome_lower
        ):
            dre_detalhe.despesas_pessoal += valor_decimal
        elif (
            "admin" in nome_lower
            or "aluguel" in nome_lower
            or "agua" in nome_lower
            or "luz" in nome_lower
        ):
    ...
    dre_detalhe.lucro_bruto = (
        dre_detalhe.receita_liquida - dre_detalhe.custo_produtos_vendidos
    )

    if dre_detalhe.receita_liquida > 0:
        dre_detalhe.margem_bruta_percent = (
            dre_detalhe.lucro_bruto / dre_detalhe.receita_liquida
        ) * 100
    ...
    dre_detalhe.lucro_operacional = (
        dre_detalhe.lucro_bruto - dre_detalhe.total_despesas_operacionais
    )

    if dre_detalhe.receita_liquida > 0:
        dre_detalhe.margem_operacional_percent = (
            dre_detalhe.lucro_operacional / dre_detalhe.receita_liquida
        ) * 100

    dre_detalhe.lucro_liquido = dre_detalhe.lucro_operacional - dre_detalhe.impostos

    if dre_detalhe.receita_liquida > 0:
        dre_detalhe.margem_liquida_percent = (
            dre_detalhe.lucro_liquido / dre_detalhe.receita_liquida
        ) * 100
    ...
    if subcategoria.tipo_custo == TipoCusto.INDIRETO_RATEAVEL:
        try:
            ...
            calcular_rateio_dre(db=db, periodo=periodo, tenant_id=tenant_id)
