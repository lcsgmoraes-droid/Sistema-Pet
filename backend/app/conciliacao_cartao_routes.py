...
class ConciliacaoCartaoIn(BaseModel):
    ...
    adquirente: str = Field(
        ...,
        min_length=1,
        max_length=50,
        description="Nome da adquirente (Stone, Cielo, etc)",
    )
    forma_pagamento_id: int | None = Field(
        None, description="ID da forma de pagamento (opcional)"
    )
...
def listar_pendentes_conciliacao(
    data_inicio: Optional[date] = Query(
        None, description="Data inicial para filtro (data de vencimento)"
    ),
    data_fim: Optional[date] = Query(
        None, description="Data final para filtro (data de vencimento)"
    ),
    adquirente: Optional[str] = Query(
        None, description="Nome da adquirente para filtrar"
    ),
    db: Session = Depends(get_session),
    user_and_tenant=Depends(get_current_user_and_tenant),
):
...
def upload_conciliacao_cartao(
    file: UploadFile = File(
        ...,
        description="Arquivo CSV com colunas: nsu, valor, data_recebimento, adquirente",
    ),
    db: Session = Depends(get_session),
    user_and_tenant=Depends(get_current_user_and_tenant),
):
    ...
    if reader.fieldnames is None or not colunas_esperadas.issubset(
    ...
    for idx, row in enumerate(reader, start=2):  # start=2 porque linha 1 é o cabeçalho
        ...
        try:
            ...
            if (
                not row.get("nsu")
                or not row.get("valor")
                or not row.get("data_recebimento")
                or not row.get("adquirente")
            ):
    ...
    return {
        ...
        "taxa_sucesso": round(
            (resultados["conciliados"] / resultados["processados"] * 100), 2
        )
