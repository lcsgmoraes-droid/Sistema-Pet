...
def get_replays(
    ...
    """
    # Query base
    query = db.query(AuditLog).filter(AuditLog.action.in_(REPLAY_ACTIONS))
    ...
    # Buscar itens
    items = (
        query.order_by(desc(AuditLog.timestamp)).limit(page_size).offset(offset).all()
    )
...
def get_rebuilds(
    ...
    """
    # Query base
    query = db.query(AuditLog).filter(AuditLog.action.in_(REBUILD_ACTIONS))
    ...
    if status:
        if status == "success":
            filters.append(
                AuditLog.action.in_(
                    ["rebuild_read_models_success", "schema_swap_success"]
                )
            )
        elif status == "failure":
            filters.append(
                AuditLog.action.in_(
                    ["rebuild_read_models_failure", "schema_swap_failure"]
                )
            )
...
def get_audit_summary(
    ...
    # Contar replays
    total_replays = base_query.filter(AuditLog.action.in_(REPLAY_ACTIONS)).count()
    ...
    # Contar rebuilds
    total_rebuilds = base_query.filter(AuditLog.action.in_(REBUILD_ACTIONS)).count()

    successful_rebuilds = base_query.filter(
        AuditLog.action.in_(["rebuild_read_models_success", "schema_swap_success"])
    ).count()

    failed_rebuilds = base_query.filter(
        AuditLog.action.in_(["rebuild_read_models_failure", "schema_swap_failure"])
    ).count()
    ...
    last_replay = (
        base_query.filter(AuditLog.action.in_(REPLAY_ACTIONS))
        .order_by(desc(AuditLog.timestamp))
        .first()
    )

    last_rebuild = (
        base_query.filter(AuditLog.action.in_(REBUILD_ACTIONS))
        .order_by(desc(AuditLog.timestamp))
        .first()
    )
