...
class VendaReadModelHandler:
    ...
    def on_venda_criada(self, evento: VendaCriada) -> None:
        ...
        try:
            ...
            # Buscar estado atual
            resumo_atual = (
                self.db.query(VendasResumoDiario)
            ...
            valores = {
                ...
                "data": hoje,
                "quantidade_aberta": (
                    resumo_atual.quantidade_aberta if resumo_atual else 0
                )
                + 1,
                "quantidade_finalizada": resumo_atual.quantidade_finalizada
                if resumo_atual
                else 0,
                "quantidade_cancelada": resumo_atual.quantidade_cancelada
                if resumo_atual
                else 0,
                "total_vendido": resumo_atual.total_vendido
                if resumo_atual
                else Decimal("0"),
                "total_cancelado": resumo_atual.total_cancelado
                if resumo_atual
                else Decimal("0"),
            }
            ...
            # NÃO FAZ COMMIT
            logger.debug(
                f"✅ UPSERT resumo: {hoje}, aberta={valores['quantidade_aberta']}"
            )
    ...
    def on_venda_finalizada(self, evento: VendaFinalizada) -> None:
        ...
        try:
            logger.info(
                f"📊 [IDEMPOTENTE] VendaFinalizada: {evento.venda_id}, total={evento.total}"
            )
            ...
            self._upsert_receita_mensal(
                mes_atual, Decimal(str(evento.total)), operacao="finalizada"
            )
    ...
    def on_venda_cancelada(self, evento: VendaCancelada) -> None:
        ...
        try:
            logger.info(
                f"📊 [IDEMPOTENTE] VendaCancelada: {evento.venda_id}, total={evento.total}"
            )
            ...
            # 3. Receita Mensal
            self._upsert_receita_mensal(
                mes_atual, Decimal(str(evento.total)), operacao="cancelada"
            )
    ...
    def _upsert_resumo_diario_finalizada(self, data: date, total: Decimal) -> None:
        ...
        resumo_atual = (
            self.db.query(VendasResumoDiario)
            .filter(VendasResumoDiario.data == data)
            .first()
        )
        ...
        valores = {
            "tenant_id": tenant_id,
            "data": data,
            "quantidade_aberta": max(
                0, (resumo_atual.quantidade_aberta if resumo_atual else 1) - 1
            ),
            "quantidade_finalizada": (
                resumo_atual.quantidade_finalizada if resumo_atual else 0
            )
            + 1,
            "quantidade_cancelada": resumo_atual.quantidade_cancelada
            if resumo_atual
            ...
            "total_vendido": (
                resumo_atual.total_vendido if resumo_atual else Decimal("0")
            )
            + total,
            "total_cancelado": resumo_atual.total_cancelado
            if resumo_atual
        ...
        if valores["quantidade_finalizada"] > 0:
            valores["ticket_medio"] = (
                valores["total_vendido"] / valores["quantidade_finalizada"]
            )
        else:
    ...
    def _upsert_resumo_diario_cancelada(self, data: date, total: Decimal) -> None:
        ...
        valores = {
            ...
            "quantidade_cancelada": (
                resumo_atual.quantidade_cancelada if resumo_atual else 0
            )
            + 1,
            "total_vendido": resumo_atual.total_vendido
            if resumo_atual
            ...
            "total_cancelado": (
                resumo_atual.total_cancelado if resumo_atual else Decimal("0")
            )
            + total,
        }
    ...
    def _upsert_performance_parceiro(
        self, funcionario_id: int, mes_referencia: date, total: Decimal, operacao: str
    ) -> None:
        """UPSERT idempotente para performance"""
        tenant_id = self._require_tenant_id()
        perf_atual = (
            self.db.query(PerformanceParceiro)
            .filter(
        ...
        if operacao == "finalizada":
            valores = {
                ...
                + 1,
                "total_vendido": (
                    perf_atual.total_vendido if perf_atual else Decimal("0")
                )
                + total,
            ...
            valores = {
                ...
                "mes_referencia": mes_referencia,
                "quantidade_vendas": perf_atual.quantidade_vendas if perf_atual else 0,
                "total_vendido": perf_atual.total_vendido
                if perf_atual
                else Decimal("0"),
                "vendas_canceladas": (perf_atual.vendas_canceladas if perf_atual else 0)
                + 1,
        ...
        if valores["quantidade_vendas"] > 0:
            valores["ticket_medio"] = (
                valores["total_vendido"] / valores["quantidade_vendas"]
            )
        else:
            valores["ticket_medio"] = Decimal("0")

        if valores["quantidade_vendas"] > 0:
            valores["taxa_cancelamento"] = (
                valores["vendas_canceladas"] / valores["quantidade_vendas"]
            ) * 100
        else:
    ...
    def _upsert_receita_mensal(
        self, mes_referencia: date, total: Decimal, operacao: str
    ) -> None:
        """UPSERT idempotente para receita mensal"""
        tenant_id = self._require_tenant_id()
        receita_atual = (
            self.db.query(ReceitaMensal)
            .filter(ReceitaMensal.mes_referencia == mes_referencia)
            .first()
        )
        ...
        if operacao == "finalizada":
            valores = {
                ...
                "mes_referencia": mes_referencia,
                "receita_bruta": (
                    receita_atual.receita_bruta if receita_atual else Decimal("0")
                )
                + total,
                "quantidade_vendas": (
                    receita_atual.quantidade_vendas if receita_atual else 0
                )
                + 1,
                "receita_cancelada": receita_atual.receita_cancelada
                if receita_atual
                else Decimal("0"),
                "quantidade_cancelamentos": receita_atual.quantidade_cancelamentos
                if receita_atual
                else 0,
            }
        else:  # cancelada
            valores = {
                "tenant_id": tenant_id,
                "mes_referencia": mes_referencia,
                "receita_bruta": receita_atual.receita_bruta
                if receita_atual
                else Decimal("0"),
                "quantidade_vendas": receita_atual.quantidade_vendas
                if receita_atual
                else 0,
                "receita_cancelada": (
                    receita_atual.receita_cancelada if receita_atual else Decimal("0")
                )
                + total,
                "quantidade_cancelamentos": (
                    receita_atual.quantidade_cancelamentos if receita_atual else 0
                )
                + 1,
            }

        # Calcular receita líquida
        valores["receita_liquida"] = (
            valores["receita_bruta"] - valores["receita_cancelada"]
        )
    ...
    def _notify_venda_finalizada(self, evento: VendaFinalizada) -> None:
        ...
        logger.debug(
            f"🔔 [SIDE EFFECT] Notificação venda finalizada: {evento.venda_id}"
        )
        # TODO: Integrar com serviço de notificação real
...
def registrar_handlers_read_models(dispatcher, db_session_factory):
    ...
    dispatcher.subscribe(
        VendaCriada, criar_handler_com_sessao(VendaReadModelHandler.on_venda_criada)
    )
