...
class ClientesRecorrentesReadModel(BaseReadModel):
    ...
    def clientes_recorrentes(
        ...
        eventos = self.get_eventos_por_usuario(
            user_id=user_id, event_type=VendaRealizadaEvent
        )
        ...
        # Agregar por cliente
        clientes_agregados = defaultdict(
            lambda: {"cliente_id": None, "vendas": [], "valor_total": 0.0}
        )
        ...
        for cliente_id, dados in clientes_agregados.items():
            ...
            if numero_compras > 1:
                ...
                dias_entre_compras = (
                    total_dias / (numero_compras - 1) if numero_compras > 1 else 0
                )
            else:
    ...
    def cliente_detalhado(
        self, cliente_id: int, user_id: Optional[int] = None, dias: Optional[int] = None
    ) -> Optional[Dict]:
        """
    ...
    def clientes_em_risco_churn(
        ...
        for evento in eventos:
            ...
            if (
                dados["ultima_compra"] is None
                or evento.timestamp > dados["ultima_compra"]
            ):
            ...
            resultado.append(
                {
                    ...
                    "ticket_medio": round(
                        dados["valor_total"] / dados["numero_compras"], 2
                    ),
                    "ultima_compra": dados["ultima_compra"].isoformat(),
                    "dias_sem_comprar": dias_sem_comprar,
                    "risco_nivel": self._calcular_nivel_risco(
                        dias_sem_comprar, dias_inatividade
                    ),
                }
            )
    ...
    def rfm_analise(self, user_id: Optional[int] = None, dias: int = 90) -> List[Dict]:
        """
