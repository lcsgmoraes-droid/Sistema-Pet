...
class ProdutosMaisVendidosReadModel(BaseReadModel):
    ...
    def top_produtos(
        ...
        produtos_agregados = defaultdict(
            lambda: {
                "produto_id": 0,
                "produto_nome": "",
                "tipo_produto": "",
    ...
    def produto_detalhado(
        self, produto_id: int, user_id: Optional[int] = None, dias: Optional[int] = None
    ) -> Optional[Dict]:
        """
    ...
    def produtos_por_tipo(
        ...
        # Filtrar por tipo
        produtos_filtrados = [
            p for p in todos_produtos if p["tipo_produto"] == tipo_produto
        ]
