...
def obter_resumo_diario(
    db: Session, data: Optional[date] = None
    ...
    resumo = (
        db.query(VendasResumoDiario).filter(VendasResumoDiario.data == data).first()
    )

    if resumo:
        logger.info(
            f"✅ Resumo encontrado: {data} - {resumo.quantidade_finalizada} vendas"
        )
...
def obter_resumo_periodo(
    ...
    resumos = (
        db.query(VendasResumoDiario)
        .filter(
            VendasResumoDiario.data >= data_inicio, VendasResumoDiario.data <= data_fim
        )
        .order_by(VendasResumoDiario.data)
        .all()
    )
...
def obter_ultimos_dias(db: Session, quantidade_dias: int = 7) -> List[Dict[str, Any]]:
    ...
    resumos = (
        ...
        .order_by(desc(VendasResumoDiario.data))
        .all()
    )

    logger.info(
        f"✅ {len(resumos)} resumos encontrados (últimos {quantidade_dias} dias)"
    )
    return [r.to_dict() for r in resumos]


def obter_ranking_parceiros(
    db: Session, mes_referencia: Optional[date] = None, limite: int = 10
) -> List[Dict[str, Any]]:
    """
    ...
    performances = (
        db.query(PerformanceParceiro)
        .filter(PerformanceParceiro.mes_referencia == mes_referencia)
        .order_by(desc(PerformanceParceiro.total_vendido))
        .limit(limite)
        .all()
    )
...
def obter_performance_funcionario(
    db: Session, funcionario_id: int, mes_referencia: Optional[date] = None
) -> Optional[Dict[str, Any]]:
    """
    ...
    logger.debug(
        f"🔍 Consultando performance: funcionario={funcionario_id}, mes={mes_referencia}"
    )
    ...
    if performance:
        logger.info(
            f"✅ Performance encontrada: {funcionario_id} - R$ {performance.total_vendido}"
        )
        return performance.to_dict()
    else:
        logger.info(
            f"ℹ️  Nenhuma performance encontrada para funcionário {funcionario_id}"
        )
        return None


def obter_receita_mensal(
    db: Session, mes_referencia: Optional[date] = None
) -> Optional[Dict[str, Any]]:
    """
    ...
    if receita:
        logger.info(
            f"✅ Receita encontrada: {mes_referencia} - R$ {receita.receita_liquida}"
        )
...
def obter_receita_por_periodo(
    ...
    receitas = (
        ...
        .order_by(ReceitaMensal.mes_referencia)
        .all()
    )
...
def obter_comparativo_mensal(db: Session, meses: int = 6) -> List[Dict[str, Any]]:
    """
    ...
    for receita in receitas:
        ...
        if receita_anterior and receita_anterior > 0:
            variacao = (
                (float(receita.receita_liquida) - receita_anterior) / receita_anterior
            ) * 100
...
def obter_resumo_diario_ou_vazio(
    db: Session, data: Optional[date] = None
) -> Dict[str, Any]:
    """
