...
class KitsMaisVendidosReadModel(BaseReadModel):
    ...
    def top_kits(
        ...
        eventos = self.get_eventos_por_usuario(
            user_id=user_id, event_type=KitVendidoEvent
        )
            ...
            resultado.append(
                {
                    ...
                    "ticket_medio": round(valor_total / numero_vendas, 2)
                    if numero_vendas > 0
                    else 0.0,
                    "preco_medio": round(valor_total / quantidade_vendida, 2)
    ...
    def kit_detalhado(
        self, kit_id: int, user_id: Optional[int] = None, dias: Optional[int] = None
    ) -> Optional[Dict]:
        """
        ...
        return {
            ...
            "ticket_medio": round(valor_total / vendas_unicas, 2)
            if vendas_unicas > 0
            else 0.0,
            "preco_medio": round(valor_total / quantidade_total, 2)
            if quantidade_total > 0
            else 0.0,
            "total_componentes_baixados": total_componentes,
            "componentes_medio_por_kit": round(total_componentes / len(eventos), 2)
            if eventos
            else 0.0,
    ...
    def componentes_mais_vendidos_em_kits(
        ...
        # Agregar componentes
        componentes_agregados = defaultdict(
            lambda: {
                "produto_id": 0,
                "produto_nome": "",
        ...
        for evento in eventos:
            for componente in evento.componentes_baixados:
                ...
                if dados["produto_id"] == 0:
                    ...
                    dados["produto_nome"] = componente.get(
    ...
    def kits_por_tipo(
        ...
        return self.top_kits(
            limit=limit, user_id=user_id, por_valor=por_valor, tipo_kit=tipo_kit
        )
