...
class ComissoesConfig:
    ...
    def criar_ou_atualizar(
        ...
        try:
            ...
            result = db.execute(
                text(
                    "SELECT id FROM comissoes_configuracao WHERE funcionario_id = :f AND tipo = :t AND referencia_id = :r"
                ),
            ...
            if result:
                ...
                db.execute(
                        ...
                        WHERE id = :id"""),
                    {
                        ...
                        "p": percentual,
                        "pl": percentual_loja,
                        "dtc": desconta_taxa_cartao,
                        "di": desconta_impostos,
                        "dte": desconta_taxa_entrega,
                        "cvp": comissao_venda_parcial,
                        "pev": permite_edicao_venda,
                        "obs": observacoes,
                        "ua": usuario_id,
                        "id": result[0],
                    },
...
class ComissoesItens:
    ...
    def registrar_comissao(
        ...
        valor_custo: float,
        quantidade: float,
        desconto_proporcional: float,
        taxa_cartao_proporcional: float,
        taxa_entrega_proporcional: float,
        imposto_proporcional: float,
        percentual_comissao: float,
        tipo_calculo: str,
        valor_base_calculo: float,
        valor_comissao: float,
        percentual_pago: float = 100.0,
    ) -> int:
        """
        ...
        try:
            ...
            result = db.execute(
                ...
                """),
                {
                    ...
                    "imposto_proporcional": imposto_proporcional,
    ...
    def listar_pendentes(
        funcionario_id: int = None, data_inicio: str = None, data_fim: str = None
    ) -> List[Dict[str, Any]]:
        """
...
class ComissaoVenda(TenantScoped, Base):
    ...
    # Status
    status = Column(
        String(20), nullable=True, default="pendente"
    )  # pendente, pago, estornado
