...
def criar_pendencia(
    ...
    cliente = (
        db.query(Cliente)
        .filter(Cliente.id == pendencia.cliente_id, Cliente.tenant_id == tenant)
        .first()
    )
    ...
    produto = (
        db.query(Produto)
        .filter(Produto.id == pendencia.produto_id, Produto.tenant_id == tenant)
        .first()
    )
    ...
    if pendencia_existente:
        ...
        if pendencia.observacoes:
            pendencia_existente.observacoes = (
                (pendencia_existente.observacoes or "") + "\n" + pendencia.observacoes
            )
        db.commit()
...
def listar_pendencias(
    ...
    query = db.query(PendenciaEstoque).filter(PendenciaEstoque.tenant_id == tenant)
    ...
    query = query.order_by(
        desc(PendenciaEstoque.prioridade), desc(PendenciaEstoque.data_registro)
    )
...
def cancelar_pendencia(
    ...
    return {"message": "Pendência cancelada com sucesso", "pendencia_id": pendencia_id}
...
def dashboard_pendencias(
    ...
    for produto_id, total, qtd in produtos_mais_aguardados:
        produto = (
            db.query(Produto)
            .filter(and_(Produto.id == produto_id, Produto.tenant_id == tenant))
            .first()
        )
        if produto:
