...
async def diagnosticar_comissoes(
    ...
    try:
        ...
        for row in result_vendas.fetchall():
            vendas_sem_comissao.append(
                VendaSemComissaoResponse(
                    venda_id=row[0],
                    numero_venda=row[1],
                    data_venda=row[2],
                    vendedor_id=row[3],
        ...
        estatisticas = {
            ...
            "vendas_sem_config": sum(
                1 for v in vendas_sem_comissao if not v.tem_configuracao
            ),
            "vendas_config_inativa": sum(
                1
                for v in vendas_sem_comissao
                if v.tem_configuracao and not v.config_ativa
            ),
            "valor_total_nao_provisionado": sum(
                c["valor_comissao"] for c in comissoes_nao_provisionadas
            ),
        }
...
async def gerar_comissoes_faltantes(
    ...
    try:
        ...
        for venda_id in request.vendas_ids:
            ...
            result_check = execute_tenant_safe(
                db,
                """
            ...
            try:
                resultado_geracao = gerar_comissoes_venda(
                    venda_id=venda_id, funcionario_id=venda.funcionario_id, db=db
                )
        ...
        logger.info(
            f"✅ Geração concluída: {sucesso_count}/{len(resultados)} vendas processadas com sucesso"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao gerar comissões: {str(e)}"
        )
...
async def reprovisionar_comissoes(
    ...
    try:
                ...
                resultados.append(
                    {
                        ...
                        "comissoes_provisionadas": resultado_provisao[
        ...
        sucesso_count = sum(1 for r in resultados if r["success"])
        valor_total_provisionado = sum(
            r.get("valor_total", 0) for r in resultados if r["success"]
        )
...
async def diagnosticar_venda_especifica(
    ...
    if venda_info["funcionario_id"]:
        result_config = execute_tenant_safe(
            db,
            """
            SELECT 
    ...
    result_itens = execute_tenant_safe(
        db,
        """
        SELECT 
    ...
    # 5. Buscar pagamentos
    result_pagamentos = execute_tenant_safe(
        db,
        """
    ...
    pagamentos_info = {"count": pag[0], "total_pago": float(pag[1])}
    ...
    if (
        venda_info["status"] in ["finalizada", "baixa_parcial"]
        and venda_info["funcionario_id"]
        and len(comissoes) == 0
    ):
        problemas.append(
            {
