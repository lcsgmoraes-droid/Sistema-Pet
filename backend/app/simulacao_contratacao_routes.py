...
class SimulacaoContratacaoRequest(BaseModel):
    ...
    inss_percentual: float = Field(
        20.0, description="Percentual de INSS patronal", ge=0, le=100
    )
    ...
    data_inicio: Optional[date] = Field(
        None, description="Data de início da contratação"
    )


@router.post("/")
def simular_nova_contratacao(
    payload: SimulacaoContratacaoRequest, auth=Depends(get_current_user_and_tenant)
):
    """
