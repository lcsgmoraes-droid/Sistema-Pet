...
class ImportadorPets:
    def __init__(
        self, database_url: str, tenant_id: str, user_id: int, dry_run: bool = False
    ):
    ...
    def truncar_campo(self, valor: str, campo: str) -> str:
        ...
        if len(valor) > limite:
            self.log(
                f"Campo '{campo}' truncado: {valor[:30]}... ({len(valor)} → {limite} chars)",
                "WARN",
            )
            return valor[:limite]
        return valor
    ...
    def validar_pet(self, row: dict, session) -> ResultadoValidacao:
        ...
        existe = session.execute(
            text(
                "SELECT 1 FROM pets WHERE codigo = :codigo AND tenant_id = :tenant_id LIMIT 1"
            ),
        ...
        if existe:
            return ResultadoValidacao(
                valido=False,
                dados_limpos=None,
                erros=[f"Pet com código {codigo} já existe"],
                avisos=[],
            )
        ...
        if not cliente_id:
            return ResultadoValidacao(
                valido=False,
                dados_limpos=None,
                erros=[f"Cliente {codigo_cliente} não encontrado"],
                avisos=[],
            )
        ...
        if not especie or especie == "":
            ...
            if raca and any(
                x in raca.upper()
                for x in ["RETRIEVER", "ROTTW", "POODLE", "BULLDOG", "PASTOR"]
            ):
                especie = "Canina"
            else:
        ...
        castrado = (
            True if esterilizacao and "CASTRADO" in esterilizacao.upper() else False
        )

        # Data nascimento
        data_nascimento = self.converter_data(
            self.normalizar_valor(row.get("ani_dat_nascimento", ""))
        )
        ...
        if erros:
            return ResultadoValidacao(
                valido=False, dados_limpos=None, erros=erros, avisos=avisos
            )
        ...
        return ResultadoValidacao(
            valido=True, dados_limpos=dados, erros=[], avisos=avisos
        )
            ...
            for idx, row in enumerate(rows, 1):
                ...
                if not resultado.valido:
                    ...
                    if any("já existe" in erro for erro in resultado.erros):
                        ...
                        self.log(
                            f"[{idx}/{len(rows)}] DUPLICADO: {row.get('ani_var_nome', 'N/A')} (#{row.get('ani_int_codigo', 'N/A')})",
                            "WARN",
                        )
                    elif any("não encontrado" in erro for erro in resultado.erros):
                        self.stats.clientes_nao_encontrados += 1
                        self.log(
                            f"[{idx}/{len(rows)}] CLIENTE NÃO ENCONTRADO: {row.get('ani_var_nome', 'N/A')} (dono: {row.get('pes_int_codigo', 'N/A')})",
                            "WARN",
                        )
                    else:
                        self.log(
                            f"[{idx}/{len(rows)}] ERRO: {', '.join(resultado.erros)}",
                            "ERROR",
                        )
                        self.stats.erros.append(
                            f"Linha {idx}: {', '.join(resultado.erros)}"
                        )
                    continue
                ...
                if not self.dry_run:
                    try:
                        ...
                        self.log(
                            f"[{idx}/{len(rows)}] IMPORTADO: {resultado.dados_limpos['nome']} (#{resultado.dados_limpos['codigo']})",
                            "OK",
                        )
                    except Exception as e:
                        erro_msg = str(e)[:200]
                        self.log(
                            f"[{idx}/{len(rows)}] ERRO ao inserir: {erro_msg}", "ERROR"
                        )
                    ...
                    self.log(
                        f"[{idx}/{len(rows)}] VÁLIDO (dry-run): {resultado.dados_limpos['nome']} (#{resultado.dados_limpos['codigo']})"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Simular importação (não salva no banco)"
    )
