...
class ImportadorProdutos:
    ...
    def truncar_campo(self, valor: Any, limite: int, nome_campo: str) -> Optional[str]:
        ...
        if len(valor_str) > limite:
            self.log(
                f"Campo '{nome_campo}' truncado de {len(valor_str)} para {limite} chars",
                "WARN",
            )
    ...
    def validar_produto(self, row: dict, session) -> ResultadoValidacao:
        ...
        codigo = (
            self.truncar_campo(codigo_bruto, limites["codigo"], "codigo")
            if codigo_bruto
            else None
        )
        ...
        if result:
            avisos.append(
                f"DUPLICADO: Produto {nome} (#{codigo}) já existe com ID {result[0]}"
            )
        ...
        dados = {
            ...
            "situacao": situacao,
            "unidade": self.truncar_campo(
                row.get("pro_var_unidade", "UN"), limites["unidade"], "unidade"
            )
            or "UN",
            "codigo_barras": self.truncar_campo(
                row.get("pro_var_codigobarra"),
                limites["codigo_barras"],
                "codigo_barras",
            ),
            "ncm": self.truncar_campo(
                row.get("pro_var_codigoncm"), limites["ncm"], "ncm"
            ),
    ...
    def importar_produtos(self, limite: Optional[int] = None):
        ...
        self.log("=" * 80)
        self.log(
            f"{'DRY-RUN - ' if self.dry_run else ''}IMPORTADOR SIMPLESVET - PRODUTOS"
        )
        self.log("=" * 80)
        ...
        percentual = (
            (self.stats.validos / self.stats.total * 100) if self.stats.total > 0 else 0
        )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Simular importação sem gravar no banco"
    )
    parser.add_argument(
        "--limite", type=int, help="Limitar número de registros importados"
    )
