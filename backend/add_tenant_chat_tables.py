"""
Script para adicionar tenant_id às tabelas de Chat IA
- mensagens_chat
...
def add_tenant_to_table(conn, table_name, fk_column=None):
    ...
    if fk_column:
        # Usa uma FK para pegar tenant_id
        conn.execute(text(f"""
            UPDATE {table_name} t
            SET tenant_id = ref.tenant_id
            FROM {fk_column['table']} ref
            WHERE t.{fk_column['column']} = ref.id
            AND t.tenant_id IS NULL
        """))
    else:
        # Usa users.id como fallback
        conn.execute(text(f"""
            UPDATE {table_name} t
...
def main():
    """Adiciona tenant_id às tabelas de Chat IA"""
    
    print("=" * 70)
    print("🔧 ADICIONANDO tenant_id às tabelas de Chat IA")
    print("=" * 70)
    ...
    with engine.connect() as conn:
        try:
            # mensagens_chat (referencia conversas_ia)
            add_tenant_to_table(
                conn, 
                'mensagens_chat',
                fk_column={'table': 'conversas_ia', 'column': 'conversa_id'}
            )
            
            # contexto_financeiro_chat (usa usuario_id)
            add_tenant_to_table(
                conn,
                'contexto_financeiro_chat',
                fk_column=None  # Usa usuario_id diretamente
            )
