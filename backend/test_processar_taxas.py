...
def testar_processar_taxas(venda_id: int):
    ...
    with get_db_session() as db:
        ...
        # Buscar pagamentos
        pagamentos = (
            db.query(VendaPagamento).filter(VendaPagamento.venda_id == venda_id).all()
        )

        print(f"\n📋 Pagamentos ({len(pagamentos)}):")
        for p in pagamentos:
            print(
                f"   - {p.forma_pagamento}: R$ {p.valor} ({p.numero_parcelas}x) - Status: {p.status}"
            )
        ...
        for f in formas:
            print(
                f"   - {f.nome} (tipo: {f.tipo}): {f.taxa_percentual}% + R$ {f.taxa_fixa}"
            )
        ...
        contas_existentes = (
            db.query(ContaPagar)
            .filter(ContaPagar.tenant_id == venda.tenant_id)
            .filter(ContaPagar.descricao.like(f"%Venda {venda.numero_venda}%"))
            .all()
        )
        ...
        try:
            ...
            for c in contas_apos:
                subcategoria = (
                    db.query(DRESubcategoria)
                    .filter(DRESubcategoria.id == c.dre_subcategoria_id)
                    .first()
                )
                subcategoria_nome = subcategoria.nome if subcategoria else "N/A"
                print(
                    f"   - ID {c.id}: {c.descricao} - R$ {c.valor_original} (Subcategoria: {subcategoria_nome})"
                )
