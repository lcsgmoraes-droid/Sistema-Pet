    ...
    try:
        with engine.connect() as conn:
            ...
            campos_novos = [
                (
                    "nota_entrada_id",
                    "ALTER TABLE pedidos_compra ADD COLUMN nota_entrada_id INTEGER REFERENCES notas_entrada(id)",
                ),
                (
                    "data_confronto",
                    "ALTER TABLE pedidos_compra ADD COLUMN data_confronto TIMESTAMP",
                ),
                (
                    "status_confronto",
                    "ALTER TABLE pedidos_compra ADD COLUMN status_confronto VARCHAR(30)",
                ),
                (
                    "resumo_confronto",
                    "ALTER TABLE pedidos_compra ADD COLUMN resumo_confronto TEXT",
                ),
            ]
