...
def verificar_dados():
    ...
    try:
        ...
        result = db.execute(
            text(
                "SELECT COUNT(*) FROM clientes WHERE codigo IS NOT NULL AND codigo != ''"
            )
        )
        ...
        result = db.execute(
            text(
                "SELECT COUNT(*) FROM produtos WHERE codigo IS NOT NULL AND codigo != ''"
            )
        )
        produtos_codigo = result.scalar()
        print(
            f"{'Produtos c/ código':15} : {produtos_codigo:6} (importados do SimplesVet)"
        )

        # Verificar vendas importadas (com prefixo IMP-)
        result = db.execute(
            text("SELECT COUNT(*) FROM vendas WHERE numero_venda LIKE 'IMP-%'")
        )
        ...
        result = db.execute(
            text(
                "SELECT codigo, nome FROM clientes WHERE codigo IS NOT NULL ORDER BY id LIMIT 5"
            )
        )
        ...
        result = db.execute(
            text(
                "SELECT codigo, nome FROM produtos WHERE codigo IS NOT NULL ORDER BY id LIMIT 5"
            )
        )
