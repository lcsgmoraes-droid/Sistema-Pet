...
Uso no ambiente correto:
  python regularizar_vendas_importadas_pagamento_historico.py \
    --tenant-email LOGIN --tenant-name EMPRESA --client-id ID \
...
def preparar(
    db, *, aplicar: bool, email: str, nome_tenant: str, cliente_id: int,
    alvos: dict[str, Decimal],
) -> tuple[list[tuple[Venda, ContaReceber]], User]:
    ...
    with tenant_context(usuario.tenant_id):
        ...
        for numero, esperado in alvos.items():
            ...
            if venda.cliente_id != cliente_id or venda.data_venda.strftime("%Y%m%d") != numero[4:12]:
                raise ValueError(f"{numero}: cliente ou data divergiu")
            ...
            if len(contas) != 1:
                raise ValueError(f"{numero}: esperada uma conta a receber; encontradas {len(contas)}")
            conta = contas[0]
            if (
            ...
            print(
                ...
                f"total=R$ {esperado:.2f}, conta_id={conta.id}, conta_status={conta.status}"
            )
...
def main() -> None:
    ...
    parser.add_argument("--client-id", required=True, type=int, help="Codigo do cliente")
    parser.add_argument(
    ...
    if len(args.sale) != 2 or args.client_id <= 0:
        parser.error("Informe exatamente duas vendas e um client-id positivo")
    alvos = {}
