...
def upgrade() -> None:
    ...
    op.add_column(
        "tenants",
        sa.Column(
            "ecommerce_ativo", sa.Boolean(), server_default="true", nullable=False
        ),
    )
    op.add_column("tenants", sa.Column("ecommerce_descricao", sa.Text(), nullable=True))
    op.add_column(
        "tenants", sa.Column("ecommerce_horario_abertura", sa.String(5), nullable=True)
    )
    op.add_column(
        "tenants",
        sa.Column("ecommerce_horario_fechamento", sa.String(5), nullable=True),
    )
    op.add_column(
        "tenants",
        sa.Column("ecommerce_dias_funcionamento", sa.String(200), nullable=True),
    )
    ...
    op.create_index(
        "ix_ecommerce_notify_requests_tenant_id",
        "ecommerce_notify_requests",
        ["tenant_id"],
    )
    op.create_index(
        "ix_ecommerce_notify_requests_product_id",
        "ecommerce_notify_requests",
        ["product_id"],
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_ecommerce_notify_requests_product_id",
        table_name="ecommerce_notify_requests",
    )
    op.drop_index(
        "ix_ecommerce_notify_requests_tenant_id", table_name="ecommerce_notify_requests"
    )
