...
def upgrade() -> None:
    ...
    op.add_column(
        "venda_pagamentos",
        sa.Column("gateway_net_amount", sa.Numeric(10, 2), nullable=True),
    )
    op.add_column(
        "venda_pagamentos",
        sa.Column("gateway_gross_amount", sa.Numeric(10, 2), nullable=True),
    )
    op.create_index(
        "ix_venda_pagamentos_gateway_provider",
        "venda_pagamentos",
        ["gateway_provider"],
        unique=False,
    )
    op.create_index(
        "ix_venda_pagamentos_gateway_payment_id",
        "venda_pagamentos",
        ["gateway_payment_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_venda_pagamentos_gateway_payment_id", table_name="venda_pagamentos"
    )
