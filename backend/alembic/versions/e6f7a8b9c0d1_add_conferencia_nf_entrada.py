...
def upgrade() -> None:
    ...
    if "conferencia_observacoes" not in nota_columns:
        op.add_column(
            "notas_entrada",
            sa.Column("conferencia_observacoes", sa.Text(), nullable=True),
        )
    if "conferencia_realizada_em" not in nota_columns:
        op.add_column(
            "notas_entrada",
            sa.Column("conferencia_realizada_em", sa.DateTime(), nullable=True),
        )
    if "conferencia_user_id" not in nota_columns:
        op.add_column(
            "notas_entrada",
            sa.Column("conferencia_user_id", sa.Integer(), nullable=True),
        )
    ...
    if "quantidade_conferida" not in item_columns:
        op.add_column(
            "notas_entrada_itens",
            sa.Column("quantidade_conferida", sa.Float(), nullable=True),
        )
    if "quantidade_avariada" not in item_columns:
        op.add_column(
            "notas_entrada_itens",
            sa.Column(
                "quantidade_avariada", sa.Float(), nullable=True, server_default="0"
            ),
        )
    if "observacao_conferencia" not in item_columns:
        op.add_column(
            "notas_entrada_itens",
            sa.Column("observacao_conferencia", sa.Text(), nullable=True),
        )
    ...
    if "ix_notas_entrada_conferencia_status" not in nota_indexes:
        op.create_index(
            "ix_notas_entrada_conferencia_status",
            "notas_entrada",
            ["conferencia_status"],
        )
    if "ix_notas_entrada_conferencia_user_id" not in nota_indexes:
        op.create_index(
            "ix_notas_entrada_conferencia_user_id",
            "notas_entrada",
            ["conferencia_user_id"],
        )
...
def downgrade() -> None:
    ...
    if "ix_notas_entrada_conferencia_user_id" in nota_indexes:
        op.drop_index(
            "ix_notas_entrada_conferencia_user_id", table_name="notas_entrada"
        )
