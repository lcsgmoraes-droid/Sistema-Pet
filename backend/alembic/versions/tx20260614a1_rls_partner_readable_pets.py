"""enable partner-readable RLS on clientes and pets

Revision ID: tx20260614a1
Revises: tw20260614a1
...
from app.tenant_rls_migration import TENANT_RLS_GUARD


revision = "tx20260614a1"
down_revision = "tw20260614a1"
branch_labels = None
depends_on = None


PARTNER_READABLE_RLS_TABLES = ("clientes", "pets")
PREEXISTING_STANDARD_RLS_TABLES = ("clientes",)
NEW_RLS_TABLES = ("pets",)
...
def _present_tables() -> list[str]:
    ...
    return [
        table_name
        for table_name in PARTNER_READABLE_RLS_TABLES
...
def _restore_standard_clientes_rls(table_name: str) -> None:
    op.execute(f"DROP POLICY IF EXISTS {table_name}_tenant_isolation ON {table_name}")
    op.execute(
...
def _disable_new_rls_table(table_name: str) -> None:
...
def downgrade() -> None:
    for table_name in reversed(_present_tables()):
        ...
        if table_name in NEW_RLS_TABLES:
            _disable_new_rls_table(table_name)
        elif table_name in PREEXISTING_STANDARD_RLS_TABLES:
            _restore_standard_clientes_rls(table_name)
