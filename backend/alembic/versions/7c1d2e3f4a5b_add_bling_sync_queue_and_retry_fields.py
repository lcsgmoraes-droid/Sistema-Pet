...
def _add_missing_sync_columns(inspector) -> None:
    ...
    for nome, tipo, nullable, default in SYNC_COLUMNS:
        ...
        op.add_column(
            "produto_bling_sync", sa.Column(nome, tipo, nullable=nullable, **argumentos)
        )
...
def _create_queue_table_if_missing(inspector) -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "forcar_sync", sa.Boolean(), server_default=sa.text("false"), nullable=False
        ),
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_produto_bling_sync_queue_produto_id ON produto_bling_sync_queue (produto_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_produto_bling_sync_queue_sync_id ON produto_bling_sync_queue (sync_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_produto_bling_sync_queue_tenant_id ON produto_bling_sync_queue (tenant_id)"
    )
