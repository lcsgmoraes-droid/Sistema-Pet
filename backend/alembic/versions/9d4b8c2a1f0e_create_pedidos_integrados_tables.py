...
def upgrade() -> None:
    ...
    if not inspector.has_table("pedidos_integrados"):
        op.create_table(
            ...
            sa.Column(
                "status", sa.String(length=30), server_default="aberto", nullable=False
            ),
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_pedidos_integrados_tenant_id ON pedidos_integrados (tenant_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_pedidos_integrados_pedido_bling_id ON pedidos_integrados (pedido_bling_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_pedidos_integrados_status ON pedidos_integrados (status)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_pedidos_integrados_expira_em ON pedidos_integrados (expira_em)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_pedidos_integrados_itens_tenant_id ON pedidos_integrados_itens (tenant_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_pedidos_integrados_itens_pedido_integrado_id ON pedidos_integrados_itens (pedido_integrado_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_pedidos_integrados_itens_sku ON pedidos_integrados_itens (sku)"
    )
...
def downgrade() -> None:
    ...
    if inspector.has_table("pedidos_integrados_itens"):
        ...
        op.execute(
            "DROP INDEX IF EXISTS ix_pedidos_integrados_itens_pedido_integrado_id"
        )
