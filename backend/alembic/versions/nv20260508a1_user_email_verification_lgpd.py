...
def upgrade() -> None:
    ...
    if "privacy_version" not in columns:
        op.add_column("users", sa.Column("privacy_version", sa.String(length=50), nullable=True))
    if "consent_ip" not in columns:
        op.add_column("users", sa.Column("consent_ip", sa.String(length=50), nullable=True))
