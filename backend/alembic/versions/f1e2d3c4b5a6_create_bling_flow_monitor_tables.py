...
def upgrade() -> None:
    ...
    if not inspector.has_table("bling_flow_events"):
        op.create_table(
            "bling_flow_events",
            sa.Column(
                "source", sa.String(length=30), nullable=False, server_default="runtime"
            ),
    ...
    if not inspector.has_table("bling_flow_incidents"):
        op.create_table(
            ...
            sa.Column(
                ...
                sa.String(length=30),
                nullable=False,
                server_default="auditoria",
            ),
            sa.Column(
                "scope", sa.String(length=30), nullable=False, server_default="pedido"
            ),
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_events_tenant_id ON bling_flow_events (tenant_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_events_pedido_integrado_id ON bling_flow_events (pedido_integrado_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_events_tenant_created_at ON bling_flow_events (tenant_id, created_at)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_events_pedido_bling_id ON bling_flow_events (pedido_bling_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_events_nf_bling_id ON bling_flow_events (nf_bling_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_events_event_type ON bling_flow_events (event_type)"
    )

    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_incidents_tenant_id ON bling_flow_incidents (tenant_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_incidents_pedido_integrado_id ON bling_flow_incidents (pedido_integrado_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_incidents_tenant_status ON bling_flow_incidents (tenant_id, status)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_incidents_code ON bling_flow_incidents (code)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_incidents_dedupe_key ON bling_flow_incidents (dedupe_key)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_bling_flow_incidents_pedido_bling_id ON bling_flow_incidents (pedido_bling_id)"
    )
