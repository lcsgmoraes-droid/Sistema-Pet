...
def upgrade() -> None:
    ...
    if inspector.has_table("categorias_financeiras"):
        columns = {column["name"] for column in inspector.get_columns("categorias_financeiras")}
        if "tipo_custo" not in columns:
            op.add_column("categorias_financeiras", sa.Column("tipo_custo", sa.String(length=20), nullable=True))
    ...
    for index_name, columns in webhook_indexes.items():
        if index_name not in indexes:
            op.create_index(index_name, "bling_pedido_webhook_events", columns, unique=False)
...
def downgrade() -> None:
    ...
    if inspector.has_table("bling_pedido_webhook_events"):
        ...
        op.execute("DROP INDEX IF EXISTS ix_bling_pedido_webhook_events_next_attempt_at")
        op.execute("DROP INDEX IF EXISTS ix_bling_pedido_webhook_events_status")
        op.execute("DROP INDEX IF EXISTS ix_bling_pedido_webhook_events_pedido_bling_id")
