...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # JSON completo
        sa.Column(
            "stone_response", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
        # Controle de erro
    ...
    op.create_index(
        "ix_stone_transactions_stone_payment_id",
        "stone_transactions",
        ["stone_payment_id"],
        unique=True,
    )
    op.create_index(
        "ix_stone_transactions_external_id",
        "stone_transactions",
        ["external_id"],
        unique=True,
    )
    ...
    op.create_index(
        "ix_stone_transactions_venda_id", "stone_transactions", ["venda_id"]
    )
    op.create_index(
        "ix_stone_transactions_created_at", "stone_transactions", ["created_at"]
    )
    op.create_index(
        "ix_stone_transactions_tenant_id", "stone_transactions", ["tenant_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "transaction_id",
            sa.Integer(),
            sa.ForeignKey("stone_transactions.id"),
            nullable=False,
        ),
        # Tipo de evento
        ...
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column(
            "webhook_data", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_stone_transaction_logs_transaction_id",
        "stone_transaction_logs",
        ["transaction_id"],
    )
    op.create_index(
        "ix_stone_transaction_logs_created_at", "stone_transaction_logs", ["created_at"]
    )
    op.create_index(
        "ix_stone_transaction_logs_tenant_id", "stone_transaction_logs", ["tenant_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "enable_credit_card", sa.Boolean(), nullable=True, server_default="true"
        ),
        sa.Column(
            "enable_debit_card", sa.Boolean(), nullable=True, server_default="false"
        ),
        sa.Column("max_installments", sa.Integer(), nullable=True, server_default="12"),
        # Webhook
        sa.Column("webhook_url", sa.String(500), nullable=True),
        sa.Column(
            "webhook_enabled", sa.Boolean(), nullable=True, server_default="true"
        ),
        # Status
