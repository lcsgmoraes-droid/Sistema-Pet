...
def upgrade() -> None:
    ...
    for table_name in NAMED_OPTION_TABLES:
        ...
        op.execute(
            f"CREATE INDEX IF NOT EXISTS ix_{table_name}_nome ON {table_name} (nome)"
        )

    if not inspector.has_table("apresentacoes_peso"):
        _create_package_weights_table()
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_apresentacoes_peso_tenant_id ON apresentacoes_peso (tenant_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_apresentacoes_peso_peso_kg ON apresentacoes_peso (peso_kg)"
    )
