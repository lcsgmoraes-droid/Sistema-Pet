...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "total_baias", sa.Integer(), nullable=False, server_default=sa.text("12")
        ),
    ...
    op.create_index(
        "ix_vet_internacao_configuracoes_tenant_id",
        "vet_internacao_configuracoes",
        ["tenant_id"],
    )
    op.create_index(
        "ix_vet_internacao_configuracoes_user_id",
        "vet_internacao_configuracoes",
        ["user_id"],
    )
    op.create_index(
        "ux_vet_internacao_config_tenant",
        "vet_internacao_configuracoes",
        ["tenant_id"],
        unique=True,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(length=30), nullable=False, server_default="agendado"
        ),
        sa.Column("id", sa.Integer(), sa.Identity(always=True), nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column(
        ...
        sa.ForeignKeyConstraint(
            ["procedimento_evolucao_id"], ["vet_evolucoes_internacao.id"]
        ),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"]),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_index(
        "ix_vet_internacao_procedimentos_agenda_internacao_id",
        "vet_internacao_procedimentos_agenda",
        ["internacao_id"],
    )
    op.create_index(
        "ix_vet_internacao_procedimentos_agenda_pet_id",
        "vet_internacao_procedimentos_agenda",
        ["pet_id"],
    )
    op.create_index(
    ...
    op.create_index(
        "ix_vet_internacao_procedimentos_agenda_status",
        "vet_internacao_procedimentos_agenda",
        ["status"],
    )
    op.create_index(
        "ix_vet_internacao_procedimentos_agenda_tenant_id",
        "vet_internacao_procedimentos_agenda",
        ["tenant_id"],
    )
    op.create_index(
        "ix_vet_internacao_procedimentos_agenda_user_id",
        "vet_internacao_procedimentos_agenda",
        ["user_id"],
    )
    ...
    op.alter_column(
        "vet_internacao_procedimentos_agenda", "lembrete_minutos", server_default=None
    )
    op.alter_column(
        "vet_internacao_procedimentos_agenda", "status", server_default=None
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ux_vet_internacao_config_tenant", table_name="vet_internacao_configuracoes"
    )
    op.drop_index(
        "ix_vet_internacao_configuracoes_user_id",
        table_name="vet_internacao_configuracoes",
    )
    op.drop_index(
        "ix_vet_internacao_configuracoes_tenant_id",
        table_name="vet_internacao_configuracoes",
