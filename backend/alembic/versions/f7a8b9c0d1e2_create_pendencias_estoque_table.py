...
def upgrade() -> None:
    ...
    if not inspector.has_table("pendencias_estoque"):
        op.create_table(
            ...
            sa.Column(
                "venda_id", sa.Integer(), sa.ForeignKey("vendas.id"), nullable=True
            ),
    ...
    existing_indexes = (
        {idx["name"] for idx in inspector.get_indexes("pendencias_estoque")}
    ...
    if "ix_pendencias_estoque_tenant_id" not in existing_indexes:
        op.create_index(
            "ix_pendencias_estoque_tenant_id", "pendencias_estoque", ["tenant_id"]
        )
    if "ix_pendencias_estoque_cliente_id" not in existing_indexes:
        op.create_index(
            "ix_pendencias_estoque_cliente_id", "pendencias_estoque", ["cliente_id"]
        )
    if "ix_pendencias_estoque_produto_id" not in existing_indexes:
        op.create_index(
            "ix_pendencias_estoque_produto_id", "pendencias_estoque", ["produto_id"]
        )
    if "ix_pendencias_estoque_status" not in existing_indexes:
        op.create_index(
            "ix_pendencias_estoque_status", "pendencias_estoque", ["status"]
        )
