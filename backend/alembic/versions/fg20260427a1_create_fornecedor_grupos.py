...
def upgrade() -> None:
    ...
    if not inspector.has_table("fornecedor_grupos"):
        op.create_table(
            ...
            sa.UniqueConstraint(
                "tenant_id", "nome", name="uq_fornecedor_grupos_tenant_nome"
            ),
        )

    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fornecedor_grupos_tenant_id ON fornecedor_grupos (tenant_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fornecedor_grupos_nome ON fornecedor_grupos (nome)"
    )
    op.execute(
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fornecedor_grupos_tenant_ativo ON fornecedor_grupos (tenant_id, ativo)"
    )
    ...
    if inspector.has_table("clientes"):
        ...
        if "fornecedor_grupo_id" not in cliente_columns:
            op.add_column(
                "clientes",
                sa.Column("fornecedor_grupo_id", sa.Integer(), nullable=True),
            )
        if "ix_clientes_fornecedor_grupo_id" not in cliente_indexes:
            op.create_index(
                "ix_clientes_fornecedor_grupo_id", "clientes", ["fornecedor_grupo_id"]
            )
...
def downgrade() -> None:
    ...
    if inspector.has_table("clientes"):
        ...
        if "fk_clientes_fornecedor_grupo_id" in cliente_fks:
            op.drop_constraint(
                "fk_clientes_fornecedor_grupo_id", "clientes", type_="foreignkey"
            )
