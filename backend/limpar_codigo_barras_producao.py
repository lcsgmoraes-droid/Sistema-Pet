...
def main():
    ...
    with Session(engine) as session:
        ...
        produtos_pai = (
            ...
            .filter((Produto.tipo_produto == "PAI") | (Produto.is_parent))
            .all()
        )
        ...
        produtos_variacao = (
            ...
            .filter(
                ...
                | (Produto.produto_pai_id.is_not(None))
            )
            .all()
        )
        ...
        total_com_codigo = sum(
            1 for p in produtos_pai + produtos_variacao if p.codigo_barras
        )
        ...
        if args.visualizar:
            ...
            backup_file = (
                backup_dir
                / f"backup_codigo_barras_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            ...
            print(
                f"    VARIAÇÕES: {sum(1 for p in produtos_variacao if p.codigo_barras)}"
            )
            ...
            try:
                ...
                print("\n✅ SUCESSO!")
                print(
                    f"   ✓ {count_pai} código(s) de barras removido(s) de produtos PAI"
                )
                print(
                    f"   ✓ {count_variacao} código(s) de barras removido(s) de VARIAÇÕES"
                )
