...
# Buscar ID da venda
venda = db.execute(
    text("SELECT id FROM vendas WHERE numero_venda = :numero"), {"numero": numero_venda}
).fetchone()
...
try:
    ...
    if comissoes_itens:
        ...
        for com in comissoes_itens:
            print(
                f"      - ID {com[0]}: Funcionário {com[1]}, Valor R$ {com[2]}, {com[3]}%, Status: {com[4]}"
            )
    else:
