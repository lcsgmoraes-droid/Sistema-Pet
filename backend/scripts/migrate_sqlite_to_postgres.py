...
class MigrationStats:
    ...
    def add_table(self, table_name: str, records: int, elapsed: float):
        ...
        self.table_stats[table_name] = {"records": records, "elapsed": elapsed}
    ...
    def print_summary(self):
        ...
        sorted_tables = sorted(
            self.table_stats.items(), key=lambda x: x[1]["records"], reverse=True
        )[:10]
...
def convert_value_for_postgres(
    value: Any, column_type: str, column_name: str = ""
) -> Any:
    """
    ...
    if value == "":
        if any(
            t in column_type.upper()
            for t in ["INTEGER", "NUMERIC", "DECIMAL", "FLOAT", "DOUBLE"]
        ):
            return None
...
def migrate_table(
    sqlite_engine, postgres_engine, table_name: str, batch_size: int = 1000
) -> int:
    """
            ...
            for i in range(0, len(rows), batch_size):
                ...
                for row in batch:
                    ...
                    for idx, col in enumerate(common_columns):  # Usar common_columns
                        ...
                        value = convert_value_for_postgres(
                            value, column_type, col
                        )  # Passar nome da coluna
                ...
                if batch_dicts:
                    ...
                    for row_dict in batch_dicts:
                        ...
                        is_debug_venda = (
                            table_name == "vendas"
                            and row_dict.get("id") in DEBUG_VENDAS_IDS
                        )
                        ...
                        try:
                            if is_debug_venda:
                                print(f"\n{'=' * 80}")
                                print(
                                    f"DEBUG: Tentando inserir venda ID {row_dict['id']}"
                                )
                                print("Dados completos:")
                                for key, val in row_dict.items():
                                    print(
                                        f"  {key}: {val!r} (tipo: {type(val).__name__})"
                                    )
                                print(
                                    f"Query: INSERT INTO {table_name} ({columns_str}) VALUES (...)"
                                )
                                print(f"{'=' * 80}")
                            ...
                            if result.rowcount > 0:
                                ...
                                if is_debug_venda:
                                    print(
                                        f"SUCESSO: Venda ID {row_dict['id']} inserida!\n"
                                    )
                            else:
                                records_skipped += 1
                            ...
                            if "duplicate key" in error_msg.lower():
                                ...
                                if (
                                    records_inserted < 5 and not is_debug_venda
                                ):  # Mostrar apenas primeiros erros
                                    print(f"\n  Erro: {short_msg}")
...
def verify_migration(sqlite_engine, postgres_engine) -> Dict[str, Dict[str, int]]:
    ...
    for table_name in MIGRATION_ORDER:
        ...
        try:
            ...
            print(
                f"{status} {table_name:30s}: SQLite={sqlite_count:>6,}  PostgreSQL={postgres_count:>6,}"
            )
    ...
    matching_tables = sum(
        1 for stats in comparison.values() if stats.get("match", False)
    )
