...
- Título/subtítulo/rótulo da tela e de `CartaoIndicador`/`SeletorOpcoes` usam as classes de `frontend/src/styles/v2-tipografia.css` (`.v2-titulo-pagina`, `.v2-titulo-secao`, `.v2-subtitulo`, `.v2-rotulo`) — não são componentes React, são classes Tailwind com `@apply`; mudar o tamanho de um papel é editar a classe uma vez, não caçar a string repetida em cada arquivo
