...
|---|---|---|---|---|
| Login | `/login` | Concluído | 2026-09-12 | Movida para `pages/v2/Login.jsx` (era `pages/Login.jsx`; import atualizado em `lazyPages.jsx`, rota `/login` sem mudança). Ver detalhe abaixo e [[Autenticacao]] para o fluxo de login multiempresa que a tela implementa (lógica 100% preservada). |
...
- **Botão de submit nativo → `BotaoSalva`.** Texto ("Entrar"/"Entrando...") e `disabled` continuam controlados pela tela; a cor/formato agora vêm do componente.
