...
STATS = {
    'especies': {'total': 0, 'sucesso': 0, 'erro': 0},
    'racas': {'total': 0, 'sucesso': 0, 'erro': 0},
    'clientes': {'total': 0, 'sucesso': 0, 'erro': 0},
    'marcas': {'total': 0, 'sucesso': 0, 'erro': 0},
    'produtos': {'total': 0, 'sucesso': 0, 'erro': 0},
    'pets': {'total': 0, 'sucesso': 0, 'erro': 0},
    'vendas': {'total': 0, 'sucesso': 0, 'erro': 0},
    'itens_venda': {'total': 0, 'sucesso': 0, 'erro': 0},
}
...
def exibir_resumo():
    ...
    print("\n" + "="*70)
    print("RESUMO DA IMPORTACAO".center(70))
    print("="*70)
    
    for entidade, stats in STATS.items():
        if stats['total'] > 0:
            taxa = (stats['sucesso'] / stats['total']) * 100 if stats['total'] > 0 else 0
            print(f"{entidade.upper():15} | Total: {stats['total']:4} | OK {stats['sucesso']:4} | ERR {stats['erro']:4} | {taxa:.1f}%")
