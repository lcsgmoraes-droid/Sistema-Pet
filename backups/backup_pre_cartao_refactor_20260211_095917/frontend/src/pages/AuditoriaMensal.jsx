...
export default function AuditoriaMensal() {
  ...
  const buscar = async () => {
    ...
    try {
      ...
      toast.error("Erro ao buscar auditoria de provisões");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusBadge = (status, emoji) => {
    const classes = {
      OK: "bg-green-100 text-green-800 border-green-300",
      AJUSTE: "bg-yellow-100 text-yellow-800 border-yellow-300",
      ACUMULANDO: "bg-blue-100 text-blue-800 border-blue-300",
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-sm font-medium border ${
          classes[status] || classes.SEM_DADOS
        }`}
      >
            ...
            <table className="min-w-full divide-y divide-gray-200">
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="text-sm text-gray-600">
                        {formatarPercentual(item.percentual_realizado)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      {getStatusBadge(item.status, item.status_emoji)}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-gray-100">
                <tr>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="text-sm font-bold text-gray-900">
                      {formatarMoeda(dados.total_provisao)}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="text-sm font-bold text-gray-900">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    ...
                    >
                      {dados.total_diferenca > 0 && "+"}
                      {formatarMoeda(dados.total_diferenca)}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="text-sm font-bold text-gray-900">
