...
def relatorio_margem_produto(
    ...
    logger.warning(f"🔍 [RELATORIO-MARGEM-PRODUTO] User: {current_user.email} | Tenant ID: {tenant_id}")
...
def relatorio_produtos_prejudiciais(
    ...
    dados_margem = relatorio_margem_produto(data_inicio, data_fim, None, None, db, user_and_tenant)
...
def exportar_csv(
    ...
    if tipo == "margem-produto":
        dados = relatorio_margem_produto(data_inicio, data_fim, None, None, db, user_and_tenant)
        linhas = dados['dados']
        ...
        dados = relatorio_produtos_prejudiciais(data_inicio, data_fim, 20, db, user_and_tenant)
        linhas = dados['dados']
